...
<xsl:stylesheet
    ...
    version="1.0">
<xsl:template name="title" match="h:h1">
    <xsl:comment>
        <xsl:text>Title taken from HTML H1 </xsl:text>
    </xsl:comment>

    <text:p text:style-name="Title"><xsl:value-of select="."/></text:p>

</xsl:template>

<xsl:template name="section" match="h:h2|h:h3|h:h4|h:h5|h:h6">

    <!-- compute level of section -->
    <xsl:variable name="level">
        <xsl:value-of select="number(substring-after(local-name(.),'h')) - $heading_minus_level"/>
    </xsl:variable>
