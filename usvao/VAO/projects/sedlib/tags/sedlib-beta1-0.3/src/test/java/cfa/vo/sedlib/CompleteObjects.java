...
public class CompleteObjects
{
     ...
     */
    static public ArrayOfParam createParams()
    {
	ArrayOfParam result = new ArrayOfParam();
	...
	result.createParam().add( new DoubleParam( 3.14159, "pi", "TBD", "TBD") );
	result.getParam().add( new IntParam( 7, "daysofweek", "TBD", "TBD") );
	result.getParam().add( new TextParam( "Galaxy", "TargetType", "TBD" ) );
	result.getParam().add( new DateParam( "2011-01-04T22:01:59", "camdate", "TBD" ) );
	result.getParam().add( new TimeParam(  "1.8", "age", "TBD", "Gyr" ) );

	// PositionParam does not extend Param.. but is a list of DoubleParam
	result.getParam().add( pos.getValue ()[0] );
        result.getParam().add( pos.getValue ()[1] );
