...
public class FitsMapper extends SedMapper
{
    ...
    /**
      ...
      * @throws SedException
      */
    public Sed populateSed (Object data, Sed sed) throws SedException
    {
        Segment segment;
        BasicHDU[] hdus;
        ...
        try
        {
            ...
            throw new SedException (exp.getMessage(), exp);
        }
     ...
     *    {@link Segment}
     */
    private Segment extractSegmentFromTable(TableHDU table) throws SedException
    {
        Segment segment;
    ...
    private void loadFitsKeywordsIntoSegments(
                                        Header header,
                                        Segment segment ) throws SedException
    {
            ...
            for (int enumKey : enumKeys)
            {
                switch ( enumKey )
                {
                    ...
                    case FitsKeywords.TARGET_REDSHIFT:
                        segment.createTarget ().setRedshift (new DoubleParam (value, null, null, unit));
                        break;
                    ...
                    case FitsKeywords.TARGET_VARAMPL:
                        segment.createTarget ().setVarAmpl (new DoubleParam (value, null, null, unit));
                        break;
                    case FitsKeywords.TARGET_RA:
                    {
                        ...
                        targetPos[0] = new DoubleParam (value, null, null, unit);
                        break;
                    }
                    case FitsKeywords.TARGET_DECL:
                    {
                        ...
                        targetPos[1] = new DoubleParam (value, null, null, unit);
                        break;
                    }

                    // Segment level cases
                    case FitsKeywords.SEDTYPE:
                        segment.setType( new TextParam( value) );
                        break;
                    ...
                    case FitsKeywords.SEG_CS_SPECTRALFRAME_REDSHIFT:
                    {
                        ...
                        ((SpectralFrame)coordFrame).setRedshift (new DoubleParam(value, null, null, unit));
                        break;
                    }
...
*/
                    ...
                    case FitsKeywords.SEG_CS_SPACEFRAME_EQUINOX:
                    {
                        ...
                        ((SpaceFrame)coordFrame).setEquinox (new DoubleParam (value, null, null, unit));
                        break;
                    }
                    ...
                    case FitsKeywords.SEG_CS_TIMEFRAME_ZERO:
                    {
                        ...
                        ((TimeFrame)coordFrame).setZero (new DoubleParam (value, null, null, unit));
                        break;
                    }
                    ...
                    case FitsKeywords.SEG_DD_SNR:
                        segment.createDerived().setSNR( new DoubleParam( value, null, null, unit ) );
                    break;

                    case FitsKeywords.SEG_DD_REDSHIFT_VALUE:
                        segment.createDerived().createRedshift().setValue( new DoubleParam( value, null, null, unit ) );
                    break;

                    case FitsKeywords.SEG_DD_REDSHIFT_ACC_STATERR:
                        segment.createDerived().createRedshift().createAccuracy().setStatError( new DoubleParam( value, null, null, unit ) );
                    break;

                    case FitsKeywords.SEG_DD_REDSHIFT_ACC_CONFIDENCE:
                        segment.createDerived().createRedshift().createAccuracy().setConfidence( new DoubleParam( value, null, null, unit ) );
                    break;

                    case FitsKeywords.SEG_DD_VARAMPL:
                        segment.createDerived().setVarAmpl( new DoubleParam( value, null, null, unit ) );
                    break;
                    ...
                    case FitsKeywords.SEG_CHAR_FLUXAXIS_ACC_STATERR:
                        segment.createChar().createFluxAxis().createAccuracy ().setStatError(new DoubleParam ( value, null, null, unit ));
                        break;
                    case FitsKeywords.SEG_CHAR_FLUXAXIS_ACC_SYSERR:
                        segment.createChar().createFluxAxis().createAccuracy ().setSysError(new DoubleParam ( value, null, null, unit ));
                        break;
                    ...
                    case FitsKeywords.SEG_CHAR_SPECTRALAXIS_COV_LOC_VALUE:
                    {
                        ...
                        if (valueList[0] == null)
                            valueList[0] = new DoubleParam( value, null, null, unit );
                        else
                            valueList[1] = new DoubleParam( value, null, null, unit );
                    }
                    break;

                    case FitsKeywords.SEG_CHAR_SPECTRALAXIS_COV_BOUNDS_EXTENT:
                        segment.createChar ().createSpectralAxis ().createCoverage ().createBounds ().setExtent (new DoubleParam (value, null, null, unit));
                    break;

                    case FitsKeywords.SEG_CHAR_SPECTRALAXIS_COV_BOUNDS_MIN:
                        segment.createChar ().createSpectralAxis ().createCoverage ().createBounds ().createRange ().setMin (new DoubleParam (value, null, null, unit));
                    break;

                    case FitsKeywords.SEG_CHAR_SPECTRALAXIS_COV_BOUNDS_MAX:
                        segment.createChar ().createSpectralAxis ().createCoverage ().createBounds ().createRange ().setMax (new DoubleParam (value, null, null, unit));
                    break;
                    case FitsKeywords.SEG_CHAR_SPECTRALAXIS_COV_SUPPORT_EXTENT:
                        segment.createChar ().createSpectralAxis ().createCoverage ().createSupport ().setExtent (new DoubleParam (value, null, null, unit));
                    break;


                    case FitsKeywords.SEG_CHAR_SPECTRALAXIS_SAMPPREC_SAMPPRECREFVAL_FILL:
                        segment.createChar ().createSpectralAxis ().createSamplingPrecision ().createSamplingPrecisionRefVal ().setFillFactor (new DoubleParam (value, null, null, unit));
                    break;

                    case FitsKeywords.SEG_CHAR_SPECTRALAXIS_ACC_BINSIZE:
                        segment.createChar ().createSpectralAxis ().createAccuracy ().setBinSize (new DoubleParam (value, null, null, unit));
                    break;

                    case FitsKeywords.SEG_CHAR_SPECTRALAXIS_ACC_STATERR:
                        segment.createChar ().createSpectralAxis ().createAccuracy ().setStatError (new DoubleParam (value, null, null, unit));
                    break;

                    case FitsKeywords.SEG_CHAR_SPECTRALAXIS_ACC_SYSERR:
                        segment.createChar ().createSpectralAxis ().createAccuracy ().setSysError (new DoubleParam (value, null, null, unit));
                    break;
                    ...
                    case FitsKeywords.SEG_CHAR_SPECTRALAXIS_RESOLUTION:
                        segment.createChar ().createSpectralAxis ().setResolution( new DoubleParam( value, null, null, unit ) );
                    break;

                    case FitsKeywords.SEG_CHAR_SPECTRALAXIS_RESPOW:
                        segment.createChar ().createSpectralAxis ().setResPower( new DoubleParam( value, null, null, unit ) );
                    break;
                    ...
                    case FitsKeywords.SEG_CHAR_SPECTRALAXIS_SAMPPREC_SAMPEXT:
                    	segment.createChar ().createSpectralAxis().createSamplingPrecision().setSampleExtent(new DoubleParam (value, null, null, unit));
                    break;
                    ...
                    case FitsKeywords.SEG_CHAR_SPATIALAXIS_RESOLUTION:
                        segment.createChar ().createSpatialAxis ().setResolution( new DoubleParam( value, null, null, unit ) );
                    break;
                    ...
                    case FitsKeywords.SEG_CHAR_SPATIALAXIS_COV_BOUNDS_EXTENT:
                        segment.createChar ().createSpatialAxis ().createCoverage ().createBounds ().setExtent( new DoubleParam( value, null, null, unit ) );
                    break;
                    ...
                    case FitsKeywords.SEG_CHAR_SPATIALAXIS_COV_SUPPORT_EXTENT:
                        segment.createChar ().createSpatialAxis ().createCoverage ().createSupport ().setExtent( new DoubleParam( value, null, null, unit ) );
                    break;

                    case FitsKeywords.SEG_CHAR_SPATIALAXIS_ACC_STATERR:
                        segment.createChar ().createSpatialAxis ().createAccuracy ().setStatError( new DoubleParam( value, null, null, unit ) );
                    break;

                    case FitsKeywords.SEG_CHAR_SPATIALAXIS_ACC_SYSERR:
                        segment.createChar ().createSpatialAxis ().createAccuracy ().setSysError( new DoubleParam( value, null, null, unit ) );
                    break;

                    case FitsKeywords.SEG_CHAR_SPATIALAXIS_SAMPPREC_SAMPPRECREFVAL_FILL:
                        segment.createChar ().createSpatialAxis ().createSamplingPrecision ().createSamplingPrecisionRefVal ().setFillFactor (new DoubleParam (value, null, null, unit));
                    break;
   
                    case FitsKeywords.SEG_CHAR_SPATIALAXIS_SAMPPREC_SAMPEXT:
                        segment.createChar ().createSpatialAxis ().createSamplingPrecision ().setSampleExtent (new DoubleParam (value, null, null, unit));
                    break;
                    ...
                    case FitsKeywords.SEG_CHAR_TIMEAXIS_ACC_BINSIZE:
                        segment.createChar ().createTimeAxis ().createAccuracy ().setBinSize( new DoubleParam( value, null, null, unit ) );
                    break;
                    
                    case FitsKeywords.SEG_CHAR_TIMEAXIS_ACC_STATERR:
                        segment.createChar ().createTimeAxis ().createAccuracy ().setStatError( new DoubleParam( value, null, null, unit ) );
                    break;
                    
                    case FitsKeywords.SEG_CHAR_TIMEAXIS_ACC_SYSERR:
                        segment.createChar ().createTimeAxis ().createAccuracy ().setSysError( new DoubleParam( value, null, null, unit ) );
                    break;
                    ...
                    case FitsKeywords.SEG_CHAR_TIMEAXIS_RESOLUTION:
                        segment.createChar ().createTimeAxis ().setResolution( new DoubleParam( value, null, null, unit ) );
                    break;
                    ...
                    case FitsKeywords.SEG_CHAR_TIMEAXIS_COV_BOUNDS_EXTENT:
                        segment.createChar ().createTimeAxis ().createCoverage ().createBounds ().setExtent (new DoubleParam (value, null, null, unit));
                    break;

                    case FitsKeywords.SEG_CHAR_TIMEAXIS_COV_BOUNDS_MIN:
                        segment.createChar ().createTimeAxis ().createCoverage ().createBounds ().createRange ().setMin (new DoubleParam (value, null, null, unit));
                    break;

                    case FitsKeywords.SEG_CHAR_TIMEAXIS_COV_BOUNDS_MAX:
                        segment.createChar ().createTimeAxis ().createCoverage ().createBounds ().createRange ().setMax (new DoubleParam (value, null, null, unit));
                    break;

                    case FitsKeywords.SEG_CHAR_TIMEAXIS_COV_SUPPORT_EXTENT:
                        segment.createChar ().createTimeAxis ().createCoverage ().createSupport ().setExtent (new DoubleParam (value, null, null, unit));
                    break;

                    case FitsKeywords.SEG_CHAR_TIMEAXIS_SAMPPREC_SAMPPRECREFVAL_FILL:
                        segment.createChar ().createTimeAxis ().createSamplingPrecision ().createSamplingPrecisionRefVal ().setFillFactor (new DoubleParam (value, null, null, unit));
     ...
     */
    private void loadDataIntoSegment( TableHDU table, Segment segment ) throws SedException 
    {
        Object[] row;
        int columns;
        ...
        if (table.getNRows() > 1)
           throw new SedException ("An Unexpected number of rows were found. Expected one row per segment, but found " + table.getNRows () + " rows.");
            ...
            switch ( dataType )
            {
                case IOConstants.FITS_FORMAT_CODE_INTEGER16:
                {
                    ...
                    { 
                        this.setPointDataField( pointList.get(jj),
                                utype,
                                new IntParam ((int)data[jj], name, ucd, unit));
                    }
                }
                break;
                ...
                case IOConstants.FITS_FORMAT_CODE_DOUBLE_PRECISION:
                {
                    ...
                    for ( int jj = 0; jj <  data.length; jj++ )
                    {
                        this.setPointDataField( pointList.get(jj),
                                utype,
                                new DoubleParam ((double)data[jj], name, ucd, unit));
                    }
                }
                break;
     ...
     */
    int getMaxFieldDepth( Object[] tableRow ) throws SedException
    {
        int maxDepth = 0;
     ...
     */
    private void matchMetaDataWithData (Segment segment) throws SedException
    {
        ...
        if (data instanceof ArrayOfFlatPoint)
            throw new SedException ("Flat points are currently not supported.");
        else
