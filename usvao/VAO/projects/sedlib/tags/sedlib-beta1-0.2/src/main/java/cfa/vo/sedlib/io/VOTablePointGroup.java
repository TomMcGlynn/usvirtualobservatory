...
class VOTablePointGroup
{
    ...
    protected Map <Integer, VOTablePointGroup> subgroups;
    protected int utype;

    public VOTablePointGroup (int utype)
    {
        constants = new ArrayList <Boolean> ();
        data = new ArrayList <List<Param>> ();
        subgroups = new HashMap <Integer, VOTablePointGroup> ();
    ...
    public void addData (int row, Param param)
    {
        ...
        if (!firstParam.getValue ().equals (param.getValue ()))
            this.constants.set (column, false);
    }

    public VOTablePointGroup createGroup (int utype)
    {
        VOTablePointGroup subgroup = new VOTablePointGroup (utype);
        if (this.subgroups.containsKey (utype))
            subgroup = this.subgroups.get (utype);
        else
        {
            subgroup = new VOTablePointGroup (utype);
            this.subgroups.put (utype, subgroup);
        }
    ...
    public void addToVOTable (VOElement parent, String namespace)
    {
        ...
        for(VOTablePointGroup subgroup : this.subgroups.values())
