...
vot_writeFITS (handle_t vot, char *oname)
{
	...
	i = 0;
	    ...
	    tform[i] = calloc (1, 16);
fprintf (stderr, "dtype[%d]='%s'  %d  asize='%s'\n", 
  i, dtype, strncasecmp(dtype,"bool",4), (asize[0] ? asize : "*"));

	    if (strncasecmp (dtype, "char", 4) == 0 ||
	        strncasecmp (dtype, "bool", 4) == 0 ||
	        strncasecmp (dtype, "unsignedByte", 12) == 0) {
fprintf (stderr, "doing char/bool/byte ....\n");
		    ...
		    if (asize[0]) {
		        ...
		        strcpy (tform[i], "A");
fprintf (stderr, "done char/bool/byte ....\n");
...
fprintf (stderr, "writeFITS: j=%d  tform='%c' fmt='%s'\n", j, tform[type], fmt[j]);
	switch (tform[type]) {
	case 'A':						/* CHAR	    */
	...
	case 'I':						/* SHORT    */
	    ...
	    if (width == 0) {
	        ...
	        fits_write_col (fp, TSHORT, j+1, frow,felem, nrows, scol, 
			&status);
	    } else {
