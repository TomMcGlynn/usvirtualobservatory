 ...
 *	    -s,--seq 		    Use sequential file numbers
...
static Task  self       = {  "votget",  votget };
static char  *opts      = "%hb:e:f:dstu:o:vxA:BCDF:N:Sm:";
static struct option long_opts[] = {
        { "base",         1, 0,   'b'},         /* task option          */
        { "extn",         1, 0,   'e'},         /* task option          */
        { "fmt",          1, 0,   'f'},         /* task option          */
        { "seq",          2, 0,   's'},         /* task option          */
...
votget (int argc, char **argv, size_t *reslen, void **result)
{
    ...
    *result = NULL;
    ...
    while ((ch = vo_paramNext (opts,long_opts,argc,pargv,optval,&pos)) != 0) {
	    ...
	    case 'A':  acol = atoi (optval); 		break;
	    case 'B':  detach++;			break;
	    case 'C':  isCache++;			break;
	    case 'D':  dir = strdup (optval);		break;
	    case 'F':  tcol = atoi (optval); 		break;
	    case 'N':  nthreads = atoi (optval); 	break;
	    case 'O':  force++;				break;
...
vot_procFile (char *iname)
{
    ...
    case -1:  	stat = ERR; 			return (ERR);
    case  0:    vot_loadText (iname); 		break;
    case  1:    vot_loadVOTable (iname); 	break;
    }
...
Tests (char *input)
{
   ...
   /*  Extract access references. */
   vo_taskTest (self, "-x", input, NULL);
   vo_taskTest (self, "-x", "-o", "/tmp/acref", input, NULL); 

   if (access ("/tmp/acref", F_OK) == 0)
	unlink ("/tmp/acref");
}
