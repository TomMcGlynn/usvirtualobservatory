...
static Task  self	= {  "vosloanspec",  vosloanspec };
static char  *opts 	= "%:hrvdDP:N:R:ST:s:t:z:cl:uf:o:bm";
...
vosloanspec (int argc, char **argv, size_t *reslen, void **result)
{
    ...
    *result = NULL;
	...
	argnum++;
	    ...
	    case 'o':  	object 	 = NEXTARG (optval);		
			if (argv[argnum]) {
		    	    ...
		    	    sr = voc_nameResolver (argv[argnum]);
    ...
    if (dec != -999.9) strcat (url, "&dec="), strcat (url, dec_str); 
...
Usage (void)
{
  fprintf (stderr, "\n  Usage:\n\t"
    ...
    "\n\n"
    "  Examples:\n\n"
    "    1)  Download all galaxy spectra w/in 0.1 deg of the HDF\n\n"
