/**
 *  VOATLAS -- Query the Aladin Image service for an all-sky image.
 *
 ...
 *  @date       7/03/12
 *
 *  @brief      Query the Aladin Image service for an all-sky image.
 */
...
static Task  self       = {  "voatlas",  voatlas };
static char  *opts      = "%hF:R:D:P:bgs:o:Srp:n:v";
...
Tests (char *input)
{
    ...
    vo_taskTest (self, "-o", "m51.fits", "m51", NULL);	
    vo_taskTest (self, "--ra=202.47", "--dec=47.195", "--samp", NULL);
    vo_taskTest (self, "--survey=list", "-v", "ngc1234", NULL);
    vo_taskTest (self, "-o", "gal.jpg", "-n", "256", 
	"--graphic", "sombrero", NULL);
    vo_taskTest (self, "-s", "20m", "--survey=wise22", "m101", NULL);
    vo_taskTest (self, "--band=radio", "3c273", NULL);

    if (access ("m51.fits", F_OK) == 0)
	unlink ("m51.fits");
    if (access ("gal.jpg", F_OK) == 0)
	unlink ("gal.jpg");
}
