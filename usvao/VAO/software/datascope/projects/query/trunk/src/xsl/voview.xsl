...
<xsl:variable name="alphanum" select="'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_'"  />
<xsl:variable name="allchars" select="'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_! @#$%^*()-+=[{]};:|&amp;&gt;&lt;&quot;'"  />

<!-- Computed variables -->

<xsl:variable name="fieldlist" select="//FIELD|//vo:FIELD|//v1:FIELD|//v2:FIELD|//v3:FIELD|//v4:FIELD"/>
<xsl:variable name="paramlist" select="//PARAM|//vo:PARAM|//v1:PARAM|//v2:PARAM|//v3:PARAM|//v4:FIELD"/>
...
<!-- WHEN IT DOESN'T, SAFARI IS UNHAPPY! -->
   ...
   <xsl:variable name="ucase" select="translate($value,$lc,$uc)" />
   <xsl:variable name='temp_column'>
      <xsl:for-each select="$fieldlist">
         <xsl:if test="translate(@ucd, $lc,$uc)= $ucase">
...
<xsl:template name="ORIGINALgetColumnByUCD">
	<xsl:param name="value"/>
	<xsl:for-each select="$fieldlist">
...
<xsl:template name="getColumnByUCDs">
	...
	<xsl:variable name="ucase" select="translate($value, $lc,$uc)" />
	<xsl:for-each select="$fieldlist">
	        <xsl:variable name="ucucd" select="translate(@ucd,$lc,$uc)" />
		<xsl:if test="contains($ucase, concat('|',$ucucd,'|')) and
...
<xsl:variable name="nrows" select="count(//TABLEDATA/TR|//vo:TABLEDATA/vo:TR|//v1:TABLEDATA/v1:TR|//v2:TABLEDATA/v2:TR|//v3:TABLEDATA/v3:TR|//v4:TABLEDATA/v4:TR)"/>
...
<!--
				...
				<span class="bbox" onclick="{$clearSelectionCallback}();">Reset TEST selection</span>
				<br />
-->
			...
			<table class="data">
				...
				<tbody>
					...
					<xsl:otherwise>
					<xsl:for-each select="DATA/TABLEDATA|vo:DATA/vo:TABLEDATA|v1:DATA/v1:TABLEDATA|v2:DATA/v2:TABLEDATA|v3:DATA/v3:TABLEDATA|v4:DATA/v4:TABLEDATA">
						<xsl:for-each select="TR|vo:TR|v1:TR|v2:TR|v3:TR|v4:TR">
							<xsl:sort select="(TD|vo:TD|v1:TD|v2:TD|v3:TD|v4:TD)[position()=$sortColumnNum]/@val" order="{$sortOrder}" data-type="{$datatype}"/>
							<xsl:sort select="(TD|vo:TD|v1:TD|v2:TD|v3:TD|v4:TD)[position()=$sortColumnNum]" order="{$sortOrder}" data-type="{$datatype}"/>
							<xsl:if test="not (position() &lt; $pageStart or position() &gt; $pageEnd)">
								<xsl:variable name="TDlist" select="TD|vo:TD|v1:TD|v2:TD|v3:TD|v4:TD" />
		...
		<xsl:for-each select="$paramlist">
			<tr>
               ...
               <xsl:choose>
                  ...
                  <xsl:otherwise>UNKNOWN PARAM</xsl:otherwise>
               </xsl:choose>
            </td>
...
<xsl:template name="fieldrow">
	...
	<tr id="fieldrow_{$posit}">
		...
		<td>
			<xsl:variable name="fieldname" select="$fieldlist[position()=$posit]/@name" />
			<xsl:choose>
				<xsl:when test="(translate(@ucd,$lc,$uc)='POS_EQ_RA_MAIN' or translate(@ucd,$lc,$uc)='POS.EQ.RA;META.MAIN') and (@datatype='double' or @datatype='float')">
					<select id="{$fieldname}_select" class="vo_converter" func="position" fn="{translate($fieldname,$allchars,$alphanum)}" scale="15" name="unit">
						<option value="degree" selected="selected">degree</option>
						<option value="hhmmss">hhmmss</option>
					</select>
				</xsl:when>
				<xsl:when test="(translate(@ucd,$lc,$uc)='POS_EQ_DEC_MAIN' or translate(@ucd,$lc,$uc)='POS.EQ.DEC;META.MAIN')  and (@datatype='double' or @datatype='float')">
					<select id="{$fieldname}_select" class="vo_converter" func="position" fn="{translate($fieldname,$allchars,$alphanum)}" scale="1" name="unit">
						<option value="degree" selected="selected">degree</option>
						<option value="hhmmss">ddmmss</option>
					</select>
				</xsl:when>
				<xsl:when test="translate(@unit,$lc,$uc)='MJD' and (@datatype='double' or @datatype='float')">
					<select id="{$fieldname}_select" class="vo_converter" func="time" fn="{translate($fieldname,$allchars,$alphanum)}" name="unit">
						<option value="mjd" selected="selected">MJD</option>
...
-->
   <xsl:if test="$row=$maxColumns or ( $row=$ncols and $maxColumns &gt; $ncols )">
		<tr class="separator">
			<td colspan="5" align="center">Columns below are hidden - Drag any to change order</td>
		</tr>
	</xsl:if>
</xsl:template>
	...
	<td>
		<xsl:if test="$posit != $urlColumnNum and (@datatype='char' or not(@arraysize)  or @arraysize=1)">
			<xsl:variable name='scale'>
				<xsl:choose>
					<xsl:when test="translate(@ucd,$lc,$uc)='POS_EQ_RA_MAIN'  or translate(@ucd,$lc,$uc)='pos.eq.ra'">15</xsl:when>
					<xsl:when test="translate(@ucd,$lc,$uc)='POS_EQ_DEC_MAIN' or translate(@ucd,$lc,$uc)='pos.eq.dec'">1</xsl:when>
					<xsl:otherwise></xsl:otherwise>
				</xsl:choose>
			</xsl:variable>
			<xsl:variable name="isChar" select="@datatype='char' or @datatype='string'"/>
			<input type="hidden" name="vovfilter{$posit}_scale" value="{$scale}" />
			<input type="hidden" name="vovfilter{$posit}_type" value="{$isChar}" />
			<input type="text" name="vovfilter{$posit}">
				<xsl:attribute name="title">
					<xsl:choose>
						<xsl:when test="$isChar" >
							Character column: Exact match or * wildcards
						</xsl:when>
						<xsl:otherwise>
...
<xsl:template name="processColumn">
	...
	<xsl:choose>
		...
		<xsl:otherwise>
			<td class="{translate($fieldlist[position()=$posit]/@name,$allchars,$alphanum)} {$fieldlist[position()=$posit]/@unit}" >
				...
				<xsl:choose>
					...
					<xsl:when test="contains('|float|int|double|',concat('|',$fieldlist[position()=$posit]/@datatype,'|'))">
						<xsl:value-of select="." />
					</xsl:when>
					<xsl:otherwise>
