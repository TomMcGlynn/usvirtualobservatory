...
function XSLTFilter(baseDocument, xslProc) {
    ...
    this.filterByColumn = function(form) {
        ...
        if (ff.userIDs.length == 0) {
            ...
            xslProc.setParameter(null, "filterScales", ff.userScales.join(","));
            if (ff.filterText == this.constraints.filterText) {
                // no change in filter settings
                return false;
            }

//	alert ("userIDs: " + ff.userIDs);			// column number
//	alert ("userTypes: " + ff.userTypes);		//	true for text, false for numbers ?
//	alert ("userConstraints: " + ff.userConstraints);	// actual filter text
//	alert ("filterText: " + ff.filterText);	//	combo of userID and userConstraints
//	alert ("filterTypes: " + ff.filterTypes);	//	undefined all the time?
for ( var i=0; i<ff.userTypes.length; i++ ) {
	if ( ff.userTypes[i] ) {
//		var invalid_chars = "<|>|=";
//		if ( ff.userConstraints[i].match(invalid_chars) ) {
//			if ( ! confirm ( "Search string contains "+invalid_chars+" in text column.  Continue?" ) ) {
		if ( ff.userConstraints[i].match("<|>") ) {
            ...
            if (this.baseDocument) {
                var xsltString;
                try {
                    xsltString = xslt(ff.userIDs, ff.userConstraints, ff.userTypes, ff.userScales);
//alert("xsltString is:\n"+xsltString);
...
//alert ( xsltString );
                    ...
                    //XXX var xsltDom = (new DOMParser()).parseFromString(xsltString, "text/xml");
            ...
            for (var j=0; j<form.elements.length; j++) {
                ...
                if (el.tagName == "INPUT" && el.type == "text") {
                    ...
                    if (constraint) {
                        ...
                        userTypes.push(v);
                        var v = form.elements[el.name+"_scale"].value;
                        userScales.push(v);
                    }
                }
            }
