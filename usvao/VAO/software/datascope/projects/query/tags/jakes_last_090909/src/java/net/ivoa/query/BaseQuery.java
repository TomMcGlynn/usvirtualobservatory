...
public class BaseQuery {
   public static void main(String[] args) {
      CGI params = new CGI();
      if (Querier.validQuery(params) && ( params.value("autosubmit") == null || !params.value("autosubmit").equals("false") ) ) {
         Querier.run(params);
      } else {
         run(params);
      }
   ...
   static void run(CGI cgi, String errorMessage) {
	  ...
	  h.setIcon("images/tools50.png", "Tools icon");
      } else {
       
          h.setBannerTitle("NVO Data Discovery: Simple Data Query");
          h.setTitle("Simple Query");
	  h.setIcon("images/simplequery50.png", "Simple Query icon");
      }
       
      // Non-cgi need to be full paths as may be accessed through /cgi-bin/
      h.addCSSFile(Settings.get("docbase","")+Settings.get("CSS_PATH","css/")+"style.css");
      h.addJavaScript(Settings.get("docbase","")+Settings.get("JS_PATH","js/")+"jquery.pack.js");
      h.addJavaScript(Settings.get("docbase","")+Settings.get("JS_PATH","js/")+"jquery.listen.js");
      h.addJavaScript(Settings.get("docbase","")+Settings.get("JS_PATH","js/")+"jquery.simplemodal.js");
      h.addJavaScript(Settings.get("docbase","")+Settings.get("JS_PATH","js/")+"validate.js");
      h.addJavaScript(Settings.get("docbase","")+Settings.get("JS_PATH","js/")+"basequery.js");

      if (view) {
          h.addToken(Settings.get("CGIbase",""), "Simple&nbsp;Query", "Query resource");
      } else {
	  h.addToken(Settings.get("CGIbase","")+"?view=1", "Table&nbsp;Viewer", "View and filter a VOTable");
      }
      ...
      h.addToken(Settings.get("VOClient",""), "Scripting", "How to run retrieval scripts on your machine");
       
      // Add this token fully formatted since we're going
      // to set it as a target.
      h.addToken("<a href='"+Settings.get("docbase","")+"helpInc.html' target=helpWin title='Help for the current form'>Help</a>");
   ...
   static void printFormFile(CGI cgi, String errorMessage) {
	  ...
	  template = "BaseQueryFormBegin.html";
      } else {
	  template = "ViewTableBegin.html";
      }
      if ( errorMessage.length() > 0 ) Settings.put("errorMessage", errorMessage);
      try {
	
         SettingsFilter.filter(template, true);// true deletes rows with matching filters == null
	  
      } catch (Exception e){}
      for (String key: cgi.keys()) {   // add hidden fields for all those that we don't use
      ...
      try {
         SettingsFilter.filter("BaseQueryFormEnd.html", true);// true deletes rows with matching filters == null
      } catch (Exception e){}
   }
