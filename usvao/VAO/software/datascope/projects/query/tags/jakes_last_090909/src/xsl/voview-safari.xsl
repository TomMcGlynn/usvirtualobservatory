				...
				<tbody>
					...
					<xsl:for-each select="DATA/TABLEDATA|vo:DATA/vo:TABLEDATA|v1:DATA/v1:TABLEDATA|v2:DATA/v2:TABLEDATA|v3:DATA/v3:TABLEDATA">
						<xsl:for-each select="TR|vo:TR|v1:TR|v2:TR|v3:TR">
							<xsl:sort select="(TD|vo:TD|v1:TD|v2:TD|v3:TD)[position()=$sortColumnNum]/@val" order="{$sortOrder}" data-type="{$datatype}"/>
							...
							<xsl:if test="not (position() &lt; $pageStart or position() &gt; $pageEnd)">
								<xsl:variable name="TDlist" select="TD|vo:TD|v1:TD|v2:TD|v3:TD" />
...
<xsl:template name="fieldrow">
	...
	<tr id="fieldrow_{$posit}">
		...
		<td>
			...
			<xsl:choose>
				<xsl:when test="@ucd='POS_EQ_RA_MAIN' or contains(@ucd,'pos.eq.ra')">
					<select id="{$fieldname}_select" class="vo_converter" func="position" fn="{$fieldname}" scale="15" name="unit">
						<option value="degree" selected="selected">degree</option>
						<option value="hhmmss">hhmmss</option>
					</select>
				</xsl:when>
				<xsl:when test="@ucd='POS_EQ_DEC_MAIN' or contains(@ucd,'pos.eq.dec')">
					<select id="{$fieldname}_select" class="vo_converter" func="position" fn="{$fieldname}" scale="1" name="unit">
						<option value="degree" selected="selected">degree</option>
						<option value="hhmmss">ddmmss</option>
					</select>
				</xsl:when>
				<xsl:when test="@unit='mjd'">
					<select id="{$fieldname}_select" class="vo_converter" func="time" fn="{$fieldname}" name="unit">
	...
	<td>
		<xsl:if test="$posit != $urlColumnNum and (@datatype='char' or not(@arraysize)  or @arraysize=1)">
			<xsl:variable name='scale'>
				<xsl:choose>
					<xsl:when test="@ucd='POS_EQ_RA_MAIN' or contains(@ucd,'pos.eq.ra')">15</xsl:when>
					<xsl:when test="@ucd='POS_EQ_DEC_MAIN' or contains(@ucd,'pos.eq.dec')">1</xsl:when>
...
<xsl:template name="processColumn">
	...
	<xsl:choose>
		...
		<xsl:otherwise>
			<td class="{$fieldlist[position()=$posit]/@name} {$fieldlist[position()=$posit]/@unit}" >
