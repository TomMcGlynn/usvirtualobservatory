...
public class Oasis {
	...
	private void checkField(Attributes attrib) {
	    ...
	    if (ucd != null &&
	        (ucd.equals("POS_EQ_RA_MAIN")  || ucd.equals("pos.eq.ra;meta.main"))) {
		raCol = ncol;
	    } else if (ucd != null &&
	        (ucd.equals("POS_EQ_DEC_MAIN")  || ucd.equals("pos.eq.dec;meta.main"))) {
		decCol = ncol;
	    }
	    ncol += 1;
	}
	...
	if (max > 10) {
	    System.out.println("<hr><b>Note:</b> More than 10 items were selected but only 10 can "+
			       " be sent to OASIS<hr>");
	    max = 10;
	}
	
	System.out.print(
    "<h2> OASIS Launcher </h2>"+
	...
	for (int i=0; i<max; i += 1) {
	    if (urls[i] != null) {
                System.out.println("<PARAM NAME = 'dataurl"+ocount+" VALUE='"+urls[i]+"'>");
		ocount += 1;
	    }
	}
	...
	if (flds.length == 2) {
	    ...
	    String file = DS.baseToHome(cache)+DS.validFileName(flds[0])+"."+flds[1]+"."+inp[0]+"."+inp[1]+".fits";
