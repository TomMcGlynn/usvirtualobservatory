...
public class VOTFormatter {
	...
	private void printPrefix() {
	    System.out.println("<SCRIPT language='JavaScript'>\n"+
			       "Tncol"+tab+"="+ncol+"\n"+
			       "Tucd"+tab+"='"+join(ucds, ",", ncol)+"'\n"+
			       "Tname"+tab+"='"+join(names, ",", ncol)+"'\n"+
			       "Ttype"+tab+"='"+join(types, ",", ncol)+"'\n"+
			       "</SCRIPT>\n"+
			       "<table border><tr bgcolor='#FFFFCC' id=tr"+tab+"-0>"+
			       "<td id=t"+tab+"-0-0-0></td>\n");
	    for (int i=0; i<ncol; i += 1) {
		System.out.println( "<th id=th"+tab+"-"+(i+1)+">"+names.get(i)+"</th>");
	    }
	...
	private void printRowStart() {
	    ...
	    if (row > 1 && (row-1)%25 == 0) {
		System.out.print("</TABLE><TABLE border><tr bgcolor='#FFFFCC'><td id=t"+tab+"-"+row/25+"-0-0></td>");
	        for (int i=0; i<ncol; i += 1) {
	    ...
	    System.out.print("<tr bgcolor="+color+"><td id=t"+tab+"-"+row+"-0></td>");
	}
