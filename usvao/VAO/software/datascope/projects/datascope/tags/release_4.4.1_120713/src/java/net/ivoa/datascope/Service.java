...
public abstract class Service {
	...
	private double interfaceVersion = 0;
	...
	public void initialize(String ivoID, String url, String cache, String sn,
			int index, double ror,  String version, String interfaceVersion) {
		this.ivoID = ivoID;
		this.index = index;
		this.ror = ror;
		this.sn = sn;
		...
		try {
			if(interfaceVersion != null){
				this.interfaceVersion = Double.parseDouble(interfaceVersion);
			}
		}catch (java.lang.NumberFormatException e){};
	...
	public boolean equals(Object o){
		Service sv = (Service)o;
		
		if( !sn.equals(sv.getShortName()) ){
			return false;
		}
		
		String ivoid_root1 = ivoID.split("#")[0];
		String ivoid_root2 = sv.getID().split("#")[0];
		if( !ivoid_root1.equals(ivoid_root2) ){
			return false;
		}
	...
	public static String get(String urlString) throws Exception {
		...
		uc.setConnectTimeout(DS.getTimeout());
	...
	static Service factory(String type) {
		...
		if (lowType.startsWith("cone")) {
			...
			return new SIA();
		} else if (lowType.startsWith("simplespectralaccess")) {
			return new SSA();
		} else {
			return null;
		}
	...
	private void save(String result) throws Exception {
		OutputStreamWriter fw = 
			new OutputStreamWriter(new FileOutputStream(getResultFilename()), "UTF-8");
		fw.write(result);
		fw.close();
	}
	
	String getResultFilename(){
		return cache + DS.validFileName(sn) + "." + index + ".xml";
	}
