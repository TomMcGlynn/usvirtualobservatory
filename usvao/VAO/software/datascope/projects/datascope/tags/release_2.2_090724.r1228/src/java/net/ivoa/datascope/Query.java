...
public class Query {
	...
	hdr.addToken(DS.getURLBase()+"helpInc.html", "Help", "Help for DataScope");
	
	hdr.setIcon(DS.getURLBase()+"images/datascope50.png", "DataScope Icon");
	
	hdr.setTitle("DataScope Query");
	
	hdr.setBannerTitle("NVO Data Discovery: DataScope");
	...
	System.out.println("<script language=JavaScript>\n"+
			   ...
			   "       url += '&skipcache=on'\n"+
			   "   }\n"+
			   "   if (document.getElementById('skipregcache').checked) {\n"+
			   "       url += '&skipregcache=on'\n"+
			   "   }\n"+
	...
	System.out.println(
	  ...
	  "<form name=dsform id=dsform action="+DS.getCGIBase()+"jds.pl method=GET onsubmit='return check()'>"+
          "<table>\n"+
          "<tr><th class=right colspan=2><label for=position>Position:</label></th><td colspan=2><input size=40 id=position name=position value='"+value+"'></td></tr>\n"+
	  ...
	  "<tr><td colspan=4><label for=skipcache> <b>Skip cache?</b><input id=skipcache  name=skipcache type=checkbox>&nbsp;&nbsp;"+
          "<label for=skipregcache><b>Refresh registry?</b></label><input  id=skipregcache name=skipsave type=checkbox></td></tr>"+
