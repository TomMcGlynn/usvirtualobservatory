...
var CGIBase     = "/cgi-bin/@SUBDIR@/datascope/";
...
function showMeta(index) {
	var str = "metadisp.pl?cache="+encCacheDir+"&id="+encodeURIComponent(metadata[index][idLoc]);
...
function setCount() {
    ...
    if (linesLength < lastLength){
        setTimeout("setCount()", 2000);
        return          
    }
	lastLength = linesLength;
...
function setHelpText() {
	var url   = "/@SUBDIR@/datascope/helpInc.html";
	var req   = getRequest();
	req.open("GET", url, false);
	req.send(null);
	if (req.status == 404) {
...
function getSelections() {
	...
	text += "<p><em>A know bug will sometime cause this page to not<br> "+
	        "properly reflect selected values.  If this happens you<br> "+
	        "should reload the page and try again.</em>";
...
function getMetadata() {
	...
	for (var i=0; i<lines.length; i += 1) {
		if (lines[i].length > 0) {
			...
			//shortName = metadata[i][snLoc];
			//shortName = shortName.replace(/\].*/, '');
			//metadata[i][snLoc] = shortName.replace(/\s*\[/, '/');
		}
	}
...
function renderNode(res_id) {
	...
	"<a title='VO Plotting applet' target=extern href=/cgi-bin/@SUBDIR@/voplot/loadjvt2.pl?suffix="+url+" onClick='return nw(this.href, this.target)'>VOPlot</a> | " +
	...
	if (meta[stypeLoc] == "ConeSearch"  &&  meta[typeLoc] == "Archive"  && !meta[pubLoc].match(/Spitzer/)) {
		if (meta[pubLoc].match(/HEASARC/)) {
			...
			var hashIndex = currentTable.indexOf('#');
			if( hashIndex != -1){
			    currentTable = currentTable.substring(0,hashIndex);
			}
			special = true;
	...
	for (findex in selFiles){
		if( findex.indexOf(res_id+"-") == 0	&& selFiles[findex] != null ){
			urlColumnNum = selFiles[findex];
			selectedResColumns.push( findex.substring(findex.indexOf('-')+1) );
		}
	}	
	vovTable.setSelectedRows(selectedResColumns);
