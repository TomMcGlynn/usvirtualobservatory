...
sub new {
	...
	$self->{CacheUrlRoot} = 'http://@HOST@@DSCache@';
	...
	$self->{resultFiles} = ["results.xml", "hits.xml", "status.xml"];
	
	$self->{XsltFilter} = '@RUNTIME@@XSL_PATH@/filterResources.xsl';
	$self->{SummaryFilter} = '@RUNTIME@@XSL_PATH@/makeSummary.xsl';
...
#
# Debug options		
#	$debugOpts = "-Xdebug -Xrunjdwp:transport=dt_socket,address=1044,server=y,suspend=y";
	
	$self->{javaCommand} = "/usr1/local/java/bin/java -Xmx500M $debugOpts net.ivoa.datascope.Response";
...
sub writeJobParameters {
	...
	foreach my $param (keys %$params){
		unless($param =~ /services/){
			print PARAMFILE "<uws:parameter id=\"" . $param . "\" >" . $params->{$param} . "</uws:parameter>\n";
		}else{
			my $servicesFile = "/job_queue/" . $self->{jobID} . "/services.txt";
			print PARAMFILE "<uws:parameter id=\"services\" byReference=\"true\" >" . $self->{CacheUrlRoot} . $servicesFile . "</uws:parameter>\n";
			unless( open(SERVICES, $self->{CacheDirRoot} . $servicesFile) ){
				$self->fatalJobError("Error writing services file " . $self->{CacheDirRoot} . $servicesFile);
			}
			print SERVICES $params->{"services"};
			close SERVICES;
		}
	}
...
sub getJobResults {
	...
	unless( $self->{jobParameters} ){
		$self->getJobParamters();
	}
	
	my $parametersElem = XMLin( join(' ', @{$self->{jobParameters}}) );

	print STDERR "Datascope JobManager: Job parameters: ", join(' ', @{$self->{jobParameters}}), "\n";
#	print STDERR "Datascope JobManager: parameter: struct:", Dumper($parametersElem), "\n";
	print STDERR "Datascope JobManager: services parameter: ", $parametersElem->{'uws:parameter'}->{SERVICES}->{content}, "\n";

	if( $parametersElem->{'uws:parameter'}->{SERVICES}->{content} ){
		$self->{services} = $parametersElem->{'uws:parameter'}->{SERVICES}->{content};
	}
	...
	my $jobdir = "/job_queue/" . $self->{jobID};
	...
	foreach my $resultsFile (@$resultFiles) {
		...
		if(@versions){
			...
			if( $suffix eq 'txt' ){
				...
				#
				# Copy the file into the local directory
				#
				unless( copy($self->{CacheDirRoot} . $cachedir . "/" . $fileName, $self->{CacheDirRoot} . $jobdir) ){
					$self->fatalJobError("Error copying file " . $self->{CacheDirRoot} . $cachedir . "/" . $fileName . " to " .
										$self->{CacheDirRoot} . $jobdir);     	
				}
				#
				# Clean up old versions to save disk space
				#
				my @localVersions = glob $self->{CacheDirRoot} . $jobdir . "/" . $prefix . "*." . $suffix;
				if( @localVersions > 3 ){
					@localVersions = sort { ($b =~ /_v(\d+)\./)[0] <=> ($a =~ /_v(\d+)\./)[0] } @localVersions;
					unlink @localVersions[3,];
				}
				
				if( $self->{services} ){
					#
					# Filter the results file to only contain the specified services
					#
					my $fullLocalFileName = $self->{CacheDirRoot} . $jobdir . "/" . $fileName;
					
					unless($self->{filterStylesheet}){
						$self->{filterStylesheet} = $self->{xslt}->parse_stylesheet_file($self->{XsltFilter});
					}
					my $result = $self->{filterStylesheet}->transform_file($fullLocalFileName, XML::LibXSLT::xpath_to_string(resources => $self->{services}));

					$self->{filterStylesheet}->output_file($result, $fullLocalFileName . "_tmp");
					unlink $fullLocalFileName;
					rename $fullLocalFileName . "_tmp", $fullLocalFileName;
				}
								
				push @results, "<uws:result id=\"" . $resultsFile . "\" xlink:href=\"" .  $self->{base} . $jobdir . "/" . $fileName . "\" />";
				
				if( $resultsFile=~/status.*xml/ ){
					$statusFile = $self->{CacheDirRoot} . $jobdir . "/" . $fileName;
				}
			}
		}
	}
	...
	if($statusFile){
		...
		#
		unless($self->{summaryStylesheet}){
			$self->{summaryStylesheet} = $self->{xslt}->parse_stylesheet_file($self->{SummaryFilter});
		}
		my $result = $self->{summaryStylesheet}->transform_file($statusFile);
		my @summary = split(/\n/, $self->{summaryStylesheet}->output_as_bytes($result));
		# print STDERR "Datascope JobManager: result, summary: ", $result, join("\n", @summary), "\n";
		push @results, grep(/<uws:result/, @summary);
	}
