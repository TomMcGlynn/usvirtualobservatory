...
public final class DS {
	...
	private static int failedServicesLimit = 10; // Number of services to fail before an entire queue is abandoned

	private static int maxPriorityServices = 100; // Max number of services to run BEFORE running a queue filter.

	private static int maxSiteFailures = 5; // Max number of failed services from a site before the site is considered broken.
	...
	public static int getFailedServicesLimit() {
		if( Settings.has("failedServicesLimit") ){
			return parseInt(Settings.get("failedServicesLimit"), failedServicesLimit);
		}else{
			return failedServicesLimit;
		}
	}
	
	public static int getMaxPriorityServices() {
		if( Settings.has("maxPriorityServices") ){
			return parseInt(Settings.get("maxPriorityServices"), maxPriorityServices);
		}else{
			return maxPriorityServices;
		}
	}
	
	public static int getMaxSiteFailures() {
		if( Settings.has("maxSiteFailures") ){
			return parseInt(Settings.get("maxSiteFailures"), maxSiteFailures );
		}else{
			return maxSiteFailures;
		}
	}
	...
	private static String set(String defaultValue, String setting) {
