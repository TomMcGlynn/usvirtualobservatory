...
public class FOV {
	...
	public static void main(String[] args) throws Exception {
		...
		String filename = DS.baseToHome(cache) + DS.validFileName(sn) + "."
				+ id + "." + index + ".fits";
		...
		if (f != null) {
			...
			String url = new FieldExtractor().find(DS.baseToHome(cache) + sn
					+ "." + id + ".xml", r, c);
			process(dssFile, url, filename);
		}
	}

	static void process(String dss, String inFile, String outFile)
			throws Exception {
		...
		double[][] ipixels = { { 0, imAxes[0], imAxes[0], 0 },
				{ 0, 0, imAxes[1], imAxes[1] } };
		...
		String[] newargs = new String[] { "survey=user", "userfile=" + dss,
		...
		ip.getProcessor().drawLine(scl(0), 300 - scl(0), scl(dsAxes[0]),
				300 - scl(0));
		ip.getProcessor().drawLine(scl(dsAxes[0]), 300 - scl(0),
				scl(dsAxes[0]), 300 - scl(dsAxes[1]));
		ip.getProcessor().drawLine(scl(dsAxes[0]), 300 - scl(dsAxes[1]),
				scl(0), 300 - scl(dsAxes[1]));
		ip.getProcessor().drawLine(scl(0), 300 - scl(dsAxes[1]), scl(0),
				300 - scl(0));

		// Now draw the borders of the image.
		for (int i = 0; i < 4; i += 1) {
			ip.getProcessor().drawLine(scl(opixels[0][i]),
					300 - scl(opixels[1][i]), scl(opixels[0][(i + 1) % 4]),
					300 - scl(opixels[1][(i + 1) % 4]));
		}
		...
		if (errorcirc != null) {
			...
			for (double a = 0; a < 360; a += 5) {
				...
				ip.getProcessor().drawLine(scl(x0), 300 - scl(y0), scl(x1),
						300 - scl(y1));
			}
		}
