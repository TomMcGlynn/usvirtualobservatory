...
public class SettingsMatcher {
   ...
   *  left unchanged if skip is false, or that line is set to null
   *  if skip is true.
   */
   public static String replaceSettings(String input, boolean skip) {
      ...
      while (match.find()) {
         if (skip && !Settings.has(match.group().substring(1)) ) {
            return "";
         }
