 ...
 */
vocMsg_t *
msg_newMsg (char *class, char *str)
{
    vocMsg_t *msg = (vocMsg_t *) calloc (1, sizeof (vocMsg_t));

    msg->type	 = MSG_MESSAGE;
    strncpy (msg->class, class, strlen (class));
...
msg_getResult (int fd)
{
    ...
    bzero (buf, SZ_MSGBUF);		/* clear buffers		*/
    bzero (rembuf, SZ_MSGBUF);
	...
	if (nbytes > 0) {
	    ...
	    int len;
	    res->buf = msg_readBulk (fd, &len, &stat);
	    res->buflen = len;
	}
    }
...
/* MSG_ADDPARAM -- Add a parameter to a Query.
 */
static void 
msg_addParam (vocMsg_t *msg, int type, char *str)
{
     ...
     */
    bzero (head, SZ_MSGBUF);
    bzero (tail, SZ_MSGBUF);
