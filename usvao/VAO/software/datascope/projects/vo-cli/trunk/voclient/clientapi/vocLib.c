...
voc_initVOClient (char *opts)
{
	...
	*/
	if (!vopt->spawn) {
	    if ((vo->io_chan = voc_openVOCServer(VOCProxy)) == (int)NULL) {
                if (!vopt->quiet)
	            fprintf (stderr,
	...
	} else if ((vo->io_chan = voc_openVOCServer(VOCServer)) == (int)NULL) {
	     ...
	     * attempts.
	     */
	    for (i=SVR_MAXTRY; i && vo->io_chan == (int) NULL; i--) {
		sleep (1);
...
** 
** 		server=8081:localhost,spawn=yes,console=yes
** 
...
voc_parseOpts (char *opts)
{
    ...
    bzero (lserver, SZ_OPT_BUF);	strcpy (lserver, DEF_SERVER);
    bzero (lproxy, SZ_OPT_BUF);		strcpy (lproxy, DEF_NET_SERVER);
    bzero (lconfig, SZ_OPT_BUF);

    /* Loop through the option string, setting values.
    */
    ip = opts;
    while (*ip) {
	bzero (keyw, SZ_OPT_BUF);
	bzero (val, SZ_OPT_BUF);
	...
	*/
	...
	else if (strncmp ("voc_path", keyw, 2) == 0)
	    strcpy (lpath, val);
            if (val[len-1] != '/')
                strcat (lpath, "/");
