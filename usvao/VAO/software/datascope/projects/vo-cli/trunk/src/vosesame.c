/************************************************************************
**  VOSESAME -- Call the Sesame name resolver service with the specified
**  target.
**
...
int     flags[MAX_FLAGS];		    /* argv flags array		    */
int	nflags		= 0; 		    /* number of flags		    */
int	ntargets	= 0; 		    /* number of input targets	    */
int	all_flags	= 0; 		    /* print all fields?	    */
int     do_votable  	= 0;                /* output a VOTable    	    */

int	format		= TRUE;		    /* format the output?	    */
int	header		= FALSE;	    /* print a header?		    */
int	force		= FALSE;	    /* force name resolution	    */
int	count		= FALSE;	    /* count of objects		    */
int	invert		= FALSE;	    /* invert to print non-matches  */
int	convert		= FALSE;	    /* invert to print non-matches  */
int	debug		= FALSE;	    /* std debug flag		    */
int	verbose		= FALSE;	    /* std verbose flag		    */
int	quiet		= FALSE;	    /* suppress output flag	    */
int	res_all		= FALSE;	    /* print all results?	    */
int	meta		= FALSE;	    /* print all results?	    */
int	user_pos	= TRUE;		    /* using user position?	    */
int	longlines	= FALSE;	    /* output long lines?	    */
int	status		= OK;		    /* return status		    */
int     inventory       = 0;                /* inventory query (NOT USED)   */
int     table_hskip     = 0;                /* header skip (NOT USED)       */
int     ecols           = 0;                /* exact columns (NOT USED)     */

char	delim		= ' ';		    /* output table delimiter	    */
...
char   *tmpdir     	= "/tmp/";          /* temp working directory       */
char   *u_ra		= (char *) NULL,    /* user-defined position	    */
       *u_dec		= (char *) NULL,
       *sep		= (char *) NULL;
char   *next_arg	= (char *) NULL;
...
main (int argc, char *argv[])
{
     ...
     */
    nflags = 0;
    bzero (target, SZ_TARGET);
    bzero (flags, MAX_FLAGS);
    out		= stdout;
...
print_header ()
{
    ...
    if (all_flags) {
        ...
        fprintf (out, "#");
	for (i=0; i < nflags; i++) {
	    sp = (((nflags-i) > 1) || nflags >= 1 ? delim : (char)NULL);
	    switch (flags[i]) {
