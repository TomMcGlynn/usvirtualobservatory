...
int     dalOnly     = 1;                /* resolve only DAL services	*/
...
main (int argc, char *argv[])
{
	...
	*/
        ...
        */
        nservices = vot_countServiceList();
...
vot_setArgWord (char *arg)
{
	...
	char *ip = vot_requiredArg (arg);
	    ...
	    bzero (v1, SZ_FNAME);	   /* clear arrays    	*/
	    bzero (v2, SZ_FNAME);
	    if (op) {
		*op = '\0';
...
vot_printSvcList (Service *sl)
{
    ...
    for (s=sl; s; s=s->next) {
	fprintf (stderr, "%15s:  nfail=%d  nrefs=%d  type=%d\ts=%d\n",
	    s->name, s->nfailed, s->nrefs, s->type, (int)s);

        for (p=s->proc; p; p=p->next) {
	    fprintf (stderr, "\t\tpid=%d  status=%d  obj=%s\t    p=%d\n",
		p->pid, p->status, p->obj->name, (int)p);
        }
    }
}
    ...
    */
	...
	if (nrunning < nprocs && nobj <= nobjects) {
		...
		*/
	    ...
	    if ((r_pid = waitpid ((pid_t)-1, &status, (int)NULL)) < 0 ) {
