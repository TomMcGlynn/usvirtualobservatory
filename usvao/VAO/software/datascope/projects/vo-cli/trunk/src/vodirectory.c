...
int     count	    = 0;		/* return count of results	*/

int	quiet	    = 0;		/* no output at all?		*/
int	verbose	    = 0;		/* verbose output?		*/
int	exact	    = 0;		/* exact match only?		*/
int	debug	    = 0;		/* debug output?		*/
int     proxy       = 0;		/* use proxy server		*/
int     group       = 0;		/* group terms			*/
int     nterms      = 0;		/* number of search terms	*/
int     res_index   = -1;		/* index of result      	*/
int     res_all     = 0;		/* print all results      	*/
int     meta        = 0;		/* print meta results      	*/
int     nresults    = 0;		/* no. of results      		*/
int     user_fields = 0;		/* uer-defined fields?		*/
int     orValues    = 0;		/* OR search terms?		*/
int     longlines   = 0;		/* don't format output		*/
int     do_votable  = 0;		/* output a Resource VOTable	*/
int     status      = OK;		/* return status		*/
#ifdef REG10_KLUDGE
int     reg10       = 0;		/* Registry 1.0 support?        */
#endif
int     dalOnly     = 0;		/* DAL services only            */
...
char   *delim      = "\t,| ";          	/* delimiter (NOT USED)		*/

char   *fields      = (char *) NULL;	/* output fields		*/
char   *output      = (char *) NULL;	/* output filename		*/
...
static double vot_calJD (int mn, double dy, int yr, int hrs, int min, int sec);
...
main (int argc, char *argv[])
{
    ...
    */
	...
	if (!count) {
		...
		subject, orValues, do_votable, dalOnly, sortRes, terse);
	} else {
	    if (orValues) {
	        for (i=0; i < nterms; i++) {
	            nresults = vot_regSearch (&terms[i], 1, stype, bandpass,
		        subject, 1, do_votable, dalOnly, sortRes, terse);
		}
	    } else {
	        nresults = vot_regSearch (terms, nterms, stype, bandpass,
		    subject, 0, do_votable, dalOnly, sortRes, terse);
	    }
	}
	break;
...
printResolveRecord ()
{
    ...
    */
    ...
    for (i=0; i < nterms; i++) {
		...
		fields, res_index, 0, dalOnly, &result);
	} else {
#endif
	    nresults = vot_regResolver (terms[i], stype, bandpass, subject, 
		fields, res_index, exact, dalOnly, &result);
#ifdef REG10_KLUDGE
	}
#endif
