...
extern int   all_data, use_name, all_named, url_proc, dalOnly;
extern int   force_read, table_hskip, table_nlines, table_sample;
#ifdef REG10_KLUDGE
extern int   reg10;
#endif
extern char *typestr, *bpass, *delim, *cols, *ecols;

extern Service *svcList, *svcTail;

static int svcIndex	= 0;
...
static int vot_printServiceList (void);
...
extern int   vot_openVOTable (char *s);
extern int   vot_getNRows (int vot);
extern int   vot_getNCols (int vot);
	
extern int   vot_findByName (int vot, char *name, char *alt);
extern void  vot_closeVOTable (int vot);
extern char *vot_getFieldName (int vot, int n);
...
vot_loadResourceVOTable (char *fname)
{
	...
	c_name  = vot_findByName (vot, "shortName", "");
	c_url   = vot_findByName (vot, "serviceURL", "accessURL");
	c_id    = vot_findByName (vot, "identifier", "");
	c_type  = vot_findByName (vot, "type", "capabilityclass");
	c_title = vot_findByName (vot, "title", "");
...
isResourceVOTable (char *fname)
{
    ...
    */
	...
	for (i=0; i < nc; i++) {
	    bzero (name, SZ_FNAME);
	    strcpy (name, vot_getFieldName (vot, i));
...
vot_printServiceList ()
{
    ...
    do {
	fprintf (stderr, "%d: %-12.12s %s\n", i++, svc->name, svc->service_url);
    } while ((svc = svc->next));
