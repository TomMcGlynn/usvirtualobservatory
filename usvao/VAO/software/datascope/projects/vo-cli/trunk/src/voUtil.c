		 ...
		 char *fields, int index, int exact, int dalOnly, char **res)
{
	...
	fprintf (stderr, "regResolver init: id='%s' type='%s'\n", 
	        (id ? id : "null"), (svctype ? svctype : "null"));


    /*  Sanity checks.
    */
    if (!id && !svctype && !svctype[0]) {
...
retry:
	    ...
	    for (i=0; i < nterms; i++) {
            ...
            if (exact)
                sprintf (qstring,
	           "((Identifier like '%s') OR (ShortName like '%s'))", id, id);
            else
                sprintf (qstring,
...
vot_printAttrs (char *fname, Query query, char *ident)
{
    ...
    */
	...
	nrec  = vot_getNRows (vot);
	nattr = vot_getNCols (vot);
...
*/
	...
	for (i=0; i < nattr; i++) {
	    ...
	    bzero (ucd,  SZ_FNAME);  strcpy (ucd,  vot_getFieldUCD (vot, i));
	    bzero (name, SZ_FNAME);  strcpy (name, vot_getFieldName (vot, i));
	    bzero (id,   SZ_FNAME);  strcpy (id,   vot_getFieldID (vot, i));
	    bzero (desc, SZ_LINE);   strcpy (desc, vot_getFieldDesc (vot, i));
