...
extern int   all_data, use_name, all_named, url_proc;
...
static int   vot_printObjectList (void);
...
extern int   vot_findByUCD (int vot, char *name, char *alt);
...
vot_loadVOTable (char *fname)
{
    ...
    if ((vot = vot_openVOTable (fname)) ) {
        ...
        c_ra  = vot_findByUCD (vot, "POS_EQ_RA_MAIN", "pos.eq.ra");
        c_dec = vot_findByUCD (vot, "POS_EQ_DEC_MAIN", "pos.eq.dec");
        c_id  = vot_findByUCD (vot, "ID_MAIN", "meta.id");
	if (OBJ_DEBUG)
