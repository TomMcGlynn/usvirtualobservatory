...
function readdata(output, baseURL, postInput) {
    ...
    this.sort = function(sortColumn, sortOrder, newpage) {
	    ...
	    me.filter = new XSLTFilter(me.xml, me.myXslProc,me.xslBase);
    ...
    this.loadingError = function(errmsg) {
        ...
        if (this.id == "data") {
            var label = "VOTable";
	    if (errmsg.match(/No information/i)) {
	        addedInfo = "This may mean there is no data in the specified region.";
	    }
        } else {
            label = "XSL for "+this.view+" view";
        }
        msg = "<b>Failed to load "+label+":</b><br>\n"+errmsg+" "+addedInfo;
        me.errorMessage(msg);
        me.saveState();
    };
...
function updateFilter(id, type, scale) {
    
    if (rd.filter.update(id, type, scale)) {
        rd.filter.filter();
        rd.sortToggle = false;
//        rd.sort(rd.sortColumn, rd.sortOrder, 1);
        rd.page = 1;
        rd.sort();
    }
}
...
function invenLink(url) {
    var spec1 = document.getElementById('_spec1');
    spec1.name='action';
    spec1.value='upload';
    var spec2 = document.getElementById('_spec2');
    spec2.name='nvo';
    spec2.value='1';
...
function vimLink(url) {
    ...
    var str = new XMLSerializer().serializeToString(rd.filter.getDocument());
    document.getElementById("sources").value = str;
    document.getElementById("sources").name  = 'sourcelist';
    form.submit();
}
