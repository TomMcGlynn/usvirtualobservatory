...
function matchTreePrint(parseTree) {
    var text = "<h3> Matching Resources </h3><p>" +
               "These resources have data in the specified region.<br>"+
	       " <dl>"+
	       " <dd> Check box to select the table for download or analysis."+
	       " <dd> Click <i> name </i> to view the table data and select FITS data."+
	       " <dd> Click <i>?</i> to see the metadata for the resource. "+
	       " <dd> <table><tr><td></td><td bgcolor='#FFAAAA'>Shaded</td><td> when you select FITS data</td></tr></table></dl>"
...
function renderNode(res_id) {
    ...
    var dataHeader = "<p><h3> Data for "+meta[titleLoc]+" ("+sn+") </h3><p>" +
          "Quick Links: "+
...
//	       "<a title='External statistics service' target=extern href=http://astrostatistics.psu.edu/cgi-bin/VOStatBeta1/stats1.cgi/load?filetype=votable&uploaded_url="+xmlURL+" onClick='return nw(this.href, this.target)'>VOStat</a> | "+
    ...
    rd         = new readdata(elem, dataURL);
    
    rd.setSelectCallBack(flipFile, flipFile);
    rd.setXSLBase("/vo/datascope/xsl");
