...
//	    return new VizierFilter();
    ...
    /** What was the count for the given id?  If the
     *  id does not seem to be handled, a 1 will be returned.
     */
    public abstract int count(String id);
    
    /** How much was added to the size field when doing the query
     *  of the service.
     */
    public abstract double fudge();
    
    /** Get back a hash of identification string and the counts
     *  associated with them.  Note that the keys will normally
     *  not be the full ivorn of the services.
     */
	...
	Filter f = (Filter) Class.forName("net.ivoa.datascope."+args[0]+"Filter").newInstance();
	f.invoke(Double.parseDouble(args[1]), Double.parseDouble(args[2]), Double.parseDouble(args[3]));
