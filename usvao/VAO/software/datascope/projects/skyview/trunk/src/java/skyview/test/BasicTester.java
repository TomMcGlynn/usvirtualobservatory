 ...
 *  <dl>
 *     <dt>1-99    <dd> Checking individual surveys.
 *     <dt>101-199 <dd> Samplers
 ...
 *     <dt>1600-1601 <dd> Image intensity scaling (log, linear, sqrt)
...
public class BasicTester {
	...
	String[] surveys= new String[]{
	    ...
	    "neat", "rass", "rasshb", "rasssb"
	};
	
	String[] allSky = new String[] {
	    "1420mhz", "408mhz", "heao1a",
	    "cobe", "cobeaam", "cobezsma",
	    "comptel",
	    "egretsoft", "egrethard", "egret3d",
	    "rass.25kev", "rass.75kev", "rass1.5kev",
	...
	String[] projections = {"Tan", "Sin", "Car", "Ait", "Csc", "Zea", "Arc"};
	...
	assertEquals("Offset test:", sum0, sum1);
	Settings.put("projection", "Car");
	Settings.put("pixels", "300");
	...
	assertEquals("XXX should be lenght 1", Settings.getArray("xxx").length, 1);
	Settings.add("XXX", "bbb");
	assertEquals("XXX should be length 2", Settings.getArray("xxx").length, 2);
	...
	assertEquals("Cleared xxx3", Settings.getArray("xxx").length, 0);
	System.err.println("Delete worked");
	Settings.put("xxx", "a,b,c");
	assertEquals("Reset xxx", Settings.getArray("xxx").length, 3);
	...
	assertEquals("Use Lat/Lon or position", val1, val2);
	img.clearImageCache();
	...
	assertEquals("Using copyWCS", val1, val3);
	...
	if (base >= minTest && base <= maxTest) {
	    ...
	    assertEquals("5x5 box: in", data[5*11+5], 1./25);
	    assertEquals("5x5 box: in", data[5*11+6], 1./25);
   	    assertEquals("5x5 box: in", data[5*11+7], 1./25);
	    assertEquals("5x5 box: in", data[6*11+5], 1./25);
	    assertEquals("5x5 box: in", data[7*11+5], 1./25);
	    assertEquals("5x5 box: out", data[8*11+5], 0.);
	    assertEquals("5x5 box: out", data[5*11+8], 0.);
	}
	base += 1;
	...
	if (base >= minTest && base <= maxTest) {
	    ...
	    assertEquals("5x5 box: in", data[5*11+5], 1./5);
	    assertEquals("5x5 box: in", data[5*11+6], 1./5);
	    assertEquals("5x5 box: in", data[5*11+7], 1./5);
	    assertEquals("5x5 box: out", data[6*11+5], 0.);
	    assertEquals("5x5 box: out", data[5*11+8], 0.);
	}
	...
	if (base >= minTest && base <= maxTest) {
	    ...
	    assertEquals("5x5 box: in", data[5*11+5], 1./5);
	    assertEquals("5x5 box: in", data[6*11+5], 1./5);
	    assertEquals("5x5 box: in", data[7*11+5], 1./5);
	    assertEquals("5x5 box: out", data[5*11+6], 0.);
	    assertEquals("5x5 box: out", data[8*11+5], 0.);
	}
