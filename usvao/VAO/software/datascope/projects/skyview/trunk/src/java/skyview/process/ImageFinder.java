...
public abstract class ImageFinder {
	...
	if (type == null) {
	    ...
	    try {
	        return (ImageFinder) Class.forName(type).newInstance();
	    } catch (Exception e) {
		try {
		    return (ImageFinder) Class.forName("skyview.process.imagefinder."+type).newInstance();
		} catch (Exception f) {
		    System.err.println("Error: Cannot create imagefinder of class:"+type);
		    // Just return a null
		    return null;
		}
	    }
