...
public class Imager {
	...
	if (Settings.get("imagefinder") == null) {
	    Settings.put("imagefinder", "Border");
	}
	lastImager = this;
    }
	...
	try {
	    finder = (SurveyFinder) 
	      Class.forName(Settings.get("surveyfinder")).newInstance();
	} catch (Exception e) {
	    System.err.println("Error creating SurveyFinder: "+e);
	    return;
	}
	...
	for (int i=0; i<updateClasses.length; i += 1) {
	    try {
		SettingsUpdater su = (SettingsUpdater) Class.forName(updateClasses[i]).newInstance();
		su.updateSettings();
	    } catch (Exception e) {
	...
	if (finder == null) {
	    finder = (SurveyFinder) Class.forName(Settings.get("surveyfinder")).newInstance();
	}
	...
	System.err.println("Imager (v2.3) starting.");
	...
	System.err.println("***Centroid:"+cpix[0]+","+cpix[1]);
	...
	Image[]  cand  = surv.getImages(pos, maxSize);
	    ...
	    String msg = "Survey: "+Settings.get("_currentSurvey")+" No candidate images were found in the region.  Outside the coverage area?";
	...
	if (match != null  && Settings.get("GeometryTwin") != null) {
	...
	if (match != null) {	
	    ...
	    if (Settings.get("mosaicker") == null) {
	        System.err.println("  No mosaicker specified");
		Settings.put("ErrorMsg", "No mosaicker specified");
		output=null;
	    } else {
		doProcess("Mosaicker");
	    }
...
//    private void writeFits(Fits f) throws Exception {
	...
	if (center != null) {
	    ...
	    System.err.println("Central coords at:"+coords[0]+","+coords[1]);
