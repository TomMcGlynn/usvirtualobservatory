...
public class ConeQuerier implements Runnable {
	...
	urlString = checkURLEnd(baseURL) + "RA="+ra+"&DEC="+dec+"&SR="+size;
	this.size = size;
	this.tableID   = id;
    }
	...
	if (!criteria) {
	    criteria = true;
	    qualifiers = new ArrayList<ConeQualifier>();
	...
	if (xid.equals("ned")) {
	    baseURL = "http://nedwww.ipac.caltech.edu/cgi-bin/nph-NEDobjsearch?search_type=Near+Position+Search&of=xml_main&";
	} else if (xid.equals("simbad")) {
	    baseURL = "http://simbad.u-strasbg.fr/simbad-conesearch.pl?";
	} else if (xid.indexOf("/") > 0) {
	    baseURL = "http://vizier.u-strasbg.fr/viz-bin/votable/-dtd/-A?-out.add=_RAJ2000,_DEJ2000&-source="+id+"&";
	} else {
	    baseURL = "http://heasarc.gsfc.nasa.gov/cgi-bin/vo/cone/coneGet.pl?table="+xid+"&";
	}
