...
public class XMLSurvey implements Survey {
	...
	ImageFinderCallBack(Position pos, double size) {
	    this.pos = pos;
	    updatePosition();
		...
		String s = new String(buf).trim();
		    ...
		    try {
		        ...
		        skyview.survey.ImageGenerator gen = 
		          (skyview.survey.ImageGenerator) Class.forName(s).newInstance();
	...
	// This should fill images with the strings for any images we want.
        doParse(sp, new XMLSurvey.ImageFinderCallBack(pos, size));
	...
	if (images.size() == 0) {
	    ...
	    ImageFactory imFac = (ImageFactory) Class.forName(imageFactory).newInstance();
