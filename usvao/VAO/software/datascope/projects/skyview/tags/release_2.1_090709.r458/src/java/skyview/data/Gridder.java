...
public class Gridder  {
	...
	if (Settings.has("Projection") && Settings.get("Projection").toLowerCase().equals("car")) {
	    ...
	    carShift = new CarShifter(coords);
	}
	...
	try {
	    if (carShift != null) {
	        // Shift the coordinates tile
	        forward.add(image.getWCS().getScaler().inverse());
	        forward.add(carShift);
	        forward.add(image.getWCS().getScaler());
	    }
	...
	for (double qlon=lonValues[0]; qlon <= lonMax; qlon += lonValues[1]) {
	    ...
	    if (Math.abs(lon-180) < 1.e-6) {
		drawLine(180.00000001, 180.00000001, latLimits[0], latLimits[1]);
		setLabel(null);
		drawLine(179.99999999, 179.99999999, latLimits[0], latLimits[1]);
	    } else {
	...
	for (int ix=0; ix<npts; ix += 1) { 
	    for (int iy=0; iy<npts; iy += 1) {
	        double[] vec = reverse.transform(new double[]{ix*dx, iy*dy});
    ...
    /** Unlike the other projections the Car projection is
     *  periodic. This doesn't cause a problem when sampling
     *  because we project the pixels in the user image into
     *  the projection of the survey data.  However when we
     *  draw a coordinate grid, we would (without this class)
     *  draw it in the projection plane near the reference
     *  point at (lon,lat)=(0,0).  If the user has requested a 
     *  center other than 0.,0., then the region displayed may
     *  go extend beyond this central tile.  E.g., if we
     *  do an all-sky plot around the anticenter (180,0), then
     *  the pixel at 190,0 is shifted left by 360 degrees
     *  from it's nominal location.  This class adds such
     *  shifts to the pixels but does only a single shift.
     *  It does not accommodate the fact that a Car projection
     *  can contain more than one tile, i.e., it can repeat
     *  an aribtrary number of times.  The gridder will only
     *  grid the single all-sky region centered at the coordinates
     *  the user has specified.
     */
    private class CarShifter extends Transformer {
	
	private double ra0, dec0;
	
	public String getName() {
	    return "Cartesian shifter";
	}
	public String getDescription() {
	    return "Shift to correct tile in repeating Cartesian plane";
	}
	...
	public CarShifter(double[] coords) { 
	    ra0  = coords[0];
	    while (ra0 > Math.PI) {
		ra0 -= 2*Math.PI;
	    }
	...
	public boolean isInverse(Transformer t) {
	    if (t instanceof CarShifter) {
		CarShifter tt = (CarShifter) t;
		return tt.ra0 == -ra0;
	    }
	    return false;
	}
	public Transformer inverse() {
	    return new CarShifter(new double[]{-ra0,-dec0});
	}
	public void transform(double[] in, double[] out) {
	    out[1] = in[1];
	    if (Math.abs(in[0]-ra0) > Math.abs(in[0]+2*Math.PI-ra0)) {
		out[0] = in[0] + 2*Math.PI;
	    } else if (Math.abs(in[0]-ra0) > Math.abs(in[0]-2*Math.PI-ra0)) {
		out[0] = in[0] - 2*Math.PI;
	    }
	}
    }
