 ...
 */
public class IJProcessor implements skyview.process.Processor, skyview.executive.SettingsUpdater {
    ...
    /** This method ensures the the IJProcessor setting is
     *  set up if any of it's facilities are requested.
     */
	...
	if ( Settings.has("invert") || Settings.has("grid") ||
	     ...
	     Settings.has("imagej")) {
	    Settings.add("postprocessor", this.getClass().getName());
...
//	byte[] xdata = (byte[]) ip.getPixels();
//	for (int i=0; i<xdata.length; i += 1) {
//	    System.out.println("Pixel:"+i+" "+xdata[i]);
//	}
    }
	...
	for (int i=0; i<pixels.length; i += 1) {
	    drawSymbol(ip, pixels[i][0], ny-pixels[i][1], symbols[i]);
	}
	if (labels) {
	    for (int i=0; i<pixels.length; i += 1) {
	...
	if (gridStr != null) {
	    ...
	    try {
		    ...
		    for (int j=1; j<line.length; j += 1) {
		        double[] p0 = line[j-1];
		        double[] p1 = line[j];
		        drawLine(ip, p0[0], ny-p0[1], p1[0], ny-p1[1]);
		    }
	        }
			...
			if (angle > Math.PI/2) {
		        ...
		        ip.addPlotString(labels[i], (line[p][0] + 3*sa+0.5), ny-(line[p][1]+1*ca+0.5), angle);
		    }
	        }
	...
	if (Settings.has("plotcolor")) {
	    Color col = getColor(Settings.get("plotcolor"));
	    if (col != null) {
	        ip.addColor(col);
	        ip.setColor(col);
	    } else {
...
//	    new ImageConverter(imp1).convertToGray8();
    ...
    void drawSymbol(ImageProcessor ip, double x, double y, int symbol) {
	...
	symbol = symbol % 5;
	if (symbol == 0) {
	    drawLine(ip, x-2*scale, y, x+2*scale, y);
	    drawLine(ip, x, y-2*scale, x, y+2*scale);
	    
	} else if (symbol == 1) {
	    drawLine(ip, x-2*scale, y-2*scale, x+2*scale, y+2*scale);
	    drawLine(ip, x+2*scale, y-2*scale, x-2*scale, y+2*scale);
	    
	} else if (symbol == 2) {
	    drawLine(ip, x-scale,y+scale, x+scale,y+scale);
	    drawLine(ip, x+scale,y+scale, x,  y-scale);
	    drawLine(ip, x,  y-scale, x-scale,y+scale);
	    ...
	    drawLine(ip, x+scale,y,   x,  y+scale);
	    drawLine(ip, x  ,y+scale, x-scale,y);
	    drawLine(ip, x-scale,y,   x,  y-scale);
	    drawLine(ip, x,  y-scale, x+scale,y);
	}
    }
    
    void drawLine(ImageProcessor ip, double x0, double y0, double x1, double y1) {
	int ix0 = (int)(x0+.5);
	int ix1 = (int)(x1+.5);
	int iy0 = (int)(y0+.5);
	int iy1 = (int)(y1+.5);
