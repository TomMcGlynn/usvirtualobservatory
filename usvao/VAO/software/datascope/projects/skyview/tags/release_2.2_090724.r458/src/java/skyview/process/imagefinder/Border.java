 ...
 * 
 *   <li> The best pixel is found for each corner.  For this class the
 *        best pixel is determined by the image where the pixel falls within
 *        the image, and the pixel is furthest from the edges of the image.
 ...
 *        as NON-PHYSICAL
 *   <li> If the best images are the same for all four corners, the match is returned.
 ...
 *   <li> If the corners all matched, we now check the edges.  If there is an edge
 *        pixel that does not match the corners (i.e., does not have the same
 *        best source image), then we do a recursion splitting that edge in half.
 *   <li> If all edges match, we mark interior pixels as using the same input image.
 *  </ul>
 */
