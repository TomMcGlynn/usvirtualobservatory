...
public class CatalogProcessor implements skyview.process.Processor,skyview.executive.SettingsUpdater {
	...
	if (centerUnit == null) {
	    centerUnit   = output.getWCS().inverse().transform(new double[]{nx/2., ny/2.});
	...
	for (int i=0; i< catalogs.length; i += 1) {
		...
		cat.startsWith("file:")) {
		cq = new ConeQuerier(cat, "cat"+i, Math.toDegrees(centerCoords[0]), Math.toDegrees(centerCoords[1]),
						   imageSize);
    ...
    /** Make sure that we have use this as a pre-processor if
     *  needed.
     */
	...
	if (Settings.has("catalog")) {
		...
		if (pres.equals(thisClass)) {
		    return;
		}
	    }
	    Settings.add("preprocessor",  this.getClass().getName());
	    if (Settings.has("postprocessor")) {
		Settings.put("postprocessor", "skyview.vo.CatalogPostProcessor,"+Settings.get("PostProcessor"));
	    } else {
		Settings.put("postprocessor", "skyview.vo.CatalogPostProcessor");
	    }
	}
    }
	...
	if (ps != null  && doPrint) {
	    ps.printf(" %3s | %-"+maxCatLen+"s| %-20s| %9s| %9s| %6s| %6s\n",
	...
	for (int i=0; i<catalogs.length; i += 1) {
		...
		if (px[0] >= 0 && px[1] >= 0 && px[0] <= nx  && px[1] <= ny) {
		    ...
		    if (hasRad) {
			if (unit[0]*centerUnit[0] + unit[1]*centerUnit[1] + unit[2]*centerUnit[2] < cosRad) {
			    // Skip this row
			    continue;
		        }
		    }
		    ...
		    if (ps != null && doPrint) {
		        ...
		        ps.printf(" %3d | %-"+maxCatLen+"s| %-20s| %9.4f| %9.4f| %6.1f| %6.1f\n",
			      count, catalogs[i], ids[p], newCoo[0], newCoo[1], px[0], px[1]);
		    }
		    pixLoc.add(px.clone());
		    symbols.add(i);
		}
