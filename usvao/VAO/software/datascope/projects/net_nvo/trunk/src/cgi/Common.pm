...
# my $cmd = "/usr1/local/java/bin/java -Xmx500M  -Xdebug -Xrunjdwp:transport=dt_socket,address=1044,server=y,suspend=y net.ivoa.util.Run ";
my $cmd = "/usr1/local/java/bin/java -Xmx500M  net.ivoa.util.Run ";
$ENV{CLASSPATH} =
     "$ALIB/" . '@JAR_NAME@'
  . ":$ALIB/net_nvo.jar"
  . ":$ALIB/skyview.jar"
  ...
  . ":$ALIB/commons-exec-1.1.jar"
  . ":$ALIB/javasysmon-0.3.3.jar"
  . ":$ALIB/stilts.jar";
...
sub run {
	...
	#
	my $query = new CGI;
	my %webParams = $query->Vars;
	$query->delete_all();
	foreach my $webKey (keys %webParams){
		my $safeKey = HTML::Entities::encode($webKey);
		my $safeValue = HTML::Entities::encode( $webParams{$webKey} );
		$query->param($safeKey, $safeValue);
	}
