...
vut_urlOpen (char *baseURL, char *resource)
{
    ...
    /* Get descriptor. */
    vutUrl_t *vutUrl = (vutUrl_t *) malloc (sizeof(vutUrl));
    vutUrl->urlbuf = (char *) malloc (SZ_URL);
...
vut_cmemInit ()
{
    ...
    vutCmem_t *md = (vutCmem_t *) malloc (sizeof (vutCmem_t));
    if (md == NULL)
	return (NULL);

    /* Allocate first buffer. */
    vutCblock_t *mb = (vutCblock_t *) malloc (sizeof (vutCblock_t));
    if (mb == NULL) {
	free ((void *)md);
	return (NULL);
    }
    ...
    /* Init buffer storage. */
    if ((mb->buf = (char *) malloc (MB_SIZE)) == NULL) {
	free ((void *)md);
	free ((void *)mb);
	return (NULL);
    }
...
vut_cmemCopy (void *cmem, char *str)
{
	...
	mb = (vutCblock_t *) malloc (sizeof (vutCblock_t));
	if (mb == NULL)
	    return (NULL);
	mb->buf = (char *) malloc (len+1);
	if (mb->buf == NULL)
	    return (NULL);
