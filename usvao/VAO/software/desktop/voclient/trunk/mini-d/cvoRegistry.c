...
/*  Private procedures
 */
static VOTResource *queryExecute (regQuery *query);
...
cvo_regSearch (char *term1, char *term2, int orValues)
{
    ...
    VOTResource *vot = (VOTResource *) VOC_NULL;


    if (!term1 && !term2)  				// NULL search terms.
	return ( (RegResult) 0);

    if (term1) {					// keyword search tern
        addSearchTerm (query, term1, TRUE);
        strcpy (query->terms[0].term, term1);
    }
    if (term2)  					// SQL constraint only
     ...
     */
    vot = queryExecute (query);

    /*  Create a handle for the result struct.
     */
    res = svr_newHandle ((void *) vot);
...
{
    regQuery *query = (regQuery *) calloc (1, sizeof (regQuery));
    RegQuery qhandle = svr_newHandle ((void *) query);
...
cvo_regExecute (RegQuery query)
{
    ...
    if (q) {
        VOTResource *vot = queryExecute (q);	/* execute the query 	   */
        res = svr_newHandle ((void *) vot);	/* create a result handle  */
    }
    return ((RegResult) res);
}
