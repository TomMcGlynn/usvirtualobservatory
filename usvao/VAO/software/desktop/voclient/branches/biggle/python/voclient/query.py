...
class DALService():
	...
	voclient.initVOClient('runid=voc')
	self.dal_id = False
	if protocol == 'ssa' :
	   self.dal_id = voclient.openSsap(baseurl)
        else : 
	   if protocol == 'sia' :
	      self.dal_id = voclient.openSiapConnection(baseurl)
           else :
	      if protocol == 'scs' :
	        self.dal_id = voclient.openConeConnection(baseurl)
	      else:
		if protocol == 'vaoreg' :
	          pass
	        else :
	          print "Error: Unkown protocol " + protocol
...
class DALResults():
    ...
    def fieldnames(self):
        ...
        """
	self._id = []
	fe = libvot.getFIELD(self._tc)
	self._id.append(libvot.getAttr(fe, 'ID'))
	while(True) :
	    fe = libvot.getNext(fe)
	    if(fe > 0) :
		self._id.append(libvot.getAttr(fe, 'ID'))
	    else :
		break
	...
	rec['param'] = 'False'
	if(fe == 0) :
	      fe = libvot.getPARAM(self._tc)
	      rec['param'] = 'True'
	rec['id'] = libvot.getAttr(fe, "ID");
	rec['name'] = libvot.getAttr(fe, "name")
	rec['datatype'] = libvot.getAttr(fe, 'datatype')
	rec['arraysize'] = libvot.getAttr(fe, 'arraysize')
	rec['width'] = libvot.getAttr(fe, 'width')
	rec['precision'] = libvot.getAttr(fe, 'precision')
	rec['unit'] = libvot.getAttr(fe, 'unit')
	rec['ucd'] = libvot.getAttr(fe, 'ucd')
	rec['utype'] = libvot.getAttr(fe, 'utype')
	rec['description'] = libvot.getAttr(fe, 'description')
	fields.append(rec)
	while(True) :
		fe = libvot.getNext(fe)
		if(fe > 0) :
	...
	self._ucd = []
	fe = libvot.getFIELD(self._tc)
	self._ucd.append(libvot.getAttr(fe, 'ucd'))
	while(True) :
		fe = libvot.getNext(fe)
		if(fe > 0) :
		   self._ucd.append(libvot.getAttr(fe, 'ucd'))
	...
	self._utype = []
	fe = libvot.getFIELD(self._tc)
	self._utype.append(libvot.getAttr(fe, 'utype'))
	while(True) :
		fe = libvot.getNext(fe)
		if(fe > 0) :
		   self._utype.append(libvot.getAttr(fe, 'utype'))
	...
	self.fieldnames()
        for i in range(len(self._id)) :
	      rstat[self._id[i]] = libvot.getTableCell(self._tdc, index, i)
	return rstat
	...
	rstat = []
        self.fieldnames()
	for i in range(len(self._id)) :
	      if(name.lower() == self._id[i].lower()) :
		   break;
	if i < len(self._id) :
	       for j in range(self._rowcount) :
	          rstat.append(libvot.getTableCell(self._tdc, j, i))
	return rstat
	...
	for i in range(len(self._id)) :
	      if(name.lower() == self._id[i].lower()) :
		   break;
	if i < len(self._id) :
	      rstat.append(libvot.getTableCell(self._tdc, index, i))
	return rstat
