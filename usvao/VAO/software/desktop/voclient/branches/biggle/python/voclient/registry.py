...
class Service(query.DALService):
    """
    A class for submitting searches to the VAO registry.  
    """

    STSCI_REGISTRY_BASEURL = "http://vao.stsci.edu/directory/NVORegInt.asmx/"
    _resAttr = ['Title', 'AccessURL', 'ContentLevel', 'Creator', 'ShortName', 'ReferenceURL',
		    'Type', 'Subject', 'Identifier', 'CapabilityClass', 'Waveband', 'Version']
    _resAttrVerbose = ['tags', 'description', 'identifier', 'type', 'version', 'capabilityValidationLevel',
		    'interfaceVersion', 'supportedInputParam', 'publishedID', 'shortName', 'publisher',
		    'updated', 'contentLevel', 'capabilityClass', 'interfaceRole', 'maxRadius', 'referenceURL',
		    'title', 'waveband', 'subject', 'regionOfRegard', 'capabilityID', 'interfaceClass',
		    'accessURL', 'maxRecords']
    ...
    def __init__(self, baseurl=None, resmeta=None, version="1.0"):
        ...
        query.DALService.__init__(self, baseurl, "vaoreg", version, resmeta)
    ...
    def create_query(self, keywords=None, servicetype=None, 
        ...
        :param sqlpred:      an SQL WHERE predicate (without the leading "WHERE") 
                             ...
                             keywords.
        """
        if not self.baseurl:  baseurl = Service.STSCI_REGISTRY_BASEURL
        srch = Query(self.baseurl)
        if sqlpred:
...
class Query(query.DALQuery):
    ...
    def __init__(self, orKeywords=True, baseurl=None, version="1.0"):
        ...
        """
        if not baseurl:  baseurl = Service.STSCI_REGISTRY_BASEURL
        query.DALQuery.__init__(self, baseurl, "vaoreg", version)
	...
	keywords = ''
	for i in range(len(self._kw)) :
	   keywords = keywords + self._kw[i] + ' '
	reg = voclient.regQuery(keywords, 0)
	if self._svctype != None:
		voclient.regConstSvcType(reg, self._svctype)
	voclient.regSortRes(reg, 0)
	ss = voclient.regGetQueryString(reg)
	return Results(ss)
	

class Results(query.DALResults):
    """
    ...
    def __init__(self,  url=None, version="1.0" ):
        ...
        """
        query.DALResults.__init__(self, url, "vaoreg", version)



class SimpleResource(query.Record):
    """
