...
def spectrumsearch(url, pos, size, band='', time='', format='all'):
    """
    ...
    return service.search(pos, size, band, time, format)
...
class SSAService(query.DALService):
    ...
    def search(self, pos, size, band='', time='', format='all'):
        """
        ...
        :param size:    search radius
	:param band:    waveband to search
...
class SSAQuery(query.DALQuery):
    ...
    def execute(self):
        ...
        """
        q = voclient.getSsapQuery(self.dal_id, self.pos[0], self.pos[1], self.size, self.band, self.time, self.format)
	ss = voclient.getQueryString(q, 2, 0)
        return SSAResults(url=ss)
...
class SSAResults(query.DALResults):
    ...
    def __init__(self, votable=None, url=None, version="1.0" ):
        ...
        """
        query.DALResults.__init__(self, url, votable, "ssa", "1.0" )
