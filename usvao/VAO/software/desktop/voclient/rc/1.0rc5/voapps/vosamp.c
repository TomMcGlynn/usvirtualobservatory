    ...
    *result     = NULL;
	...
	i++;
        if (ch > 0) {
            ...
            case 'T':  timeout 	   = atoi (optval);	i++; 	break;
		...
		if (strcasecmp (cmd, "exec") == 0 || 
		    ...
		    while (pargv[i]) {
			for (ip=pargv[i]; *ip; ip++)  /* replace ':' w/ '=' */
			    *ip = ((*ip == ':') ? '=' : *ip);
...
samp_init:
    ...
    /*  Process a single commadn if we're not in keep_alive mode.
     */
    if (!keep_alive) {
...
Tests (char *input)
{
   ...
   vo_taskTest (self, "access", "hub", NULL);
   vo_taskTest (self, "list", NULL);
   vo_taskTest (self, "load", input, NULL);
}
...
vos_msgHandler (char *sender, char *mtype, char *msg_id, int params)
{
     ...
     */
    if (!quiet)
        vos_printMessage (filt_mtype, samp_id2app (sampH, sender), params);
...
vos_printMessage (char *mtype, char *sender, Map map)
{
    ...
    for (i=0; i < npars; i++ )
        printf ("\t%s=%s", samp_getMapKey (map, i), samp_getMapVal (map, i));
