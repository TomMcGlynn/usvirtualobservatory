...
static Task  self       = {  "votcnv",  votcnv };                          
...
votcnv (int argc, char **argv, size_t *reslen, void **result)
{
	...
	if (ch > 0) {
	    ...
	    case 'f':    fmt    = strdup (optval);   	break;
	    case 'i':    indent =   atoi (optval);	break;
	    case 'n':    hdr--;				break;
	    case 'o':    name   = strdup (optval);   	break;
...
Tests (char *input)
{
    ...
    */
   vo_taskTest (self, "--fmt=vot", input, NULL);
   vo_taskTest (self, "--fmt=asv", input, NULL);
   vo_taskTest (self, "--fmt=bsv", input, NULL);
   vo_taskTest (self, "--fmt=csv", input, NULL);
   vo_taskTest (self, "--fmt=tsv", input, NULL);
   vo_taskTest (self, "--fmt=html", input, NULL);
   vo_taskTest (self, "--fmt=shtml", input, NULL);
   vo_taskTest (self, "--fmt=fits", input, NULL);
   vo_taskTest (self, "--fmt=fits", input, NULL);
   vo_taskTest (self, "--fmt=ascii", input, NULL);
   vo_taskTest (self, "--fmt=xml", input, NULL);

   /*  Example 2
    */
   vo_taskTest (self, "-f", "vot", "-i", "2", input, NULL);

   /*  Example 3
    */
   vo_taskTest (self, "-f", "vot", "-i", "0", input, NULL);

   /*  Example 4
    */
   vo_taskTest (self, "-f", "csv", "-n", input, NULL);
}
