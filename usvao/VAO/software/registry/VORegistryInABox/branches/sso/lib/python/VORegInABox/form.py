...
class RepositoryService: 
    ...
    def openRepositoryWithToken(self, user, token):
        ...
        """
        r = Repository(self.dir, user)
        if os.environ.has_key('VAOLOGIN_SESSION_ID') and \
           os.environ['VAOLOGIN_SESSION_ID'] == token:
            r.token = token
            # self.cacheToken(user, token)

        elif not self.tokenAuthenticates(user, token):
    ...
    def getSession(self):
        ...
        """
        if os.environ.has_key('VAOLOGIN_SESSION_ID'):
            # OpenID-based authentication accomplished
            return (os.environ.get('REMOTE_USER',''), 
                    os.environ['VAOLOGIN_SESSION_ID'])
...
#            doc.writexml(sys.stdout)
    ...
    def createNewUser(self):
        ...
        if len(problems) > 0:
            ...
            repos = Repository(self.dir, user)
            ...
            if os.environ.has_key('VAOLOGIN_SESSION_ID'):
                ...
                repos = makeRepository(self.dir, user, '', file, True)
                os.remove(file)
                self.redirectToView()
            else:
