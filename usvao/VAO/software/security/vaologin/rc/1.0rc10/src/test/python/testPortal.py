...
class PortalTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.cfg['vaologin.portal.logoutalias'] = "/logout"
    ...
    def testLogout(self):
        ...
        self.portal = ProtectedPortal(self.cfg, execScripts=False,
                                      ostrm=self.ostrm, estrm=self.estrm)
        os.environ['QUERY_STRING'] = ''
        os.environ['PATH_INFO'] = "/logout/goob"

        rurl = "http://www.example.org/"
        # pdb.set_trace()
        self.portal.enter()
        resp = self.ostrm.getvalue()

        murl = re.search(r"Location: (%s\S+)" % logout_url, resp)
        self.assert_(murl, "Unexpected response:\n"+resp)
        redirect = murl.group(1)
        self.assert_("returnURL="+rurl in redirect)

        ssosite = login.SSOSite(provider_server, rurl)
        return_url = ssosite.logout(redirect)

        self.assert_(not self.portal.sessionValid())
        self.assertEqual(rurl, return_url)

        # do it again, while not logged in
        self.portal.enter()
        resp = self.ostrm.getvalue()
