...
usage = "Usage: %prog [options] request|process return_url"
...
def main():
    ...
    if len(args) < 1:
        log.error("Missing arguments (operation return_url)")
    ...
    if len(args) < 2:
        ...
        log.fatal(None, 1)
    if len(args) > 2:
        log.error("Warning: ignoring extra arguments: " + str(args[1:]))
    return_url = args[1]
    ...
    conf = config.getConfig(opts.conffile)
    if opts.data_path:
    ...
    try:
        if op == opREQUEST:
            ...
            if resp.protocolFailed():
                log.fatal("OpenId protocol failure", 6)
            elif resp.wasCancelled():
                log.fatal("User cancelled login (session id=%s)"% req.sessionid,
                          2)
            elif resp.neededUserInput():
                log.fatal("non-interactive authentication not possible", 3)
            elif not resp.isAuthenticated():
                log.fatal("unknown authentication failure", 4)
