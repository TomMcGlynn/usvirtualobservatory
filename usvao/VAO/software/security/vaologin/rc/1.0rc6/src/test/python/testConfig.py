...
class ConfigTestCase(unittest.TestCase):
    ...
    def testGetConfigLocations(self):
        ...
        self.assertEquals(septr("src/test/conf/vaologin-py.cfg"), locs[0])
        self.assertEquals("vaologin-py.cfg", locs[1])
        self.assertEquals(septr("/etc/httpd/conf/vaologin-py.cfg"), locs[2])
        ...
        self.assertEquals(septr("/opt/sw/vaologin/1.0/cfg/vaologin.conf"), 
                          locs[0])
        self.assertEquals(septr("target/conf/vaologin-py.cfg"), locs[1])
        self.assertEquals("vaologin-py.cfg", locs[2])
        self.assertEquals(septr("/etc/httpd/conf/vaologin-py.cfg"), locs[3])
        self.assertEquals(4, len(locs))

    def testGetDefaultHomeDir(self):
        loc = config.getDefaultHomeDir()
        self.assertTrue(loc)
        config.def_product_home = "/usr"
        os.environ['VAOLOGIN_HOME'] = "/usr/local"
        loc = config.getDefaultHomeDir()
        self.assertEquals(loc,"/usr/local")
        del os.environ['VAOLOGIN_HOME']
        loc = config.getDefaultHomeDir()
        self.assertEquals(loc,"/usr")
        config.def_product_home = "/goob"
        loc = config.getDefaultHomeDir()
        self.assert_(loc!="/usr" and loc!="/usr/local")
    ...
    def testLocateConfigFile(self):
        ...
        os.environ['VAOLOGIN_CONF'] = \
            os.path.join(os.getcwd(), 'examples', "vaologin-py.cfg")
        ...
        self.assertEquals(loc, os.path.join(os.environ['VAOLOGIN_HOME'],
                                            'conf', 'vaologin-py.cfg'))

    def testGetConfig1(self):
        conffile = os.path.join(os.getcwd(), 'examples', "vaologin-py.cfg")
