...
usage = "Usage: %prog [options] status|request|process|end [return_url]"
description = """Authenticate a web service user via the OpenId protocol.
This tool supports four different operations:
(1) status (returns what stage of the login process the session is in),
(2) request (initiates the login process),
(3) process (validates the response from the openid service), and 
(4) end (ends the session--i.e. logs out the user). 
When either the request or process argument is specified, the return_url
argument is required.  See VAOLogin documentation for usage details."""

prog = "vaoopenid"
deferrstrm = sys.stderr
defoutstrm = sys.stdout

operations = ["request", "process", "status", "end", "logout" ]
opREQUEST = operations[0]
opPROCESS = operations[1]
opSTATUS  = operations[2]
opEND     = operations[3]
opLOGOUT  = operations[4]
...
def main():
    ...
    try:
        if op == opREQUEST:
            ...
            log.tell(req.logout(return_url))
...
def createCliParser(prog=prog):
    ...
    parser.add_option('-a', '--attributes', dest='attributes', 
                      help='Required attributes needed about user as comma-separated string; either the short name or URI can be provided')
