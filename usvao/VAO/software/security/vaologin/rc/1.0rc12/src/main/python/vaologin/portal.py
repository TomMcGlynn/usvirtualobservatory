...
class ProtectedPortal(object):
    ...
    def logout(self, forwardUrl=None, globally=None):
        """
        ...
        is already authenticated.  Note that sso.usvao.org recommends that
        globally be set to True, so this is the default (see below).
        @param forwardUrl   the URL to redirect to.  This can either be an
                             an absolute URL or a relative one.  If it starts
                             with a '/', it is taken as a relative URL with
                             respect to the server root.  
        @param globally     if True log the user out of the VAO network 
                             ...
                             "portal.logoutMode".  globally will default to 
                             True if the parameter is not set.
        """
        if not forwardUrl:
        ...
        if not re.match(r'^\w+:/', forwardUrl):
            if forwardUrl.startswith('/'):
                # relative to server root
                forwardUrl = self._formURLserver() + forwardUrl
            else:
                # relative to this URL
                forwardUrl = self._formURLbase() + '/' + forwardUrl

        redirect_url = self.request.logout(forwardUrl, globally);
