...
This package is supported on Linux and MacOS platforms.  To install
it, you must already have installed Perl (unless you skip the openssl
installation) and a C compiler such as gcc.  
...
prerequisites you've just installed.  To do this, use the
--with-support option when installing: 

   python setup.py install --home=$VAOLOGIN_HOME --with-support

The above assumed that your system's native version of python was
used; to use the python you just installed (e.g. because your system 
doesn't have one), type the following instead:

   $VAOLOGIN_HOME/bin/python.sh setup.py install --home=$VAOLOGIN_HOME \
      --with-support
...
Next when you build vaologin, you will need to use the --with-support 
option:
...
In this case, no extra options are needed when you build vaologin:

   python setup.py install --home=$VAOLOGIN_HOME

For more options, see type "python setup.py install --help".  

A Note about LD_LIBRARY_PATH
----------------------------

If you need to have the LD_LIBRARY_PATH set while you build this
package, it is highly recommended that you first update it to prepend
the VAOLogin destination directory:

  LD_LIBRARY_PATH=${VAOLOGIN_HOME}:$LD_LIBRARY_PATH

This will prevent any previously installed versions of the software
you are installing from interfering.  
