...
providing the software package dependencies required by vaologin.
You will find this useful on systems where prerequisite software must be
installed in user space.  You might need to do this if you do not have
root permission to install system software or when your web server
operates in a restricted environment (via chroot) which does not
include vaologin's required prerequisites.
...
no error message.  Note that to access the VAL Login Service's HTTPS
