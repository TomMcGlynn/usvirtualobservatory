...
cacert_files = os.path.join(*'etc/cacerts'.split('/'))
cacert_files = glob.glob(os.path.join(cacert_files,'*'))
...
setup(name='vaologin',
      ...
      cmdclass={'build_py':       ext_build_py,
                ...
                'build':          ext_build,
                'install':        ext_install        },
      data_files=[('cacerts', cacert_files), ('web', []), ('sys', []), 
                  ('var', []), ('conf', ['etc/vaologin-py.cfg']),
                  ('var/VAOsessions', []), ('var/associations', []), 
                  ('var/nonces', []), ('var/temp', []), ('var/repos', []) ])
