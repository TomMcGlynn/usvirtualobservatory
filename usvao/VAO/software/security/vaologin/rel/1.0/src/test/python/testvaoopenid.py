...
class VAOopenidTestCase(unittest.TestCase):
    ...
    def testRequest(self):
        ...
        # make sure we went stateful on authentication request
        tmpdir = os.path.join(statedir,'associations')
        self.assert_(os.path.exists(tmpdir), "Missing "+tmpdir)
    ...
    def testStatelessRequest(self):
        ...
        # sanity check
        tmpdir = os.path.join(statedir,'associations')
        self.assert_(not os.path.exists(tmpdir))
        tmpdir = os.path.join(statedir,'nonces')
        self.assert_(not os.path.exists(tmpdir))

        # tweak the config file for stateless protocol
        cfgfile = os.path.join(statedir,'test-stateless.cfg')
        os.system("sed -e '/useStatelessProtocol/ s/false/true/' %s > %s" % (conffile, cfgfile))
        self.assert_(os.path.exists(cfgfile))

        cl = "vaoopenid -c %s request %s" % (cfgfile, login.returnurl)
        sys.argv = cl.split()
        # pdb.set_trace()
        vaoopenid.main()
        lines = self.stdout.getvalue().strip().split('\n')
        ...
        # make sure we went stateless on authentication request
        tmpdir = os.path.join(statedir,'associations')
        self.assert_(not os.path.exists(tmpdir))
        tmpdir = os.path.join(statedir,'nonces')
        self.assert_(not os.path.exists(tmpdir))
    ...
    def testLogout(self):
        ...
        # now test ending the session via logout

        self.stdout = StringIO()
        vaoopenid.defoutstrm = self.stdout
        cl = "vaoopenid -c %s -s %s logout %s" % \
            (conffile, sessionid, login.returnurl)
        sys.argv = cl.split()
        vaoopenid.main()
        lines = self.stdout.getvalue().strip().splitlines()
        self.assert_(len(lines) > 0)

        redirect = lines[0].strip()
        self.assert_(redirect.startswith("https://%s/openid/logout" % server),
                     "unexpected logout url returned: " + redirect)
        
        self.stdout = StringIO()
        vaoopenid.defoutstrm = self.stdout
        ssosite = login.SSOSite(server, login.returnurl)
        returnedto = ssosite.logout(redirect)
