...
public class RegisterUser {
	...
	try {
        ...
        logger.info("Registered new user: " + userData.getUserName());
    ...
    public static StatusMessage resendEmail(String name) {
        ...
        try {
            ...
            if (data == null) {
                logger.info("resendEmail: user not found: "+name);
                return new StatusMessage("Sorry, but \"" + name
                    +"\" is not a known login name or registered email address.", false);
            }
            if (data.getStatus() != StatusDataHandler.getRequestStatusId()) {
                logger.info("resendEmail: user already confirmed: "+name);
	...
	logger.debug("Token is " + token);
        ...
        String usrPassToRmv = userData.passwordSha;
 	String usrSaltToRmv = userData.salt;
	String usrPasswordMethodToRmv = userData.passwordMethod;
	String[] hashNsalt = UserDataHandler.passwordSha("dummy"); // can be encrypted and saved in userData
	userData.passwordSha = hashNsalt[0];
	userData.salt = hashNsalt[1];
	userData.passwordMethod = hashNsalt[2];
        ...
        throws RegistrationException, MailAccessException
    {
        ...
        logger.info("Accepted confirmed user: " + userData.getUserName());
