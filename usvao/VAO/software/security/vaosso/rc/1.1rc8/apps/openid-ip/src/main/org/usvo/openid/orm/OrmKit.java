...
public class OrmKit {
    ...
    static {
        try {
            ...
            String propfile = System.getProperty("openid.hibernate.properties");
            if (propfile != null) {
                try {
                    if ((new File(propfile)).exists()) {
                        propdata = new FileInputStream(propfile);
                        log.info("Loading hiberate properties from "+propfile);
                    }
                    else 
                        log.warn("openid.hibernate.properties property set " +
                                 "but file does not exist: " + propfile);
                } catch (Exception ex) {
                    log.error("Failed to open hibernate properties: " +
                              propfile + "; using default resource instead.");
                }
            }
            if (propdata == null) 
                propdata = OrmKit.class.getResourceAsStream("hibernate.properties");
            cp.load(propdata);
            propdata.close();
    ...
    /*
    public static PasswordData loadPwData(final String username) {
        return username == null ? null : go(new SessionAction<PasswordData>() {
            @Override public PasswordData go(Session session) {
                return (PasswordData) session.createCriteria(PasswordData.class)
