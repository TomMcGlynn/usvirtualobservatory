...
public class RADataHandler {
    ...
    public static boolean addNilRAData() throws DatabaseAccessException {
        logger.debug("add the Nil RA record, ra_id=-1");
        if (isNilRADataSet()) return false;

        String query = "insert into " + DatabaseConstants.RA_TABLE_NAME
	    + "( " + DatabaseConstants.RA_COL_ID + ", " 
            + DatabaseConstants.RA_COL_NAME + ", " 
            + DatabaseConstants.RA_COL_EMAIL_ID + ", "
	    + DatabaseConstants.RA_COL_DESC 
            + ") values (-1, 'PurseAdmin', 'root', '" 
            + "the nil RA (when no RA is needed)' )";
	DatabaseManager.runUpdateQuery(query);
        return true;
    }

    public static boolean isNilRADataSet() throws DatabaseAccessException {
        logger.debug("is Nil RA data set?");
        String query = "select * from " + DatabaseConstants.RA_TABLE_NAME
            + " where " + DatabaseConstants.RA_COL_ID + "=-1";
	...
	try {
	    ...
	    resultSet = statement.executeQuery(query);
            ...
            String msg = "failed to get RA table count";
            logger.error(msg, sqlExcep);
            throw new DatabaseAccessException(DatabaseManager.connectionURL, 
					      DatabaseManager.userName, msg,
                                              sqlExcep);
        }
        finally {
    ...
    public static int getRACount() throws DatabaseAccessException {
	logger.debug("count data");
        String query = "select count(*) as count from " 
            + DatabaseConstants.RA_TABLE_NAME;
	...
	try {
	    ...
	    resultSet = statement.executeQuery(query);
            if (resultSet == null) {
                String msg = "Programming error: empty count query on ra_table";
                throw new DatabaseAccessException(DatabaseManager.connectionURL,
