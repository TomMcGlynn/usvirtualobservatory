...
public class IdServer {
    ...
    /**
     * create a request from the web app's IdServer
     */
    public static IdRequest createIdRequest(ServletContext context,
                                            HttpServletRequest request, 
                                            HttpServletResponse response) 
        throws IOException, OpenIDException
    {
        return getInstance(context).createIdRequest(request, response);
    }
    ...
    private void init() throws IOException {
        ...
        Conf conf = Conf.get();
        if ("true".equalsIgnoreCase(conf.getProperty(Conf.KEY_DB_ASSOCIATIONS))) {
            // share stateful associations via a database table (which can be synchronized with mirror server if necessary)
            MysqlConnectionPoolDataSource ds = new MysqlConnectionPoolDataSource();
            ds.setUser(conf.getProperty(Conf.KEY_DB_USERNAME));
            ds.setPassword(conf.getProperty(Conf.KEY_DB_PW));
            ds.setUrl(conf.getProperty(Conf.KEY_DB_URL));
