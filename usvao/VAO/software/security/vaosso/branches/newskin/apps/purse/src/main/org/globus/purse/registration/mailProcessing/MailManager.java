...
public class MailManager {
	...
	logger.debug("Sending token mail to " + toAddress);
        ...
        if (logger.isDebugEnabled()) {
            StringBuilder sb = new StringBuilder("authentication data:");
            sb.append("\n  secret: ").append(secretToken);
            sb.append("\n  token: ").append(data.getProperty("token"));
	...
	Message msg = constructMailToSend(mailOptions.getUserAccountAddr(), 
			  ...
			  mailMessage);
	sendMail(msg);
        logger.info("Sent confirmation request email with token to "+toAddress);
    }
	...
	Message msg = constructMailToSend(mailOptions.getUserAccountAddr(), 
			  ...
			  mailMessage);
	sendMail(msg);
        logger.info("Sent acceptance/welcome email to " + toAddress);
    }
	...
	logger.debug("Sending mail");
        ...
        for(int tries=1; tries <= limit; ++tries) {
	    try {
                if (tries > 1) logger.debug("Send attempt No. " + tries);
	        Transport.send(message);
                break;
	    } catch (SendFailedException sfe) {
                if (tries == limit) {
                    String err = "Error sending message; giving up";
                    logger.error(err, sfe);
                    throw new MailAccessException(err, sfe);
                } else {
                    String err = "Error sending message (will try again)";
                    logger.error(err, sfe);
                }
	    } catch (MessagingException exp) {
                if (tries == limit) {
