...
public class DatabaseManager {
	...
	if (!initialized) {
	    ...
	    hashIterations = dbOptions.getHashIterations();
	    initialized = true;
	}
    } 
	...
	try {
		...
		DriverManager.getConnection(baseURI + purseURI, config);
	    connNumber++;
    ...
    public static void setConfigProperty(String name, String val) {
        config.setProperty(name, val);
    }
    public static String getConfigProperty(String name) {
        return config.getProperty(name);
    }
    public static void removeConfigProperty(String name) {
        config.remove(name);
    }
}
