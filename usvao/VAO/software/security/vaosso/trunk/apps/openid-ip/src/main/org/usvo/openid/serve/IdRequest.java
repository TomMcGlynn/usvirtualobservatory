...
public class IdRequest {
     ...
     * @param status   the status of their authentication.  This will 
     *                    be consulted for its error message, if any, and
     *                    the associated username.  The success indicated 
     *                    by the status is ignored; it is assumed that 
     *                    authentication failed.
     * @param portal   the portal being logged into.  If null, the portal
     *                    will be discovered automatically.  
     */
    public void sendPortalLogin(AuthnAttempt status, Portal portal) 
        throws OpenIDException, IOException
    {
        if (oidreq == null) 
    ...
    {
        if (oidparams == null) 
            throw new IllegalStateException("No current OpenID request in "+
                                            "progress");
        ...
        throws OpenIDException, IOException 
    {
        ...
        processPortalLogin(status, immediate);
    }

    /**
     * processs a portal login request based on the given results at 
     * authentication.  If authentication was successful, this method 
     * will either send the user onto the portal, or, if necessary, 
     * present a form for confirming the sharing of user data.  Otherwise,
     * a login page is presented.
     * @param status     the results of authentication
     * @param immediate  if true, process this as a checkid_immediate request
     */
    protected void processPortalLogin(AuthnAttempt status, boolean immediate) 
        throws OpenIDException, IOException         
    {
    ...
    public AuthnAttempt signin(String username) {
        ...
        if (oidreq != null) authn.initAuthRequest(oidreq);
    ...
    /**
     * handle a username/password signin request. The signin credentials will
     * be retrieved from the request parameters, "username" and "password".
     */
    public void handlePortalSignin() throws OpenIDException, IOException {
        if (oidparams == null) 
            throw new IllegalStateException("No current OpenID request in "+
                                            "progress");
        AuthnAttempt status = signin();
        processPortalLogin(status, false);
    }

    /**
     * attempt to authenticate the user, either via a cookie or by processing
     * a username and password.  This is intended for accessing local services.
     ...
     * This is intended for accessing local services.
     * @param force      if true, force authentication by username and password
        ...
        throws OpenIDException, IOException
    {
        if (oidreq == null) 
            throw new IllegalStateException("No current OpenID Auth request in "+
                                            "progress");
        AuthnAttempt status = getAuthenticationStatus();
        String username = status.getUsername();
        if (! status.isSuccessful()) {
    ...
    public void logout(String returnto) throws OpenIDException, IOException {
        ...
        if (returnto == null && oidparams != null) {
            // we were in the middle of connecting to a portal; cancel it
            cancelLogin();
            return;
        }
