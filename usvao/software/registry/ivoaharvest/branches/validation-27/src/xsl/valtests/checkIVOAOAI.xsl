...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
   ...
   <!-- 
     ...
     -->
   <xsl:param name="doVOResourceCheck"/>
   ...
   <xsl:template match="oai:ListMetadataFormats" mode="tests">
      ...
      <xsl:for-each select="oai:metadataFormat[oai:metadataPrefix='ivo_vor']">

         <xsl:variable name="stat1">
            <xsl:copy-of 
                 select="oai:schema='http://www.ivoa.net/xml/RegistryInterface/v1.0'"/>
         </xsl:variable>

         <xsl:call-template name="reportResult">
            <xsl:with-param name="item">RI3.1.2d</xsl:with-param>
            <xsl:with-param name="status" select="$stat1"/>
            <xsl:with-param name="type">rec</xsl:with-param>
            <xsl:with-param name="desc">
               <xsl:text>Recommend setting &lt;schema&gt; to </xsl:text>
               <xsl:text>http://www.ivoa.net/xml/VOResource/v1.0 </xsl:text>
               <xsl:text>for the ivo_vor format.</xsl:text>
            </xsl:with-param>
         </xsl:call-template>

         <xsl:variable name="stat2">
            <xsl:copy-of 
      select="oai:metadataNamespace='http://www.ivoa.net/xml/RegistryInterface/v1.0'"/>
         </xsl:variable>

         <xsl:call-template name="reportResult">
            <xsl:with-param name="item">RI3.1.2e</xsl:with-param>
            <xsl:with-param name="status" select="$stat2"/>
            <xsl:with-param name="type">rec</xsl:with-param>
            <xsl:with-param name="desc">
               <xsl:text>Recommend setting &lt;metadataNamespace&gt; </xsl:text>
               <xsl:text>to http://www.ivoa.net/xml/VOResource/v1.0 </xsl:text>
               <xsl:text>for the ivo_vor format.</xsl:text>
            </xsl:with-param>
         </xsl:call-template>
