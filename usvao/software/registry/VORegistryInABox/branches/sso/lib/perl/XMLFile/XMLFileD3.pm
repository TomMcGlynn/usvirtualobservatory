...
sub new
{
   ...
   $self->{'identScheme'} = $con->{'identScheme'};
   ...
   foreach my $metadata (@{$con->{'metadata'}})
   {
    ...
    # Get Ident Scheme
    $identScheme = $self->{'identScheme'}->[0];
    $ident_scheme = $identScheme->{'scheme'}->[0];
    $ident_idTag = $identScheme->{'identifierTag'}->[0];
    $ident_authId = $identScheme->{'authorityIdTag'}->[0];
    $ident_resKey = $identScheme->{'resourceKeyTag'}->[0];
...
#         my $datestamp = sprintf ("%04d-%02d-%02dT%02d:%02d:%02dZ", $year+1900, $mon+1, $mday, $hour, $min, $sec);
        ...
        my $in = new Pure::X2D ("$self->{'datadir'}$directory/$afile");
         ...
         # create identifier
         my $identifier = "";
#    my $ident_authId = $identScheme->{'authorityIdTag'}->[0];
#    my $ident_resKey = $identScheme->{'resourceKeyTag'}->[0];
        if ($ident_idTag) {
            $identifier = $ident_scheme.$in->param ($ident_idTag, "");
        } else {
            my $authId = $in->param ($ident_authId, "");
            my $resKey = $in->param ($ident_resKey, "");
            if ($self->{'longids'} eq 'no')
            {
               if ($resKey ne "") {
                  $identifier = 'ivo://'.$authId.'/'.$resKey; 
               } else {
                  $identifier = 'ivo://'.$authId; 
               }
            }
            else
            {
