...
<xsl:param name="maxColumns">100</xsl:param>
...
<xsl:param name="selectAllString" />
            ...
            <table class="data">
               ...
               <tbody>
                  <xsl:choose>
                     ...
                     <xsl:otherwise>
                        <xsl:apply-templates select="DATA/TABLEDATA|vo:DATA/vo:TABLEDATA|v1:DATA/v1:TABLEDATA|v2:DATA/v2:TABLEDATA|v3:DATA/v3:TABLEDATA|v4:DATA/v4:TABLEDATA" />
                     </xsl:otherwise>
                  </xsl:choose>
               </tbody>
...
<xsl:template name="columnheader">
   ...
   <xsl:choose>
      <xsl:when test="$posit = $urlColumnNum">
         <th class="unsortable" id="{$ID}_{$posit}"><xsl:value-of select="$name"/></th>
      </xsl:when>
      <xsl:otherwise>
...
<xsl:template name="selectAllList">
	...
	<xsl:if test="contains($currentRowNode, $selectAllString) and $currentRowNode/@vovid">
		<xsl:if test="not($rowCounter = 1)">,</xsl:if>
		<xsl:value-of select="$currentRowNode/@vovid"/>
	</xsl:if>
...
<xsl:template name="prefix-column">
	...
	<xsl:choose>	
		<xsl:when test="$renderPrefixColumn = 1">
			<td><input id="cb-{$ident}" type="checkbox" name="cb-{$ident}" value="cb-{$ident}">
			<xsl:variable name="isSelected">
				<xsl:call-template name="isSelected">
