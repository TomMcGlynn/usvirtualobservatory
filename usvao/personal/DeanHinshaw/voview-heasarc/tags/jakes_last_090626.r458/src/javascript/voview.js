...
/*
    ...
    522     this.selectRow = function(el, dataset) {
    523         var cclass = el.className;
    524    var boxes = el.getElementsByTagName("input");
    525    if (boxes.length > 0) {
    526             for (var i=0, f; i < me.selectedRows.length; i++) {
    527                 if (me.selectedRows[i] == dataset) {
    528                     // second click disables selection
    529                     me.selectedRows.splice(i,1);
    530                     if (cclass) {
    531                         el.className = removeSubstring(cclass,"selectedimage");
    532                     }
    533           boxes[0].checked = false;
    534           if (this.selectDel) {
    535               this.selectDel(dataset);
    536           }
    537                     return;
    538                 }
    539             }
    540        boxes[0].checked=true;
    541 
    542             // not in current selection, so add this to selection
    543             me.selectedRows.push(dataset);
    544             me.selectedRows.sort();
    545        if (this.selectAdd) {
    546            this.selectAdd(dataset);
    547        }
    548             if (cclass) {
    549                 el.className = cclass + " selectedimage";
    550             } else {
    551                 el.className = "selectedimage";
    552             }
    553    }
    554     };
...
if (boxes.length > 0) {
//		for (var i=0, f; i < me.selectedRows.length; i++) {      // what's the f for?
...
if (this.selectDel) {
this.selectDel(dataset);   // I don't think that this is ever defined?
}
				return;
			}
		}
...
if (this.selectAdd) {
this.selectAdd(dataset);   // I don't think that this is ever defined?
}
		if (cclass) {
...
/*
	this.selectRow_ORIG = function(el,dataset) {
		var cclass = el.className;
...
// Replaced by TAM to make work on IE.
//		    Sarissa.updateContentFromNode(xmlDoc, me.output, me.myXslProc,
//			function() {
...
// NEED @URL_PATH@ so long as the cgi-bin link exists
	this.valueXSLloader.makeRequest("@URL_PATH@@XSL_PATH@addValue.xsl");
}
