...
//		if ( me.tagRowsXslImported && !me.tagRowsAttributeAdded ) {
			...
			tmpDoc = me.tagRowsXslProc.transformToDocument(me.xml);
			me.xml = tmpDoc;
			me.filter.setBaseDocument(tmpDoc);
			// if ( me.xslt) me.sort();
//		}
...
//	this.removeSelections = function(selectors) {
//		if (! selectors) return;
//		var removeRows = new Array();
//		if (selectors.split) {
//			// looks like a string
//			removeRows = selectors.split(",");
//		} else if (selectors.length) {
//			// looks like a list
//			removeRows = selectors;
//		}
//		// **** Not finished !!!!
//	}
...
//	this.processData = function(sortColumn, sortOrder, newpage) {
...
//		if (me.tagRowsImported && !me.tagRowsAttributeAdded) me.tagRowsAttribute();
   ...
   this.applyUserFormats = function(){
	   ...
	   if( tables.length > 1 ){
		   this.printErrorMsg("More than one data table found.");
		   return;
	   }
	   var table = tables[0];
	   var rows = table.rows;
	   ...
	   for(var column in me.columnFormats){
		   ...
		   switch( typeof me.colFormatTypes[column] ){  
	   		...
	   		case "object":
	   			...
	   			break;
	   		default:
	   			this.printErrorMsg("column in columnFormats not of a valid type.");
	   			return;
		   }
