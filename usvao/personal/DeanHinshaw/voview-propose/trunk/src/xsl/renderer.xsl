...
<xsl:stylesheet version="1.0"
 ...
 exclude-result-prefixes="vo v1 v2 v3 v4">
    ...
    <xsl:param name="renderPrefixColumn">1</xsl:param>
    ...
    <xsl:variable name="filterCount" select="$paramlist[@name='VOV:FilterCount']/@value"/>
    <xsl:variable name="pageStart" select="$paramlist[@name='VOV:PageStart']/@value"/>
    <xsl:variable name="pageEnd" select="$paramlist[@name='VOV:PageEnd']/@value"/>
    <xsl:variable name="selectAllList" select="$paramlist[@name='VOV:SelectAllRows']/@value"/>
    ...
    <xsl:template name="resource">
		...
		<head>
			...
			<link rel="stylesheet" type="text/css" href="voview.css"/>
		</head>
		<body>
    	<!-- End debug header -->
        <div>
	...
	<xsl:template name="dataTable">
		...
		<form method="get" name="{$filterForm}" id="{$filterForm}"
			onsubmit="return {$filterCallback}(this);" onreset="return {$filterReset}(this);"
			action="">
    ...
    <xsl:template name="processIncludedRow">
        ...
        <tr id="{$selector}" onclick="{$selectRowCallback}(this,'{$selector}',event)">
    ...
    <xsl:template name="fieldstable">
        ...
        <span class="bbox rightbutton" onclick="{$resetColumnOrderCallback}();" title="Restore original column order">
            Reset&#160;column&#160;order
        </span>
    ...
    <xsl:template name="buttons">
        ...
        <div class="buttons {$location}">
            <div class="pagelabel">
                ...
                <xsl:if test="$npages != 1 or $filterCount"> of <b><xsl:value-of select="$filterCount"/>
                    </b></xsl:if>
                <xsl:if test="$totalCount">
                    (<b><xsl:value-of select="$totalCount"/></b> before filtering)
                </xsl:if>
                <xsl:if test="$sortColumnNum != ''">sorted by<xsl:value-of select="$sortColumn"/>
                </xsl:if>
            </div>
    ...
    <xsl:template name="columnheader">
        ...
        <xsl:choose>
            <xsl:when test="$posit = $urlColumnNum">
                <th class="unsortable" id="{$ID}_{$posit}">
                    <xsl:value-of select="$name"/>
                </th>
            </xsl:when>
            <xsl:otherwise>
                <th onclick="{$sortCallback}('{$ID}')" id="{$ID}_{$posit}">
