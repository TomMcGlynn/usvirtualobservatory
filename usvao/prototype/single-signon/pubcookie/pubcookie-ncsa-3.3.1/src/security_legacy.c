...
int security_init (pool * p, security_context ** contextp)
{
    ...
    pbc_log_activity (p, PBC_LOG_ERROR, "security_init: ready to load granting key (%s)", g_keyfile);
    ...
    if (g_keyfile) {
        ...
        if (fp) {
	    pbc_log_activity (p, PBC_LOG_ERROR, "security_init: loading granting key");
            context->g_key =
                (EVP_PKEY *) PEM_read_PrivateKey (fp, NULL, NULL, NULL);
	    pbc_log_activity (p, PBC_LOG_ERROR, "security_init: loaded granting key");
 ...
 * libpbc_mk_priv()).
 * @param include_sig if non-zero, include the signature in outbuf; otherwise,
 * just check it and remove it
...
/* old des decoder */
int libpbc_rd_priv_des (pool * p, const security_context * context,
			const char *peer, const char use_granting,
			const char include_sig,
			...
			int *outlen)
{
    ...
    pbc_log_activity (p, PBC_LOG_ERROR, "libpbc_rd_priv_des: hello\n");
    ...
    /* allocate outbuf */
    decrypt_len = len - sig_len - 2;
    if (include_sig) *outlen = decrypt_len + sig_len;
    else *outlen = decrypt_len;
    *outbuf = (char *) pbc_malloc (p, *outlen);
    ...
    des_cfb64_encrypt ((unsigned char *) buf + sig_len,
                       (unsigned char *) *outbuf, decrypt_len, ks, &ivec, &i,
                       DES_DECRYPT);

    pbc_log_activity (p, PBC_LOG_ERROR, "rd_priv: len = %d; sig_len = %d; outlen = %d; decrypt_len = %d", len, sig_len, *outlen, decrypt_len);

    /* verify signature */
    r = libpbc_rd_safe (p, context, peer, use_granting, *outbuf, decrypt_len,
                        mysig, sig_len);

    if (include_sig) memcpy(*outbuf + decrypt_len, mysig, sig_len);
...
/* AES decoder */
int libpbc_rd_priv_aes (pool * p, const security_context * context,
			const char *peer, const char use_granting,
			const char include_sig,
			const char *buf, const int len, char **outbuf,
			int *outlen, char alg)
{
    ...
    pbc_log_activity (p, PBC_LOG_ERROR, "libpbc_rd_priv_aes: hello\n");
    pbc_log_activity (p, PBC_LOG_ERROR, "libpbc_rd_priv_aes: including sig? %d\n", include_sig);
    ...
    if (!r) {
        ...
        *outlen = (include_sig ? tlen : tlen - siglen);
	pbc_log_activity (p, PBC_LOG_ERROR, "libpbc_rd_priv_aes: copying cookie %s: len = %d; tlen = %d; siglen = %d; outlen = %d", (include_sig ? "and sig" : ""), len, tlen, siglen, *outlen);
        *outbuf = (char *) malloc (*outlen);
	/* copy cookie */
        memcpy (*outbuf, tbuf + siglen, tlen - siglen);
	/* copy signature also, if requested */
	if (include_sig) memcpy(*outbuf + tlen - siglen, tbuf, siglen);
    }

    pbc_log_activity (p, PBC_LOG_ERROR, "libpbc_rd_priv_aes: len = %d; tlen = %d; siglen = %d; outlen = %d", len, tlen, siglen, *outlen);
...
{
    if ((alg == 'a') || (alg == 'A')) {
	pbc_log_activity (p, PBC_LOG_ERROR, "rd_priv: AES");
        return (libpbc_rd_priv_aes (p, context, peer, use_granting, include_sig,
                                    buf, len, outbuf, outlen, alg));
    }
    else {
	pbc_log_activity (p, PBC_LOG_ERROR, "rd_priv: DES");
        return (libpbc_rd_priv_des (p, context, peer, use_granting, include_sig,
                    ...
                    int *outlen)
{
    ...
    *outlen = 0;
    ...
    pbc_log_activity (p, PBC_LOG_ERROR, "libpbc_mk_safe: allocating sig (%d) for key (%d)", EVP_PKEY_size(thekey), (int) thekey);
