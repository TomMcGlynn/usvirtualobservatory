                    ...
                    pubcookie_dir_rec * cfg, pubcookie_req_rec * rr)
{
	...
	/* ---- */
	ap_log_rerror (PC_LOG_ERR, r, "pubcookie_user: granting cookie present (%d)", (int) cookie);
	/* decrypt cookie and re-encode in base64, for MyProxy */
	{
	    ...
	    char *signature = NULL;
	    int siglen;
	    int myproxy_result = 0;
	    ap_log_rerror(PC_LOG_ERR, r,
			  "myproxy prep: cookie length = %d", strlen(cookie));
	    /* 1. base 64 decode */
	    if (!libpbc_base64_decode(p, (unsigned char *) cookie,
				      (unsigned char *) blob, &bloblen)) {
		ap_log_rerror (PC_LOG_ERR, r,
			       "myproxy prep: libpbc_base64_decode() failed");
		myproxy_result = -1;
	    }
	    ap_log_rerror(PC_LOG_ERR, r, "myproxy prep: base64 decoded cookie."
			  "  Blob length = %d", bloblen);

	    /* 2. decrypt */
	    if (!myproxy_result && libpbc_rd_priv 
		(p, scfg->sectext, ap_get_server_name (r), 1, 1,
		 blob, bloblen, &plain, &plainlen,
		 scfg->crypt_alg)) {
		ap_log_rerror (PC_LOG_ERR, r,
			       "myproxy prep: libpbc_rd_priv() failed");
		myproxy_result = -1;
	    }

	    ap_log_rerror(PC_LOG_ERR, r, "myproxy prep: decrypted cookie."
			  "  Plain length = %d", plainlen);

	    /* 2. base 64 re-encode */
	    if (!myproxy_result) {
		plain64 = ap_palloc(p, plainlen * 4 / 3 + 20);
		if (!libpbc_base64_encode (p, plain, plain64, plainlen)) {
		    ap_log_rerror (PC_LOG_ERR, r,
				   "myproxy prep: base64_encode() failed");
		    myproxy_result = -1;
		}
		ap_log_rerror (PC_LOG_ERR, r, "myproxy prep: base64 encoded unencrypted cookie length = %d", strlen(plain64));
	    }

	    /* 4. store in requests's notes, for other modules to read */
	    if (!myproxy_result) {
		apr_table_set(r->notes, "plain_pubcookie_g", plain64);
		ap_log_rerror (PC_LOG_ERR, r, 
			       "myproxy prep: set unencrypted cookie in notes: %s", apr_table_get(r->notes, "plain_pubcookie_g"));
	    }
	    
	    /* 5. clean up */
	    if (plain) pbc_free(p, plain);
	    if (signature) pbc_free(p, signature);
	}
    ...
    if (cookie) {
        ap_log_rerror (PC_LOG_ERR, r, "pubcookie_user: got a cookie: $s", cookie);
        char *blob = ap_palloc (p, strlen (cookie));
        int bloblen;
        char *plain = NULL;
        int plainlen;
	...
	ap_log_rerror (PC_LOG_ERR, r,
		       "pubcookie_user: decrypting cookie with libpbc_rd_priv()");
        if (!res && libpbc_rd_priv (p, scfg->sectext, cred_from_trans ?
                                    ap_get_server_name (r) : NULL,
                                    cred_from_trans ? 1 : 0, 0,
                                    blob, bloblen, &plain, &plainlen,
                                    scfg->crypt_alg)) {
            ap_log_rerror (PC_LOG_ERR, r,
	...
	ap_log_rerror (PC_LOG_ERR, r, "unencrypting cookie");
	if (plain) {
	  char *plain64 = malloc(plainlen * 4 / 3 + 20);
	  if (libpbc_base64_encode (p, plain, plain64, plainlen)) {
	    ap_log_rerror (PC_LOG_ERR, r, "unencrypted cookie: %s", plain64);
	  }
	  free (plain64);
	}
