...
public class Range implements Comparable<Range> {
     ...
     * Input null for a parameter value if it is not used.
     *
     ...
     *
     * @throws  IllegalArgumentException   If value inputs not appropriate for type.
     * @throws  InvalidDateException       If ISO date string value is invalid.
     *
     */
    public Range (RangeType type, String value1, String value2,
	boolean order, boolean isoDate) throws InvalidDateException {

        // Enforce value requirements by type
        String errmsg = null;
        if ( type==RangeType.ANY ){
          if ( (value1 != null) || (value2 != null) )
             errmsg = "".format("received non-null value(s) for %s type, requires 0",type);
        }
        else
        {
          if ( (type == RangeType.ONEVAL )||(type == RangeType.HIVAL )||(type == RangeType.LOVAL ) )
          {
            if ( (value1 != null ) && (value2 != null) )
               errmsg = "".format("received 2 values for %s type, requires 1",type);
            else if ( value1 == null )
               errmsg = "".format("received null value1 for %s type, requires non-null",type);
          }
          else if ( type == RangeType.CLOSED ){
            if ( (value1 == null) && (value2 == null) )
              errmsg = "".format("received 0 values for %s type, requires 2",type);
            else if ( (value1 == null ) || (value2 == null) )
              errmsg = "".format("received 1 value for %s type, requires 2",type);
          }
        }
        if ( errmsg != null )
