...
/**
 * Generic HTTP Servlet for the SIAP(V2) protocol.  By default this service
 * merely implements a data query and returns a query response, but never
 * finds any data.  If properly configured to point at a data collection,
 * this generic SIAP class also has the ability to query a DBMS for image
 * metadata, implementing a real SIAP service for "archival" data (whole
 * images).  A cutout capability is also planned but is not yet
 * implemented.
 * 
 * <p>This is a generic HTTP servlet which implements a SIAP service using
 * the service implementation provided by the generic {@link
 * dalserver.siapv2.SiapService} class.  By default it merely echoes its
 * input arguments and defines and returns an empty VOTable containing all
 * predefined SIAP and Image Data Model (ImageDM) table fields.  To turn
 * this into a real data service, one can either configure it (in the
 * servlet web.xml) to query a local DBMS to find ImageDM metadata
 * describing the image data collection to be accessed, or subclass the
 * service to implement a custom SIAP service.
 * 
 * <p>In most cases, using the generic service code and precomputed ImageDM
 * metadata in a DBMS will be the simplest approach, and one which does not
 * require writing any service code.  If special processing is required a
 * custom service may be required.  To implement a custom service one would
 * 1) subclass {@link dalserver.siapv2.SiapService} and modify it as
 * necessary to query the local data collection, and 2) subclass the class
 * in this file (SiapServlet) and override the {@link
 * #newSiapService(SiapParamSet) newSiapService} method to call the custom
 * SiapService class provided in step 1).  The modified Servlet class
 * should then be runnable in any compliant Java application server such as
 * Apache Tomcat.
 */
public class SiapServlet extends HttpServlet {
	...
	try {
	    ...
	    if (serviceName == null)
		params.setValue(pname, serviceName = "siap");

	    serviceVersion = params.getValue(pname = "serviceVersion");
	    if (serviceVersion == null)
		params.setValue(pname, serviceVersion = "2.0");
	...
	// -------- QUERYDATA operation. --------
		...
		boolean textOut=false, csvOut=false;
		if (responseFormat != null) {
		    if (responseFormat.equalsIgnoreCase("text")) 
			textOut = true;
     ...
     * {@link dalserver.siapv2SiapService} class is used.  To build a real
     * data service, subclass SiapServlet and replace the newSiapService
     * method with one which calls a custom replacement for the builtin
     * generic SiapService class.
     * 
     * <p>This version includes the dataDir and dataType parameters, used
     * to implement a simple mechanism (see {@link
     * dalserver.siapv2.SiapService#accessData} for returning static
     * precomputed archival files from local storage on the server.
     *
