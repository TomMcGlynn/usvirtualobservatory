...
public class RequestResponse
{
    ...
    /** Table data rows. */
    private java.util.Vector<Vector<SavotTD>> rows;
    
    /** Active table row pointer (for get/set operations). */
    private java.util.Vector<SavotTD> row;
    ...
    public RequestResponse()
    {
        ...
        rows = new java.util.Vector<Vector<SavotTD>>();
    }
    ...
    public void addField(TableField field)
    {
        ...
        if (!fields.containsKey(field.getUtype()))
        {
        ...
        // Index by both ID and UTYPE.
        String key;
        if ((key = field.getId()) != null)
            fields.put(key, field);
        if ((key = field.getUtype()) != null)
            fields.put(key, field);
    ...
    public String getValue(String key) throws DalServerException
    {
        ...
        SavotTD td = row.get(field.getIndex());
        if (td == null)
            return (null);
        else
    ...
    public int addRow()
    {
        row = new java.util.Vector<SavotTD>(nFields);
        row.setSize(nFields);
        rows.add(row);
    ...
    // VOTable and CSV.  "Native" format is supported as a pass-through.
...
//		vot.setXmlns(this.xmlnsPrefix + "," + this.xmlnsUrl);
        // Set global VOTable metadata (namespaces, version, etc.) to obtain a valid VOTable
        vot.setXmlns("http://www.ivoa.net/xml/VOTable/v1.1");
        vot.setXmlnsxsi("http://www.w3.org/2001/XMLSchema-instance");
        vot.setXsischema("http://www.ivoa.net/xml/VOTable/v1.1 http://www.ivoa.net/xml/VOTable/v1.1");
    ...
    public void addResource(SavotVOTable vot) throws DalServerException
    {
        ...
        for (Iterator i = params.entrySet().iterator();  i.hasNext();  )
        {
            ...
            // remove "ssa" name space for user defined fields (they must not belong to any group), change it by "saf"
            if (param.getGroupId() == null && obj != lastObj)
            {
                String xmlnsPrefix= "saf";
        ...
        for (Iterator i = fields.entrySet().iterator();  i.hasNext();  )
        {
            ...
            obj = (Object) me.getValue();
            // remove "ssa" name space for user defined fields (they must not belong to any group), change it by "saf"
            TableField field = (TableField) obj;
