...
public class SsapServlet extends HttpServlet
{
     ...
     */
    private String DalServerVersion = "0.1";
    ...
    throws ServletException, IOException
    {
        ...
        try
        {
            ...
            // dalserver 0.1:
//            params.addParam(new Param("ServiceEngine", serviceName +
//                    ": SSAP version " + serviceVersion +
//                    " DALServer version " + DalServerVersion));
            //from the new version of the dalserver 0.1.1
...
// Previous implementation FORMAT=metadata using query REQUEST=FORMAT=METADATA
...
//
//                String pp = null;
//                pp = params.getValue("FORMAT");
//                if (pp.equalsIgnoreCase("metadata")){
//                    InputStream inStream = service.getMetadata(params);
//                    BufferedReader in =
...
//        }
        ...
        if (operation.equalsIgnoreCase("queryData"))
        {
            ...
            try
            {
                ...
                // Implement FORMAT=METADATA query return service metadata
                Param p = params.getParam("POS");
                Param s = params.getParam("SIZE");
                Param t = params.getParam("TIME");
                Param b = params.getParam("BAND");
                //Param f = params.getParam("FORMAT");
                //String ff = params.getParam("FORMAT").toString();
                String f = params.getValue("FORMAT");
                
                if (f ==null && !p.isSet() && !s.isSet() && !t.isSet() && !b.isSet())
                {
                    throw new DalServerException("no operation specified");
                }
                else if ( (f !=null && f.equalsIgnoreCase("metadata")) && !p.isSet() && !s.isSet() && !t.isSet() && !b.isSet())
                {
                    InputStream inStream = service.getMetadata(params);
                    BufferedReader in =
                            new BufferedReader(new InputStreamReader(inStream));
        ...
        else if (operation.equalsIgnoreCase("getData"))
        {
            ...
            {
                // Write a text-formatted data stream.
