...
public class SsapService
{
    ...
    throws DalServerException
    {
        ...
        // Set global metadata.
        //r.setDescription("DALServer null echo/text SSAP service");
        //r.setType("Results");
        ...
        r.addField(new TableField("DisplayRef",
                "DisplayRef", null, "char", "*", null, "Access.Display",
                "meta.display.url", "URL used to display dataset"));
    ...
    /**
     * Process a service metadata query using FORMAT=METADATA (similar to getCapabilities).
     * Although early SSA implementations included a getCapabilities operation for
     * returning service metadata, specification of this has been deferred to SSA V1.1.
     * When FORMAT=METADATA is given all the other input parameters should be ignored
     * and service metadata must be returned. This operation returns an InputStream to
     * an actual file in the local file system on the server. The file name is formed by
     * concatenating the serviceName with "Capabilities.xml". The file is assumed to be
     * available in the directory specified when the SsapService object was created, e.g.,
     * "path/myServiceCapabilities.xml".
     *
     */
    public InputStream getMetadata(SsapParamSet params)
    throws FileNotFoundException
    {
        File capabilities = new File(this.configDir,
                this.serviceName + "Metadata.xml");
        
        // Implementation for the EuroVO-DCA workshop:
        // The file to be retrived is under ESO/config folder
        //File capabilities = new File(System.getProperty("ESO") + "/config",
        //        "ForsProxySsaMetadata.xml");
        return ((InputStream) new FileInputStream(capabilities));
    }
