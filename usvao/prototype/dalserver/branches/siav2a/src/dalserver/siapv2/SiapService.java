...
public class SiapService {
     ...
     */
    public SiapService(SiapParamSet params) {
	if (params == null) {
	...
	// Set global metadata.
	r.setDescription("DALServer SIAP Version 2.0 Service");
	r.setType("results");
	...
	r.echoParamInfos(params);
        ...
        // This implementation currently supports only SIAPV2.
	if (!serviceClass.equalsIgnoreCase("siapv2"))
	    throw new DalServerException(
	      "SIAPV2 service called for unknown service class '" +
	      serviceClass + "'");

	// If dbType is null or is set to "test" we execute a query
	// against the testQuery DBMS class, otherwise we query an actual
	// DBMS (only MySQL is currently supported).

	if (dbType == null || dbType.equalsIgnoreCase("test")) {
	    ...
	    // has been configured to query an actual image database.
    ...
    /**
     * Directly access and individual image dataset.  This first version
     * of accessData merely returns the entire dataset.  The dataset to
     * be returned is specified by the PubDID parameter in the input
     * request.  The value of PubDID is an ivoa dataset identifier as
     * returned in an earlier call to the queryData operation.
     *
	...
	if (pubDid == null)
	    throw new DalServerException("missing PubDID value");
	...
	try {
	    ...
	    archiveId = mysql.queryDataset(pubDid, "archiveid");
	    if (archiveId == null)
	...
	// Get the metadata for the file and pass this to the client,
	// then copy the file data out.
	// TODO: add support to conditionally uncompress compressed files.
	...
	if (args.length == 0 || args[0].equals("query")) {
	    ...
	    SiapService service = new SiapService(null);
