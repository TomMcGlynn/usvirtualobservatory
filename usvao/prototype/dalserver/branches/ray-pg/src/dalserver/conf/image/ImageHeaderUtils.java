...
public class ImageHeaderUtils {
     ...
     */
    public String getInstrument(int hduIndex) {
        return kws.getStringValue(hduIndex, "INSTRUME", null);
    }
     ...
     */
    public double getDateObs(int hduIndex) {
        return kws.getDoubleValue(hduIndex, "DATE-OBS", 0);
    }

    /**
     * return the RA value.  0 is returned if the HDU does 
     * not have RA value.
     */
    public double getRA(int hduIndex) {
        int[] axes = findSpatialAxes(hduIndex);
        double val = 0;

        if (axes != null && axes[0] != 0)
             val = kws.getDoubleValue(hduIndex, "CRVAL" + Integer.toString(axes[0]), 0);

        return val;
    }

    /**
     * return the DEC value.  0 is returned if the HDU does 
     * not have DEC value.
     */
    public double getDEC(int hduIndex) {
        int[] axes = findSpatialAxes(hduIndex);
        double val = 0;

        if (axes != null && axes[1] != 0)
             val = kws.getDoubleValue(hduIndex, "CRVAL" + Integer.toString(axes[1]), 0);
     ...
     */
    public double[] getScale(int hduIndex) {
        int naxes = getNaxes(hduIndex);
        double[] out = null;
     ...
     */
    public String getCoordProjection(int hduIndex) {
        int[] axes = findSpatialAxes(hduIndex);
     ...
     */
    public double[] getCoordRefPixel(int hduIndex) {
        int naxes = getNaxes(hduIndex);
        double[] out = null;
     ...
     * not have CD* values.
     */
    public double[] getCoordCDMatrix(int hduIndex) {
        int naxes = getNaxes(hduIndex);
        double[] out = null;
    ...
    double sexg2dec(String val) throws NumberFormatException {
        double out = 0.0;
