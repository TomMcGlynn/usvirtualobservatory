...
public class SiapServlet extends HttpServlet {
    ...
    /**
     * The description of the columns we will expose in the response
     */
	...
	super.init(config);
        ...
        ServletConfig sletcfg = getServletConfig();
        String coldesc = sletcfg.getInitParameter("columnDescFile");
        if (coldesc != null) {
            InputStream res = getServletContext().getResourceAsStream("WEB-INF/"+coldesc);
            if (res != null) {
                try {
                    kwf = new XMLConfiguredKeywordFactory(res);
                }
                catch (Exception ex) {
                    System.err.println("Warning: Failed to parse keyword " +
                                       "config: " + ex.getMessage());
                }
            }
            else {
                System.err.println("Warning: keyword config file, " +
                                   coldesc+", not found in WEB-INF");
            }
        }
    }
	...
	return (new SiapService(params, kwf));
    }
