...
public class SiapMySql {
     ...
     */
    public SiapMySql(String jdbcDriver, KeywordFactory kwfact) throws DalServerException {
        kwf = kwfact;
	try {
	...
	RequestResponse r, String format) throws DalServerException {
        ...
        if (kwf != null) {
            for(Iterator<String> it=kwf.idIterator(); it.hasNext();) {
                Object item = kwf.getKeyword(it.next());

                if (item instanceof TableParam) {
                    TableParam tp = (TableParam) item;

                    if (!tp.hintContains("x"))
                        continue;
                    ...
                    if (name != null) {
                        ...
                        if (name.equalsIgnoreCase("AcRef")) {
                        ...
                        } else if (name.equalsIgnoreCase("AccessRefTTL")) {
                            value = params.getValue("accessRefTTL");
                            ...
                            File file = new File(datasetId);
                            if (file != null)
                                value = Long.toString(file.length());
                        } else {
                            value = getColumn(rs, name);
                        }
                        if (value != null)
