...
public class ScsService {
     ...
     */
    public ScsService(ScsParamSet params, KeywordFactory kwfact) {
    ...
    public void queryData(ScsParamSet params, RequestResponse response)
	...
	if (!serviceClass.equalsIgnoreCase("scs"))
	...
	if (dbType.equalsIgnoreCase("builtin")) {
		...
		mysql = new ScsMySql(jdbcDriver, kwf);
	    ...
	    throw new DalServerException("DBMS not supported");
	}
	...
	if (args.length == 0 || args[0].equals("query")) {
	    ...
	    ScsService service = new ScsService(null, null);
	    ...
	    ScsService service = new ScsService(params, null);
