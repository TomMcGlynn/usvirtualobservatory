...
public class ScsMySql {
     ...
     */
    public ScsMySql(String jdbcDriver, KeywordFactory kwfact) 
	...
	if (allsky)
        ...
        if (kwf != null) {
            // Base the output table on the configuration in kwf
            Object item = null;
            TableParam tp = null;
            
            for(Iterator<String> it=kwf.idIterator(); it.hasNext();) {
                item = kwf.getKeyword(it.next());
                if (item instanceof TableParam) {
                    tp = (TableParam) item;
                    if ("ID_MAIN".equals(tp.getUcd())) 
                        idColumn = tp.getName();
                    else if ("POS_EQ_RA_MAIN".equals(tp.getUcd())) {
                        raColumn = tp.getName();
                        ra_numeric = true;
                    }
                    else if ("POS_EQ_DEC_MAIN".equals(tp.getUcd())) {
                        decColumn = tp.getName();
                        dec_numeric = true;
                    }
                    fields.add(tp.getName());
                }
            }

            kwf.addFields(response, null);
        }
        else {
	...
	try {
		...
		if (dec_numeric) {
		    ...
		    where += decColumn + " BETWEEN " + dec1 + " AND " + dec2;
		}
		    ...
		    double ra2 = ra + sr;
                    ...
                    if (ra1 >= 0 && ra2 <= 360) 
                        where += raColumn + " BETWEEN " + ra1 + " AND " + ra2;
                    else if (ra1 < 0) 
                        where += "("+raColumn+" BETWEEN "+ (ra1+360) + 
                            " AND 360 OR "+raColumn+" BETWEEN 0 AND "+ra2+")";
                    else if (ra2 > 360) 
                        where += "("+raColumn+" BETWEEN "+ra1+" AND 360 OR " +
                            raColumn+" BETWEEN 0 AND "+ (ra2-360) +")";
                }
                query += " WHERE " + where;
	    }
