DALServer Package V0.3
D.Tody August 2008
----------------------------

This package contains code for implementing IVOA Data Access Layer (DAL)
services.  This version supports both SSAP and SCS (simple cone search).
...
The dalserver package currently includes base class implementations
of both the SCS (simple cone search) and SSAP (simple spectral access
protocol) services.  The base class implementations are functional
and can be executed without need to change any code.
