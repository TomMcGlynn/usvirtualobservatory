...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
	...
	<xsl:template match="ad:Select">
                <xsl:call-template name="geoName" /> 
	...
	<xsl:template match="ad:Where">  
                <xsl:call-template name="geoJoin" />                 
   ...
   -->
	...
	<xsl:template match="*[ contains(@xsi:type, 'trigonometricFunctionType') ]
        ...
        <xsl:template match="*[ contains(@xsi:type, 'systemFunctionType') ]">
            <xsl:if test="@Name='AREA'">
               <xsl:call-template name="geoArea" />               
            </xsl:if>
            <xsl:if test="@Name='CONTAINS'">               
                <xsl:call-template name="geoContains"/>   
            </xsl:if>
        </xsl:template>
...
<!--	<xsl:template match="*[ contains(@xsi:type, 'systemFunctionType') ]
		...
		<xsl:text> dbo.fSphRegionContainsXYZ(dbo.fSphSimplifyString('</xsl:text>
                <xsl:call-template name="regionTest" />                
		<xsl:text>'), </xsl:text>
                <xsl:value-of select="child::*/child::ad:Arg[@xsi:type ='adql:columnReferenceType']/@Table"/><xsl:text>.cx ,</xsl:text>
                <xsl:value-of select="child::*/child::ad:Arg[@xsi:type ='adql:columnReferenceType']/@Table"/><xsl:text>.cy,</xsl:text>
                <xsl:value-of select="child::*/child::ad:Arg[@xsi:type ='adql:columnReferenceType']/@Table"/><xsl:text>.cz) </xsl:text>                
		<xsl:value-of select="$spaceCharacter"/>
	</xsl:template>-->
...
<xsl:template name="geoContains">	    
  <xsl:choose>
  <xsl:when test="descendant::ad:Arg[@xsi:type='adql:geometryFunctionType' ]"> 
  <xsl:text> dbo.fSphRegionContainsXYZ(dbo.fSphSimplifyString('</xsl:text>
  <xsl:call-template name="regionTest" />                
  <xsl:text>'), </xsl:text>
  <xsl:value-of select="child::*/child::ad:Arg[@xsi:type ='adql:columnReferenceType']/@Table"/><xsl:text>.cx ,</xsl:text>
...
<xsl:template name="geoArea">	           
  <xsl-text>  dbo.fSphGetArea(dbo.fSphSimplifyString('</xsl-text>
  <xsl:call-template name="regionTest" /> 
  <xsl-text>'))</xsl-text>   
 </xsl:template>   

<!--This is been called from every sub templates where there are possibilities of occurances of spatial condtions e.g. In join and Where clauses-->
<xsl:template name="geoName">	           
   <xsl:choose>
             <xsl:when test="descendant::ad:Arg[@xsi:type='adql:geometryFunctionType']">
            <xsl-text>   DECLARE @htmTemp TABLE (HtmIdStart bigint, HtmIdEnd bigint);
                         INSERT @htmTemp SELECT HtmIdStart, HtmIdEnd FROM  </xsl-text>
                         
             <xsl-text> dbo.fSphGetHtmRanges(dbo.fSphSimplifyString('</xsl-text>                 
<!--         regionTest calls the template which returns region string to be used-->
             <xsl:call-template name="regionTest" />  
             <xsl-text>'))</xsl-text>                 
<!--             <xsl:if test="count(descendant::ad:Arg[@xsi:type='adql:geometryFunctionType' and @Name = 'POINT' ]/ad:Arg[@xsi:type ='adql:columnReferenceType']/@Table) >= 1 ">
             <xsl:value-of select="descendant::ad:Arg[@xsi:type='adql:geometryFunctionType' and @Name = 'POINT' ]/ad:Arg[@xsi:type ='adql:columnReferenceType']/@Table"/>
             <xsl-text>.</xsl-text>                      
             </xsl:if>  
             <xsl-text>HtmID between h.HtmIdStart and h.HtmIdEnd</xsl-text>-->
             </xsl:when>
             <xsl:otherwise> 
             </xsl:otherwise>
    </xsl:choose>         
 </xsl:template>   
 <xsl:template name="geoJoin">	           
   <xsl:choose>
             <xsl:when test="descendant::ad:Arg[@xsi:type='adql:geometryFunctionType']">                          
             <xsl-text> inner join  @htmTemp h on o.HtmID between h.HtmIdStart and h.HtmIdEnd </xsl-text>                                            
             </xsl:when>
             <xsl:otherwise> 
             </xsl:otherwise>
    </xsl:choose>         
...
<!--This is the template which actually returns region string -->
<xsl:template name="regionTest">	         
<!--   <xsl:if test="descendant::ad:Arg[@xsi:type='adql:systemFunctionType']">                         -->
   <xsl:choose>
     <xsl:when test="descendant::ad:Arg[@Name='REGION']">
          <xsl:value-of select="descendant::ad:Arg[@Name='REGION']/ad:Arg/ad:CharacterFactor/ad:Literal/@Value"/>                 
     </xsl:when>
     <xsl:otherwise>
           <xsl:choose>
            <xsl:when test="descendant::ad:Arg[@Name ='POLYGON' and @xsi:type='adql:geometryFunctionType']">    
                <xsl-text>POLY</xsl-text>   
                <xsl:value-of select="$spaceCharacter"/>                    
            </xsl:when>   
            <xsl:otherwise>
                <xsl:value-of select="descendant::ad:Arg[@Name != 'POINT' and @xsi:type='adql:geometryFunctionType' ]/@Name"/><xsl:text></xsl:text>
                <xsl:value-of select="$spaceCharacter"/>
            </xsl:otherwise>    
          </xsl:choose>
          
<!--          <xsl:value-of select="descendant::ad:Arg[@Name != 'POINT' and @xsi:type='adql:geometryFunctionType' ]/@Name"/><xsl:text> </xsl:text>-->
          <xsl:value-of select="descendant::ad:Arg[@Name != 'POINT' and @xsi:type='adql:geometryFunctionType']/ad:Arg/ad:CharacterFactor/ad:Literal/@Value"/><xsl:text> </xsl:text>
          <xsl:call-template name="for.loop">
          <xsl:with-param name="i">2</xsl:with-param>
          <xsl:with-param name="count"><xsl:value-of select="count(descendant::ad:Arg[@Name != 'POINT' and @xsi:type='adql:geometryFunctionType']/ad:Arg)"/></xsl:with-param>
          </xsl:call-template>
     </xsl:otherwise>
   </xsl:choose>             
<!--   </xsl:if>       -->
...
<!-- This is a for loop used to get values for region-->
...
<!--<b><xsl:value-of select="$i" />.</b>Hello world!-->
 <xsl:choose>
 <xsl:when test="count(descendant::ad:Arg[@Name != 'POINT' and @xsi:type='adql:geometryFunctionType']/ad:Arg[$i]/@Table)= 1 "> 
 <xsl:value-of select="descendant::ad:Arg[@Name != 'POINT' and @xsi:type='adql:geometryFunctionType']/ad:Arg/@Table"/><xsl:text>.</xsl:text><xsl:value-of select="child::ad:Arg[@Name != 'POINT']/ad:Arg[2]/@Name"/>
 </xsl:when>                  
 <xsl:otherwise>
 <xsl:value-of select="descendant::ad:Arg[@Name != 'POINT' and @xsi:type='adql:geometryFunctionType']/ad:Arg[$i]/ad:Literal/@Value"/>
 </xsl:otherwise>
 </xsl:choose> 
 <xsl:if test="$i != $count">
 <xsl:text> </xsl:text>
