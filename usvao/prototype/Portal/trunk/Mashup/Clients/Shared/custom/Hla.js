...
Ext.define('Mvp.custom.Hla', {
    statics: {
            downloadRecords: function (record, controller) {
                var urlColumn = Mvp.util.TableUtils.getColumnNameByUCD(controller.lastUpdateObject.columnInfo.columns, /VOX\:Image_AccessReference/);
                url = record.get(urlColumn),
                source = record.get('Source'),
                format = record.get('Format');
            //Ext.log('Downloading HLA file: Source = ' + source + ', Format = ' + format);

                if (source.match(/DADS/i) && (format = 'text/html')) {
                    var args = {
                        inputText: url,
                    ...
                    (Mvp.util.Util.isFtpUrl(url)) ? Mvp.util.Exporter.download(url) : Mvp.util.Exporter.downloadUrl(record.get(urlColumn), record.get('Dataset'));
                }
            },
        ...
        computePreviewHtml: function(record, title, imageSize, displayHeight, displayWidth) {
            ...
            if (!previewUrl || record.imageError) {
                previewUrl = '../Shared/img/nopreview.png';
                title = 'Preview Not Available';
            }

            // Construct the html to render.
            var html = util.createImageHtml(previewUrl, title, displayHeight, displayWidth, record);
            return html;
        },
        ...
        computePreviewUrl: function(record, imageSize) {
            ...
            if (dataUrl.match('fitscut.cgi')) {
                ...
                if (datasetName) {
                    previewUrl = 'http://hla.stsci.edu/cgi-bin/preview.cgi?output_size=256&dataset=' + datasetName;
                }
            }
