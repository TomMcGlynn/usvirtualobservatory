...
Ext.define('Mvp.custom.Caom', {
    statics: {
        caomPreviewRenderer: function (value, metaData, record, rowIndex, colIndex, store, view) {
            ...
            var type = record.get('obs_collection'),
            ...
            } else if ((type == 'HLA') || ((type == 'HST') && project == 'HLA')) {
                linkUrl = caom.hlaPreviewUrl(obs_id);
                imageUrl = caom.hstPreviewUrl(obs_id, 'small');
            } else if (type == 'HST') {
                linkUrl = imageUrl = record.get('jpegURL');
            } else if ((type == 'IUE') || (type == 'FUSE') || (type == 'BEFS')) {
                var linkUrl = record.get('jpegURL');
                linkUrl = caom.iuePreviewUrl(linkUrl);
                imageUrl = caom.iuePreviewUrl(linkUrl, 'small');
            }

            var title = record.get('target_name') + ' - ' + record.get('obs_id');
            // Construct the html to render.
            var html = (imageUrl) ? Mvp.util.Util.createImageHtml(imageUrl, title, 120, 120, record) :
                                    '<img src="../Shared/img/nopreview.png" alt="No Preview" title="No Preview" height="120" width="120" />';
        ...
        simplePreviewGenerator: function (record, height, width) {
            ...
            var title = ((type == 'HLA') || (((type == 'HST') && project == 'HLA'))) ? 'View in Interactive Display' : 'View Full Size Image';
            var html = (imageUrl) ? Mvp.util.Util.createImageHtml(imageUrl, title, height, width, record) :
                ...
                '</tpl>',
                {
                    moreInfoLink: function (collection, jpegURL, obs_id, target_name, instrument) {
                        ...
                        if (type === 'GALEX') {
                            ...
                            '&target=' +
                            target_name;
                        } else if ((type === 'IUE') || (type === 'FUSE')) {
                            linkUrl = jpegURL;
                        }
                        var html = linkUrl ? Mvp.util.Util.createLink(linkUrl, 'More...'): "";
        ...
        releaseDateRenderer: function (value, meta, record, rowIndex, colIndex, store, view) {
            if (value <= 0) return;
            var mjdTime = Mvp.custom.Generic.mjdRenderer(value);
            return mjdTime;
        }
    }
})