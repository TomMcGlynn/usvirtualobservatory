Ext.define('Mvp.gui.GridView', {
    ...
    */
    ...
    constructor: function (config) {
        ...
        this.crossMatchButton = Ext.create('Ext.button.Button', {
            icon: Mvp.util.Constants.CROSS_MATCH_ICON[Mvp.util.Constants.ICON_SIZE],
            scale: Mvp.util.Constants.ICON_SIZE,
            tooltip: 'Cross-Match this table with any table at CDS.',
            scope: this,
            shadow: true,
            style: 'border: 1px solid #000000;',
            margin: '0 1 0 1',
            handler: this.crossMatchPressed
        });
    ...
    getCrossMatchButton: function () {
        return this.crossMatchButton;
    },
    ...
    crossMatchPressed: function () {
        // Replace title characters that could be problematic in a filename, then remove duplicate underscores.
        var title = this.controller.getTitle();

        // Specify the filename.
        var filename = Mvp.util.Util.filenameCreator(title, 'xml');

        // Present the user dialog so they can select a file type and modify the file name.
        var crossMatchWindow = Ext.create('Mvpd.view.CrossMatch', {
            grid: this,
            filename: filename
        });
        crossMatchWindow.show();
    },
    ...
    addPressed: function () {
        ...
        if (collection.match('CAOM') !== null) collection = 'CAOM';
        var changed,
            obsid,
            field = (collection == 'CAOM') ? 'obsID' : 'Z_JOBNUM';
        if (field == 'obsID') {
            if (!records[0].get(field)) field = 'obsid';
        }

        Mvp.app.DownloadBasket.add(collection, records, field);

        var caom = Mvp.app.DownloadBasket.getBasket('CAOM'),
            dads = Mvp.app.DownloadBasket.getBasket('SIDBYJOB');
