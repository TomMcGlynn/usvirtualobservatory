...
Ext.define('Mvpc.view.CaomDetailsContainer', {
    ...
    constructor: function (config) {
        ...
        var collection = record.get('obs_collection'),
            ...
            caom_obsid = record.get('obsID') || record.get('obsid'),  // this is the unique ID within CAOM.
            ...
            title = target + ' - ' + obsId,
            urlTitle = ((collection == 'HLA') || (collection == 'HST')) ? 'View in Interactive Display' : 'View Full Size Preview';
        var url = urls[0],
            imageLink = urls[1];
        ...
        if ((collection != 'HLA') && (collection != 'HST')) interactiveDisplayButton.setTooltip('View Larger Image');
    ...
    openDetails: function (button, evt, config) {   // builds URL to MAST classic page or HLA download page
        var record = config.record,
            ...
            obsId2 = record.get('obsID') || record.get('obsid');
        if ((collection == 'HLA') || (collection == 'HST')) {
    ...
    download: function (button, evt, config) {
        ...
        var service = this.controller.searchParams.uid == 'CAOMDBTEST' ? 'Mast.Caomtest.Products' : 'Mast.Caom.Products';
        var request = {
            service: service,
            params: {
                obsid: this.record.get('obsID') || this.record.get('obsid'),
    ...
    addToBasket: function (button, evt, config) {
        var idCol = this.record.get('obsID') ? 'obsID' : 'obsid';
        Mvp.app.DownloadBasket.add('CAOM', [this.record], idCol);
        var basket = Mvp.app.DownloadBasket.getBasket('CAOM'),
            args = {
                obsid: basket,
                inputText: basket
            },
            search = this.controller.searchParams.uid.match('TEST') !== null ? 'CAOMDownloadTest' : 'CAOMDownload',
            searchParams = Mvp.search.SearchParams.getSearch(search);
