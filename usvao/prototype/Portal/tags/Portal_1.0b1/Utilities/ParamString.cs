			...
			if (paramString.IndexOf("[") >= 0)
			{
				...
				foreach (string param in paramStrings)
				{
					if (param.IndexOf("]") > 0)
					{
						...
						//
						object val="";
						if (dict.TryGetValue(psName, out val) && val.ToString().Trim().Length > 0)
						{
							sb.Replace("[" + psNameVal + "]", val.ToString());
						} 
						else if (dict.TryGetValue(psName.ToUpper(), out val) && val.ToString().Trim().Length > 0)
						{
							sb.Replace("[" + psNameVal + "]", val.ToString());
						} 
						else if (psVal.Length > 0)
						{
							sb.Replace("[" + psNameVal + "]", psVal);
						}
						else
						{
