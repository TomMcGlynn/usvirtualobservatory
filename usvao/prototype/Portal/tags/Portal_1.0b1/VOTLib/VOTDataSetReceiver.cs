	...
	public class VOTDataSetReceiver : VOTReceiver
	{
		...
		public void Warning (string format, params Object[] args)
		{
			if (log.IsFatalEnabled) 
			{
				...
				log.WarnFormat(tid + "   at line {0}, position {1}", reader.LineNumber, reader.LinePosition);
			}
			else
			{
				Console.WriteLine ("Warning: " + format, args);
				Console.WriteLine ("   at line {0}, position {1}", reader.LineNumber, reader.LinePosition);
			}
		}
		...
		public void Error (string format, params Object[] args)
		{
			if (log.IsFatalEnabled) 
			{
				...
				log.ErrorFormat(tid + "   at line {0}, position {1}", reader.LineNumber, reader.LinePosition);
			}
			else
			{		
		...
		private Type getDataSetType (string fieldName, string votDataType, string votArraySize)
		{
			...
			} else if ((votDataType == null) || !typeMapping.TryGetValue (votDataType, out dataSetType)) {
