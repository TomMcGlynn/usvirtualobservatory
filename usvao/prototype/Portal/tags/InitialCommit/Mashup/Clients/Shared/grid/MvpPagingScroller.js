/**
 * @class Mvp.grid.MvpPagingScroller
 * @extends Ext.grid.Scroller
 *
 * @private
 */
Ext.define('Mvp.grid.MvpPagingScroller', {
    extend: 'Ext.grid.Scroller',
    alias: 'widget.mvppagingscroller',
    //renderTpl: null,
    //tpl: [
    //    '<tpl for="pages">',
    ...
    getSizeCalculation: function() {
        ...
        // Gross hack because we are losing ownerCt somewhere.
        var owner = this.ownerCt;
        if (!owner) {
            owner = this.savedOwnerCt;
        } else {
            this.savedOwnerCt = owner;
        }
        
        var view   = owner.getView(),
            store  = this.store,
            dock   = this.dock,
            elDom  = this.el.dom,
            width  = 1,
