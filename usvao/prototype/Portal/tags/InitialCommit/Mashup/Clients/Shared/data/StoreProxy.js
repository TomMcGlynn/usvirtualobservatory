Ext.define('Mvp.data.StoreProxy', {
    ...
    doRequest: function(operation, callback, scope) {
        ...
        if (action === 'read') {
            ...
            console.log('Paging info: page(' + page + '), limit(' + limit + '), totalRecords(' + totalRecords + ')');
            var records = {};
            var start = limit * (page - 1);
            if (start < totalRecords) {
                var end = start + limit - 1;
                console.log('Paging info: start(' + start + '), end(' + end + ')');
                if (end >= totalRecords) {
                    end = totalRecords - 1;
                    console.log('trimmed end to ' + end);
                }
                records = backingData.getRange(start, end);
            } else {
                console.log('Warning:  start(' + start + ') < totalRecords(' + totalRecords + ')');
            }
