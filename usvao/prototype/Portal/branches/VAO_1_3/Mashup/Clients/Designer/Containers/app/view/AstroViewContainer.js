...
Ext.define('Mvpc.view.AstroViewContainer', {
    ...
    createAstroViewJavascript: function () {
        if (this.avcontainer)
        {
        	...
        	if (element)
        	{
        		// Create AstroView ThreeJs Version and set the global variable AstroView 
    ...
    createAstroViewFlash: function () {
        ...
        // Create Flash Vars
        var flashVars = {};
        flashVars.debug = (AppConfig && AppConfig.isDevelopment);
    ...
    onAstroViewEvent: function (msg) {
        ...
        if (msg.type === 'AstroView.Objects.Selected') {
			...
			this.updateAstroView(activeContext, false, false);
		}
    },
    ...
    fireSelectedEventForContexts: function(contexts) {
        for (var i=0; i<contexts.length; i++) {
            ...
            this.updateAstroView(context, true, false);
    ...
    onRecordsFiltered: function(event) {
        this.updateAstroView(event.context, false, false);
    },
    ...
    moveTo: function (position) {
		...
		var task = this.delayedCall(function(pos) {
			if (!this.avReady()) return;
			if (!this.hasMovedTo) {
				// Set a zoom level only on the first call to this.
