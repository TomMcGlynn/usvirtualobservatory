...
{	
	...
	{	
		...
		protected DataSet filterSortDataSet(MashupRequest muRequest, DataSet ds)
		{
			...
			if (ds != null && ds.Tables != null && ds.Tables.Count > 0)
			{
				...
				SortedDictionary<int, DataColumn> orderedColumns = getOrderedColumns(muRequest, dt);
				...
				// Find any column sorts applied in column order.
				string colSort = extractColumnSorts(orderedColumns);
				...
				copyTableExtendedProperties(dt, dtpage);
	...
	//		copyTableExtendedProperties(dt, dtpage);
		...
		protected void copyTableExtendedProperties(DataTable dtin, DataTable dtout) {
		...
		protected SortedDictionary<int, DataColumn> getOrderedColumns(MashupRequest muRequest, DataTable dt)
		{
		...
		protected string extractColumnSorts(SortedDictionary<int, DataColumn> orderedColumns) {
			string colSort = "";
			string sep = "";
			foreach (DataColumn col in orderedColumns.Values) {
				string sort = getColumnSort(col);
				if (sort != null) {
		...
		protected string getColumnSort(DataColumn col)
		{
			string sort = null;
			if (col != null && 
				col.ExtendedProperties != null && 
