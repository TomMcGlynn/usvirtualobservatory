...
namespace Mashup.Adaptors
{
	...
	public class Distribution : IAsyncAdaptor
	{
	    ...
	    {	
            ...
            if (muRequest.paramss.ContainsKey("prefixList") && muRequest.paramss["prefixList"].ToString().Trim().Length > 0)
            {
                prefixList = muRequest.paramss["prefixList"].ToString().Trim();
            }
            ...
            string[] files = filelist.Split(','),
                     prefixIds = prefixList.Split(',');
            int i = 0;
			foreach (string urlid in files)
			{
				string filename = getFileNameFromUrl(urlid),
                    fullname = (i >= prefixIds.Length) ? filename : prefixIds[i] + "/" + filename;
				ResourceClass resource = new ResourceClass(urlid, fullname);  // this lets us build a hierarchy in the bundle file
		...
		{	
			...
			if (response.IndexOf("|") >= 0)
			{
                ...
                for (int i = 1; i < len; i++)
                {
                    string[] files = records[i].Split('|');
                    if (files.Length > 1) {
                        realLen++;
                        if (files[1] == "COMPLETE") completed++;
                    }
                    if (files.Length > 2)
                    {
                        nBytes += System.Convert.ToInt32(files[2]);
                    }
                }
