...
Ext.define('Mvpc.view.FilterContainer', {
    ...
    doFilter: function (store, searchFn) {    // applies an array of filters to a store or MixedCollection
        ...
        if (this.appliedNumericFilters.length) {
            ...
            while (i--) {
                filterDef.filters.push({ property: this.appliedNumericFilters[i].column.dataIndex, ignoreValue: this.appliedNumericFilters[i].ignoreValue, range: this.appliedNumericFilters[i].range });
            }
    ...
    manageFilter: function (column, value, store, filters, addFilter, separator, separatorType) {   // add/remove a (key: value) pair filter to a set of filters
        ...
        if (separator && separatorType == 'bookend') value = separator + value + separator;
    ...
    constructor: function (config) {
        ...
        this.chartableFacets = { decimalFacets: [], integerFacets: [], categoryFacets: [] };    // used for charting
    ...
    autoCreateFacets: function () {
        ...
        for (var i = 0, fhtLength = this.allColumns.length; i < fhtLength; i++) {
            ...
            switch (rule) {     // rule - ColumnsConfig may override the other rules - calculate automatic rule first before overriding
                ...
                case 'never': continue;
                case 'hide': doNotAutoFacet = true; break;
                default:
            }
            if ((histType == 'discrete') && doNotFacet) continue;   // skip rest of loop if calculated to not be a facet and not overridden by CC

            if (histType == 'numeric') this.chartableFacets.decimalFacets.push({ column: this.allColumns[i].dataIndex, index: i });
            else if (type == 'int') this.chartableFacets.integerFacets.push({ column: this.allColumns[i].dataIndex, index: i });
            else this.chartableFacets.categoryFacets.push({ column: this.allColumns[i].dataIndex, index: i });
            ...
            if (histType == 'discrete') this.discreteFacets.push({ column: this.allColumns[i].dataIndex, separator: separator, count: len, dataType: type, index: i });
            if (!doNotAutoFacet) {
                this.facets.push({ column: this.allColumns[i].dataIndex, separator: separator, separatorType: separatorType, count: len, dataType: type, autoFacetRule: rule, index: i });
                this.automaticFacets.push({ column: this.allColumns[i].dataIndex, separator: separator, separatorType: separatorType, count: len, dataType: type, autoFacetRule: rule, index: i });     // force a backup copy of the auto facets so we don't have to do this again later
            }
        }
    ...
    prepareUpdateFromSelection: function (event) {
        if (event.context === this.controller) {
            // The event applies to us.
            if (!this.isResetting && (this.selectedFacets > 0)) {
                Ext.Msg.wait('Filtering...');
                this.start = (new Date()).getTime();    // can uncomment the other half of this for performance testing
                var task = new Ext.util.DelayedTask(function () {
                    if (this.staticCounts) {
        ...
        while (f--) {
            ...
            while (i--) {         // disable all checkboxes and count labels
                ...
                if (item.itemId.match('Count$')) {      // set count labels to 0 of original count
                    ...
                    var text = (this.isResetting) ? '(' + this.originalCounts[j].count : '(0';
                    item.setText(text + ' of ' + this.originalCounts[j].count + ')');
                }
            }
                ...
                if (hist.histogram.length > 0) {
                    if (facet.column === hist.columnName) {     // get the histogram for each column, if it exists
                        ...
                        while (j--) {         // repopulate the counts and enable checkable boxes
                            ...
                            var countbox = column.getComponent(record.key + 'Count');
                            checkbox.enable();
                            obj;
    ...
    populateFacets: function (config) {           // broken out of constructor to allow changing facets
        ...
        for (var i in this.allColumns) {
            ...
            if (c.dataIndex == '_selected_') {
                var trueBox = Ext.create('Ext.form.field.Checkbox', {
                    xtype: 'checkboxfield',
                    width: 170,
                    boxLabel: 'true',
                    itemId: 'true'
                }), falseBox = Ext.create('Ext.form.field.Checkbox', {
                    xtype: 'checkboxfield',
                    width: 170,
                    boxLabel: 'false',
                    itemId: 'false'
                });
                trueBox.on('change', this.filter, this, { column: '_selected_', value: 'true' });
                falseBox.on('change', this.filter, this, { column: '_selected_', value: 'false' });
                var fs = {
                    xtype: 'fieldset',
                    collapsible: true,
                    layout: 'column',
        ...
        if (!this.usingDefaultFacets || (this.facets.length != 0)) {
            ...
            for (var i = 0, htLen = histogramTable.length; (i < htLen) && (!this.usingDefaultFacets || (i < maxFacets)); i++) {
                ...
                for (var j = 0, cgLen = columnGroup.length; (j < cgLen); j++) {
                    ...
                    var v = val.key.replace(/\+/g, '\\+').replace(/\?/g, '\\?').replace(/\./g, '\\.').replace(/\$/g, '\\$').replace(/\^/g, '\\^').replace(/\*/g, '\\*').replace(/\[/g, '\\[').replace(/\]/g, '\\]').replace(/\(/g, '\\(').replace(/\)/g, '\\)');
                    cb.addListener('change', this.filter, this, { column: columnName, value: v, separator: this.facets[i].separator, separatorType: this.facets[i].separatorType });
                    var labelText = (this.staticCounts) ? '(' + val.count + ' Total)' : '(' + val.count + ' of ' + val.count + ')';
    ...
    populateNumericFacets: function (selectedFacets) {
        ...
        if (!this.usingDefaultFacets || (selectedFacets != 0)) {
            ...
            for (var i = 0, len = selectedFacets.length; (i < len) && (i < maxFacets); i++) {
                ...
                if (!(min == NaN) && !(max == NaN)) {
                    ...
                    var c = Ext.create('Mvpc.view.MinMaxSliderContainer', cfg);
