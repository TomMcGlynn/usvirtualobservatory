...
Ext.define('Mvpc.view.FilterContainer', {
    ...
    doFilter: function (filters, numericFilterFn, store, searchFn) {    // applies an array of filters in the form {property, [val1, val2,...], regexAsString} to a store
        var filterTable = [];
        if (this.availableNumericFilters) filterTable.push(numericFilterFn);      //  numeric facets are the quickest way to reject a column - do them first
    ...
    searchStore: function (value) {
        ...
        if (value !== '') {
            ...
            if ((filters.length > 0) || (this.availableNumericFilters.length > 0)) this.doFilter(filters, this.numericFilterFn, store);
        }
    },
    ...
    populateNumericFacets: function (selectedFacets) {
        if (!this.usingDefaultFacets || (selectedFacets.length < 21)) {
            ...
            for (i in selectedFacets) {
                ...
                this.availableNumericFilters.push({ column: column, range: [min, max], ignoreValue: ignoreValue, /*checkbox: fs.checkboxCmp,*/handle: c });
            }
        }
        else {
            var fs = Ext.create('Ext.form.FieldSet', {
    ...
    rangeChanged: function (property, vals) {
        ...
        if (!found) {
            ...
            while (i--) {
                facet = this.availableNumericFilters[i];
                if (facet.column == property) {
                    var f = Ext.clone(facet);
    ...
    filterRemoved: function (property) {
        var i = this.appliedNumericFilters.length,
            facet;
        while (i--) {
            facet = this.appliedNumericFilters[i];
