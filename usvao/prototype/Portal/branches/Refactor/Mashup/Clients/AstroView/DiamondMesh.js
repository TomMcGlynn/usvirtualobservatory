    ...
    if (this.inFrustum(frustum))
    {
        ...
        if (zlevel === this.geometry.zlevel)
        {
            ...
            {               
                ...
                this.proxyurl="http://mastdev.stsci.edu/portal/Mashup/MashupTest.asmx/MashupTestHttpProxy?url="+this.imageurl;   
                this.loadTexture(this.proxyurl);                
...
{
    console.debug(this.name + " onLoad() url: " + this.image.src);
...
{
    console.debug(this.name + " *** onError() url: " + this.image.src);
