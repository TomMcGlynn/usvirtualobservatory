Ext.define('Mvp.gui.GridView', {
    ...
    */
    ...
    addPressed: function () {
        ...
        var changed;
        var obsid;
        if (!window.DownloadBasket || !window.DownloadBasket[collection]) {
            window.DownloadBasket = {};
            obsid = records[0].get('obsID') || records[0].get('obsid');
            var bucket = ' ' + obsid; // space makes finding duplicates later easier
            for (var r = 1; r < len; r++) {
                obsid = records[r].get('obsID') || records[r].get('obsid');
                bucket += ', ' + obsid;
            }
            window.DownloadBasket[collection] = bucket;
            changed = true;
        } else {
            var bucket = '';
            var changed = false;
            for (var r = 0; r < len; r++) {
                obsid = records[r].get('obsID') || records[r].get('obsid');
                var s = ' ' + obsid;
                if (window.DownloadBasket[collection].indexOf(s) == -1) {
                    bucket += ', ' + obsid;
                    changed = true;
                }
            }
        ...
        var basket = window.DownloadBasket;
