	...
	public class Distribution : IAsyncAdaptor
	{
	    ...
	    {	
			...
			while (status == "PROCESSING")
			{
				...
				if (!parseResponse(response, out status, out message, out size, out progress))
				{
					throw new Exception("Distribution: Unable to parse Poll Response = " + response);
				}
                updateProgress(muResponse, progress);
			}
		...
		// parseResponse()
		////////////////////
		protected bool parseResponse(string response, out string status, out string message, out string size, out string progress)
		{	
			status = "";
			message = "";
            size = "0";
		...
		// updateProgress()
		///////////////////
		protected void updateProgress(MashupResponse muResponse, string percent)
		{	
			// Load the muResponse Data
			//muResponse.load(ds, complete);
			Dictionary<string, object>data = new Dictionary<string, object>();
