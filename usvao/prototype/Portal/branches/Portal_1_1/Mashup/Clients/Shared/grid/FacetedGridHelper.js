...
Ext.define('Mvp.grid.FacetedGridHelper', {
    ...
    constructor: function (config) {
        ...
        this.chartableFacets = { decimalFacets: [], categoryFacets: [] };        // list of facets that can be charted, populated by FilterContainer
    ...
    updateDisplay: function () {
        ...
        this.grid = Mvp.grid.MvpGrid.createGrid(table, this.centerWidth, this.centerHeight, this.title, dockedItems, this.extraColumns, this.pagesize, this.niceColumnNames, this.renderers);
        ...
        if (this.currentRowCnt > 0) {
            ...
            var config = { store: this.grid.store, recommendedFacets: this.facetConfigs, columns: keys, chartableFacets: this.chartableFacets, niceColumnNames: this.niceColumnNames, exclude: this.exclude, gridColumns: this.grid.columns, autoFacetRules: this.autoFacetRules };
        ...
        if (this.createImagePanel) {
            ...
            var tpl = Mvp.custom.Caom.dataviewTemplate();
    ...
    createDockedItems: function () {
        ...
        this.cancelButton = Ext.create('Ext.button.Button', {
            text: 'Cancel',
            tooltip: 'Cancel loading data for this table',
            hidden: false,
            scope: this,
            shadow: true,
            style: 'border: 1px solid #000000;', // The 'border' config has no effect, overriding the toolbar button style is a pain
        ...
        var dockedItems;
        if (useAV || isDevelopment) {
            dockedItems = [{
                xtype: 'toolbar',
                height: 28,
                items: [
                        this.statusLabel, this.reloadButton, this.cancelButton, '->', this.layerChooser, this.colorPicker, this.chartButton, this.exportButton, this.resolveLabel
            ...
            dockedItems = [{
                ...
                height: 28,
                items: [
                        this.statusLabel, this.reloadButton, this.cancelButton, '->', this.exportButton, this.resolveLabel
                ] // '->' used to right justify next element
            }];
        }
    ...
    computeStatusText: function () {
        ...
        if (this.cancelled) {
            text += '...<i>Load cancelled</i>';
        }
        if (this.additionalRowCnt) {
            text += '...' + this.additionalRowCnt + ' new rows received';
        }
        return text;
    },
        ...
        if (useAV && (this.grid.hasFootprints() || this.grid.hasPositions())) {
            ...
            if (layerOn) {
                ...
                if (rows.length > 0) {
                    ...
                    this.astroViewLayerId = AstroView.createLayer(Ext.encode(layer));
                }
            }
        }
    }
