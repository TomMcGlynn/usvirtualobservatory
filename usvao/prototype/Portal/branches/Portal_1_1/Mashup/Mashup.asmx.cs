...
{	
    ...
    {	
        ...
        {	
            ...
            {
				cleanUp(requestInfo, ex);
            }
        }
		#endregion
		
		#region cleanUp()
		private void cleanUp(string requestInfo, Exception ex)
		{	
			//
			// STEP 1: Log Original Exception Error Message and Send Email Notification to Developers
			//
			try
			{
				...
				string msg = "[CLEANUP] Exception caught while trying to log/email the Original Exception. " + requestInfo;
				Console.WriteLine(msg + "\n" + ex2.ToString());
			}
			
			//
			// STEP 2: Notify the pending Client of the failure:
			...
			{
				string msg = "[CLEANUP] Exception caught while Writing Error Response to Client. " + requestInfo;
			...
			{
				string msg = "[CLEANUP] Exception caught while Terminating Active Response Thread. " + requestInfo;
				log.Error(tid + msg, ex2);
				Mail.sendException(ref ex2, ex2.Message);
			}
			...
			{
				string msg = "[CLEANUP] Exception caught while Removing Error Response from Cache. " + requestInfo;
				log.Error(tid + msg, ex2);
				Mail.sendException(ref ex2, ex2.Message);
			}
		}
