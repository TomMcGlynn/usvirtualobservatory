	...
	public class Transform
	{
        ...
        public static void DataTableToJson(DataTable dt, StringBuilder sb )
        {
            ...
            {			
				...
				sb.Append("\n");
				DataTableExtendedProperties props = DataTableExtendedProperties.getProperties(dt);
				sb.Append(props.ToJson() + ",");
            ...
            {			
					...
					if (!checkRemoveColumn(col))
					{
						...
						if (properties != null && properties.Count > 0)
						{
							...
							foreach (string key in properties.Keys)
							{
								...
								object val = properties[key];							// value
								if (val == null)
								{
									Console.WriteLine("val = " + val);
								}
								sb.Append(DataRowValueToJson(val, val.GetType()));  	
							}
							sb.Append("}");
						}
						sb.Append("}");         				// col[i]
				...
				foreach (DataColumn col in dt.Columns)
                {
					if (!checkRemoveColumn(col))
					{
						...
						sb.Append("\"type\": \"" + Transform.SytemToExtjsType(col.DataType) + "\"");
						sb.Append("}");            				// field[i]
					}
				}
		...
		public static string SytemToExtjsType(System.Type type)
		{
			//
