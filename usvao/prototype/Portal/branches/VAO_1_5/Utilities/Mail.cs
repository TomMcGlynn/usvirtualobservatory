...
namespace Utilities
{
	public class Mail
	{
		...
		public static readonly string MAIL_DOMAINS_KEY = "MailDomains";
		...
		static string mailDomains = System.Configuration.ConfigurationManager.AppSettings.Get(MAIL_DOMAINS_KEY);
		...
		public static Boolean checkMailDomains ()
		{
			// Determine request domain and see if it is in the Domain Filter List
			Boolean domainFound = false;
			if (mailDomains != null && mailDomains.Trim ().Length > 0) 
			{
				string myDomain = HttpContext.Current.Request.Url.GetLeftPart(UriPartial.Authority);
				domainFound = mailDomains.Contains(myDomain);
			}
			return domainFound;
		}
