...
namespace VOTLib
{
	public class VOTWriter : XmlTextWriter
	{
		...
		public override void WriteStartDocument() {
			// This override is needed because the real implementation gets the encoding from
			// the TextWriter.  When we use a StringWriter as the text writer, it says the encoding is UTF-16,
			// which is not only not what we want (doubles the size unnecessarily), but the file doesn't even get written 
			// as UTF-16, which causes some parsers (like the CDS xmatch service) to fail to parse the file.
			// So instead, we write UTF-8, and hope that that's how the file ends up being written.
