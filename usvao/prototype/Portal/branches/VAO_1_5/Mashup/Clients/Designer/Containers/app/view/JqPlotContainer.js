...
Ext.define('Mvpc.view.JqPlotContainer', {
    ...
    setPlotExtrema: function (xField, yField) {
        ...
        // adjust maxima away from 0, minima towards 0
        xFacet.expandedMax = ((Math.abs(xmax) < .1) && (xRange < 10)) ? 0.1 : xmax * (xmax > 0 ? (1 + xFudge): (1 - xFudge));
        xFacet.expandedMin = ((Math.abs(xmin) < .1) && (xRange < 10)) ? -0.1 : xmin * (xmin > 0 ? (1 - xFudge): (1 + xFudge));
        yFacet.expandedMax = ((Math.abs(ymax) < .1) && (yRange < 10)) ? 0.1 : ymax * (ymax > 0 ? (1 + yFudge): (1 - yFudge));
        yFacet.expandedMin = ((Math.abs(ymin) < .1) && (yRange < 10)) ? -0.1 : ymin * (ymin > 0 ? (1 - yFudge): (1 + yFudge));
    },
