...
Ext.define('Mvp.search.receivers.ExtjsReceiver', {
    ...
    onResponse: function (extjsDataSet, requestOptions, queryScope, complete, updated, pagingExpectedRowCount, respObj) {
        ...
        if (respObj.updated) {
            ...
            // We store the entire extjs response, so that the entire DataSet object can be recreated on export actions,
            // and in case we ever want to use all that metadata for something else.
            // We only store it the first time, because the server only sends it the first time in order to reduce the size of
            // subsequent pages of data.
            if (!this.extjsDataSet) {
                this.extjsDataSet = extjsDataSet;
                this.tableExtendedProperties = table.ExtendedProperties;
                
                // Since we're storing the contents of the first table in the data store, etc., we can get save memeory by not storing all
                // that here.  In the export use case, we recontstruct that table's contents anyway.
                this.extjsDataSet.data.Tables[0] = null;
            }
            ...
            updateObject.createStore = function () {
                var store = me.createStore(dataInfo, me.columnInfo, me.extjsDataSet, me.tableExtendedProperties,
    ...
    createStore: function (dataInfo, columnInfo, extjsDataSet, tableExtendedProperties, prefetchCallback) {
        ...
        // We store the entire extjs response, so that the entire DataSet object can be recreated on export actions,
        // and in case we ever want to use all that metadata for something else.
        store.extjsDataSet = extjsDataSet;
        store.tableExtendedProperties = tableExtendedProperties;
