...
Ext.define('Mvpd.view.DownloadWindowContainer', {
    ...
    downloadHandler: function () {
        ...
        var prefixList = records[0].get('obs_collection') + '/' + records[0].get('obsID');
        this.size = records[0].get('size');
        this.useFilesize = true;
        for (var i = 1; i < len; i++) {
            var uri = records[i].get('dataURI'),
                id = records[i].get('obsID'),
                mission = records[i].get('obs_collection'),
                size = records[i].get('size');
            if (uri) {
                fileList += ', ' + uri;
                prefixList += ',' + mission + '/' + (id ? id : 'no_id');     // this lets us build a hierarchy in the bundle file
            }
            this.size += size;
    ...
    onResponse: function (responseObject, requestOptions, queryScope, complete, updated) {
        ...
        if (complete) {
            ...
            if (data) {
                ...
                //var fileList = (recoverable ? '<h3>Recoverable Files</h3><p />' + recoverableList: '') + (unrecoverable ? '<p /><h3>Unrecoverable Files</h3><p />' + unrecoverableList : '');
                if (incompleteFiles) {
