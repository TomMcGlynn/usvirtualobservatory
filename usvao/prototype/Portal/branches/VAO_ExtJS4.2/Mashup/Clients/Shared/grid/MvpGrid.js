...
Ext.define('Mvp.grid.MvpGrid', {
    ...
    requires: [
               ...
               'Mvp.data.Columns', 'Mvp.grid.BufferedCheckboxModel', 'Ext.grid.RowNumberer'],
    ...
    constructor: function (config) {
        ...
        var c = this.origAllColumns.length;
        while (c--) {
        ...
        while (c--) {
            ...
            col.on("render",
                function (val) {
                    var confCol = config.columns[val.getIndex()];
                    if (confCol) {
                        var html = confCol.tip;
                        if (html) {
    ...
    /**
     * This will return a JavaScript object containing all the data to be exported,
     * including all field metadata.
     *
     * filtercolumns - boolean: if true, only include columns that are currently visible
     * in the grid.  If false, include all non-hidden columns (this means include visible columns, but not hidden ones).
     *
     * tableOnly- boolean:  if false (default), include the entire original DataSet from the server, substituting the
     * table data we're exporting for the original first table in that data set.  If true, only return the table
     * object for the one table we're exporting.
     * Most of our DataSets only have one table, so making tableOnly false only has the effect of including all the
     * extra metadata from the original response.  This allows us to accurately reproduce the original VO Table export if that
     * is the chosen export mechanism.  tableOnly was the original behavior of this method, before we started more accurately
     * reproducing those VO Tables.
     *
     * NOTE:  The server now expects the entire DataSet, so know what you're doing if you set tableOnly to true.
     * 
     */
    getExportTable: function (filtercolumns, tableOnly) {
        var table = {};
        ...
        if (filtercolumns) {
            ...
            while (c--) {
                ...
                if (exportColumn) {
                    ...
                    //Ext.log("export column found: " + col.text + ", hidden = " + col.hidden);
                } else {
                    //Ext.log("export column not found: " + col.text);
                }
            }
        }
        ...
        // Restore any extended properties the table may have had.
        table.ExtendedProperties = store.tableExtendedProperties;
        
        // Wrap the table within the whole DataSet unless tableOnly is true.
        if (!tableOnly) {
            // The clone may not be necessary, but prevent possible sync problems if we were to do
            // a bunch of these close together.
            var ds = Ext.clone(store.extjsDataSet.data);
            
            // Put the table we just generated in the first table slot in the DataSet.
            // If we ever support multi-table DataSets in the GUI, we will need to be more
            // elaborate about this.
            ds.Tables[0] = table;
            
            // Make sure we return the whole DataSet.
            table = ds;
        }
