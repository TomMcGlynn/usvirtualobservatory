...
namespace Mashup
{
    public class Histogram
    {
        ...
        public static Dictionary<string, Object> DecimalHistogram(DataTable dt, DataColumn column, int totalBuckets, bool isDate)
        {
            ...
            {   // find the bounds of the histogram
                ...
                if (type == "DBNull")
                {
                    ...
                    {
                        ignore = isDate ? 0 : -9999.0;
                        column.ExtendedProperties["cc.ignoreValue"] = ignore;
                    }
                    else
                    {
				...
				double val = convertObjToDouble(obj, ignore);
                if (val != ignore)
                {
		...
		private static double convertObjToDouble(object obj, double ignore) {
