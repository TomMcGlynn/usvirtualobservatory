...
Ext.define('Mvpd.view.VaoPositionSearchPanel', {
    ...
    requires: ['Mvp.util.Util', 'Ext.data.Store'],
    /*
    statics: {
        radiusUnitsStore:
    ...
    */
    ...
    initComponent: function () {
        this.radiusUnitsStore = Ext.create('Ext.data.Store', {
            storeId: 'RadiusUnits',
            fields: ['displayField', 'valueField'],
            data: [
                    { displayField: 'Deg', valueField: 'deg' },
        ...
        this.radiusBox.validator = function (val) {
            if (val === "") return true;
            var isNum = /^\s*(\+|-)?((\d+(\.\d+)?)|(\.\d+))\s*$/;
            if ((Ext.Number.from(val, -1) < 0) || (!val.match(isNum))) {
    ...
    startSearch: function () {
        ...
        if ((value.length > 0) && this.radiusBox.isValid()) {
            ...
            if (radiusValue >= 0) {
                ...
                var valueWithRadius = value + ' r=' + radiusValue + unitsAbbr;
                var valueOnly = value;
                this.fireEvent('searchInitiated', [valueWithRadius, valueOnly]);
            }
        }
    },
