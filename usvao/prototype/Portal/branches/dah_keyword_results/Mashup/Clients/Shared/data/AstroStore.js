Ext.define('Mvp.data.AstroStore', {
    ...
    getFootprints: function (which) {
        ...
        if (columnName) {
            ...
            for (i = 0; i < count; i++) {
                ...
                if (fp) {
                    //fullId = record[record.idProperty]; //record.getId();
                    var fullId = record.internalId; 
                    fp._id_ = fullId;
                    footprints.push(fp);
                }
    ...
    getSiaExtentFpForRecord: function (record) {
        var fpObject = record.fpObject;
        if (fpObject === undefined) {
            fpObject = null;  // null means we tried.  undefined means we haven't tried yet.
