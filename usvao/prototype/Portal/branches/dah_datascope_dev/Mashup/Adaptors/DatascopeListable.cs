...
{
	[Serializable]
	public class DatascopeListable : IAsyncAdaptor
	{
		//
		// Log4Net Stuff
		//
	    ...
	    {	
			...
			//
			// Peel off the parameters from the URL, which will be posted.
			//
			string[] urlParts = url.Split('?');

			string sUrl = urlParts[0];
			StringBuilder sPost = new StringBuilder( Utilities.ParamString.replaceAllParams(urlParts[1], muRequest.paramss) );

			string[] services = (string[]) muRequest.data;
			if( services != null && services[0] != null )
			{
				sPost.Append ("&services=");
				string delimiter = "";
				foreach (string service in services)
				{
					sPost.Append( delimiter );
					sPost.Append( Uri.EscapeDataString(service) );
					delimiter = ",";
				}
			}
			...
			string sSubmitResponse = Utilities.Web.postWebResponseString(sUrl, sPost.ToString(), "application/x-www-form-urlencoded");
			if (!parseResponse(sSubmitResponse, out phase, out job, out numberOfHits, out resultUrls, out percentComplete, out error))
			{
			...
			if (phase == "PENDING")
			{
				...
				while (phase == "PENDING")
				{
					...
					log.Debug(tid + "DatascopeListable: Sleeping for " + npolltime + " millisecs.");
					...
					if (!parseResponse(sRunResponse, out phase, out job, out numberOfHits, out resultUrls, out percentComplete, out error))
					{
			...
			{	
				...
				/////////////////////////////////////////////////////////////////////////////////
				// For searching a list of services, data scope will finish those services (as 
				// indicated by percentComplete well before COMPLETE-ing
				/////////////////////////////////////////////////////////////////////////////////
				while ( (phase == "QUEUED" || phase == "EXECUTING") && percentComplete < 1.0 )
				{
					// Sleep for second iteration on...
					log.Debug(tid + "DatascopeListable: Sleeping for " + npolltime + " millisecs.");
					if (npolltime > 0) Thread.Sleep(npolltime); npolltime = Convert.ToInt32(polltime) * 1000;
					...
					if (parseResponse(sPollResponse, out phase, out job, out numberOfHits, out resultUrls, out percentComplete, out error))
					{
						...
						if (numberOfHits != lastNumberOfHits || numberOfHits < 0)
						{
							...
							/////////////////////////////////////////////////////////////////////////////////							
							try {
								loadResults(resultUrls, percentComplete, false, muRequest, muResponse);
								parsingExceptions=0;
							} 
							catch (Exception ex)
							{
								parsingExceptions++;
								string msg = "WARNING: DatascopeListable: Parsing Exception caught [" + parsingExceptions + "] while parsing VotTable. request = " + muRequest.ToJson();
								log.Warn(tid + msg, ex);
						...
						{
							log.Debug(tid + "DatascopeListable: Not loading results: NumberOfHits = " + numberOfHits + " LastNumberOfHits = " + lastNumberOfHits);
						}
			...
			//////////////////////////////////////////////////
			if (phase == "COMPLETED" || percentComplete >= 1.0)	
			{
				if (resultUrls != null && resultUrls.Count > 0)
				{
					loadResults(resultUrls, percentComplete, true, muRequest, muResponse);
				}
				else
				{
		...
		// parseResponse()
		////////////////////
		protected bool parseResponse(string sResponse, out string phase, out long job, out int numberOfHits, out Dictionary<string, string> resultsURLs,  out float percentComplete, out string error)
		{				
			//
			...
			log.Debug(tid + "DatascopeListable: job = " + job + " phase = " + phase);
			...
			switch (phase)
			{
					...
					if (xmllist.Count > 0)
					{
		                foreach (XmlNode xnode in xmllist)
		                {
							if (xnode.Attributes["id"] != null && xnode.Attributes["id"].Value != null)
							{
								...
								{
							case "numberofhits":
								log.Debug(tid + "DatascopeListable: NumberOfHits: " + xnode.InnerText);
								numberOfHits = Convert.ToInt32(xnode.InnerText);
								break;
								
							case "percentcomplete":
								log.Debug(tid + "DatascopeListable: percentComplete: " + xnode.InnerText);
								percentComplete = Convert.ToSingle(xnode.InnerText);
								break;
				...
				log.Debug(tid + "DataScopeListable: Unknown phase = " + phase);
				    valid = false;
					break;
			}
		...
		// loadResults()
		///////////////////
		protected void loadResults(Dictionary<string, string> resultUrls, float percentComplete, bool complete, MashupRequest muRequest, MashupResponse muResponse)
		{
			if (resultUrls != null && resultUrls.Count > 0)
			{
				foreach (KeyValuePair<string, string> resultKeyVal in resultUrls)
				{
					if (resultKeyVal.Key != null && resultKeyVal.Key.Equals("hits.xml"))
					{
						...
						if (ds != null)
						{
							log.Debug("DatascopeListable: Value of percentComplete: " + percentComplete);
							...
							// Load the DataSet into the Mashup Response
				            muResponse.load(ds, complete, percentComplete);
						}
					}
				}
			}
