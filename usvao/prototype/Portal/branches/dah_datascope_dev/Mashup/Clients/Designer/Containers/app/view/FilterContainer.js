Ext.define('Mvpc.view.FilterContainer', {
    ...
    requires: ['Mvp.util.Util', 'Mvpc.view.FacetChooserContainer', 'Mvpc.view.NumericFacetContainer', 'Ext.form.Panel', 'Ext.form.FieldSet', 'Ext.form.Checkbox', 'Ext.layout.container.Column', 'Mvpc.view.TextFacetContainer'],
    ...
    manageFilter: function (column, value, store, filters, addFilter, separator, separatorType) {   // add/remove a (key: value) pair filter to a set of filters
        ...
        if (separator) {
            if (separatorType == 'bookend') {
                ...
                value = '(^|\s+' + separator + '\s+)' + value + '($|\s+' + separator + '\s+)';
            }
        } else {
            value = '^' + value + '$';
        }
