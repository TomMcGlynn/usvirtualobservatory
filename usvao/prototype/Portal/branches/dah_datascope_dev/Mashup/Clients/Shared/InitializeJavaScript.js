...
(function () {
    ...
    AppConfig.avRenderType = getUrlParam('avRenderType', queryString, (AppConfig.mobile ? 'canvas' : 'flash')).toLowerCase();
    
    // avSurveyType tells the app which survey to use.  Legal values are:
    // toast, healpix.
    AppConfig.avSurveyType = getUrlParam('avSurveyType', queryString, 'toast').toLowerCase();
    ...
    if (AppConfig.appLoadFile) {
        ...
        document.write('<script class="include" type="text/javascript" src="' + path + '/plugins/jqplot.cursor' + (AppConfig.isDebug ? '' : '.stsci..min') + '.js"></script>'); // since STSCI changed this source we have manually minify the source
