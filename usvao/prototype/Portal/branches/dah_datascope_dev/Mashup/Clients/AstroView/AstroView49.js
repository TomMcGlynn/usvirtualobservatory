...
ASTROVIEW.AstroBasicView = function ( config )
{
    ...
    {     
        ...
        switch (rendertype)
        {
            case "webgl":
            {
                try{
                    ...
                    this.diamondRenderer.autoClear = true;
                    this.diamondRenderer.sortElements = true;
                    ...
                    this.graphicsRenderer.autoClear = true;
                    this.graphicsRenderer.sortElements = true;
                    ...
                    this.selectRenderer.autoClear = true;
                    this.selectRenderer.sortElements = true;
                    renderers.push(this.selectRenderer); 
                    
                    // Create Cross Renderer (4)
                    this.crossRenderer = new THREE.WebGLRenderer();
                    this.crossRenderer.setSize( window.innerWidth, window.innerHeight ); 
                    this.crossRenderer.autoClear = true;
                    this.crossRenderer.sortElements = true;
                    renderers.push(this.crossRenderer); 
                    
                    // Create Hover Renderer (5)
        ...
        //
        // Set Canvas Properties so they stack up in the browser as separate Image Layers
        //
        var zIndex = 1;
    ...
    this.createScenes = function(renderType)
    {
        ...
        // 
        // NOTE: Hack-A-Thon for line below:
        ...
        // Create Toast Sphere        
        this.toastSphere = new ASTROVIEW.ToastSphere(ASTROVIEW.RADIUS);
        this.toastSphere.matrixAutoUpdate = true;
        this.toastSphere.name = "ToastSphere";
        this.toastSphere.visible = (this.survey.type == "toast"); 
        this.diamondScene.add(this.toastSphere);
        
        // Create Healpix Sphere     
        this.healpixSphere = new ASTROVIEW.HealpixSphere(ASTROVIEW.RADIUS);
        this.healpixSphere.matrixAutoUpdate = true;
        this.healpixSphere.name = "HealpixSphere";
        this.healpixSphere.visible = (this.survey.type == "healpix");   
        ...
        // Cross Scene
        this.camera.parent = undefined;
        this.crossScene = (renderType == "canvas" ? new THREE.CanvasScene() : new THREE.Scene());
    ...
    {     
        ...
        if (this.toastSphere && this.toastSphere.visible)
            this.toastSphere.renderScene(this);
            
        if (this.healpixSphere && this.healpixSphere.visible)
    ...
    {   
        ...
        { 
            case "CAMERA":
            {
                // (0) Camera Changed: Clear camera existing Canvas Layer
            ...
            case "CROSS":
            {
                // (1) Cross: Render the select cross hair
            ...
            case "DIAMOND":
            {
                // (2) Diamond: Render the background diamond image(s)  
                this.diamondRenderer.render(this.diamondScene, this.camera);
            ...
            {   
                ...
                //     Delayed Rendering of Graphics Takes Place in 'GRAPHICS2' State
                if (!this.graphicsRenderer.isClear) this.clearRenderer(this.graphicsRenderer); this.graphicsRenderer.isClear = true;
                ...
                ri.state = (ri.count > 0 ? "GRAPHICS2" : (ri.all ? "SELECT" : "COMPLETE"));
                break;
            }
            case "GRAPHICS2":
            {
                ...
                //if (this.mobile) this.refreshRenderer(this.graphicsRenderer);
            ...
            case "IDLE":
            {
                // If render state queue is not empty, pop next value off
...
ASTROVIEW.Diamond = function( geometry, material )
{
    ...
    // Image Attributes must be set
    this.flipSided = geometry.flipSided; 
    this.doubleSided = geometry.doubleSided; 
...
{   
    ...
    {         
        ...
        // Clear/Fill Texture based on active zoom level for WebGL Only
        if (this.av.renderType == "webgl")
        {
            this.setOpacity(zlevel == this.geometry.zlevel);
        }
    }
    else    
    {
...
{    
    // Debug Only
    if (this.geometry.zlevel == 4 && this.geometry.tx == 15 && this.geometry.ty == 8)
    {
...
var uv00 = uv00 || new THREE.UV( 0, 0 );
var uv01 = uv01 || new THREE.UV( 0, 1 );
var uv10 = uv10 || new THREE.UV( 1, 0 );
var uv11 = uv11 || new THREE.UV( 1, 1 );
...
{
    //return this.facesInFrustum(frustum);
    return (this.sphereInFrustum(frustum) || this.verticesInFrustum(frustum));
};
...
ASTROVIEW.ToastSphere.prototype.createDiamonds = function(dgs, material)
{
    ...
    { 
        var dg = dgs[i];
        if (dg.zlevel !== 4)
            console.debug ("Oh Crap !!! zlevel = " + dg.zlevel);
...
// getFaceUV()
//////////////////////////
ASTROVIEW.HealpixGeometry.prototype.getFaceUV = function(vertices, pixel12)
{   
    var fuv;
    // Works with Dummy Images OUTSIDE Sphere
    //fuv = [uv00, uv01, uv11, uv10];  
    
    // Works with Dummy Images INSIDE Sphere
    //if (!fuv) fuv = [uv00, uv10, uv11, uv01];   
    
    // Works with Aladin Helapix Images INSIDE Sphere
    if (!fuv) fuv = [uv11, uv10, uv00, uv01];
...
// expandDiamond()
//////////////////////////
ASTROVIEW.HealpixGeometry.prototype.expandDiamond = function() 
{    
    //
    //      NOTE: Diamond.uv[4] vector is the 4 corners of the diamond on the Unit Sphere
    //
    ...
    for (var pixel=pixel4; pixel < pixel4+4; pixel++)
    {
        ...
        var dg = new ASTROVIEW.HealpixGeometry(uv, 0, 1, this.pixel12, zlevel, pixel, this.radius);
        dgs.push(dg);
    }   
...
ASTROVIEW.HealpixSphere.prototype.createSphere = function( radius )
{
    ...
    for (var pixel=0; pixel < hpi.npix; pixel++)
    {
        var corners = hpi.corners_nest(pixel, step); 
        var uv = this.cornersToVector3(corners);
        var geometry = new ASTROVIEW.HealpixGeometry(uv, 0, 1, pixel, zlevel, pixel, radius);
        dgs.push(geometry);
    }
