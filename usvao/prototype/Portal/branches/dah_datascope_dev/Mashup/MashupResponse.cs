...
{	
	...
	{	
		...
		{ 
			get { return ((this.mrd != null && this.mrd.percentComplete != null) ? this.mrd.percentComplete : float.NaN); } 
		...
		public void writeMashupResponse(MashupRequest muRequest, System.Web.HttpResponse httpResponse)
		{
			...
			{	
				...
				if (mrd.dsin != null && mrd.dsin.Tables.Count > 0)
				{
					...
					if ((muRequest.page == "1") && !(muRequest.ContainsKey("attachment") && (muRequest["attachment"].ToString() == "true")))
					//if (muRequest.page == "1")
                    {
					...
					//
					formatDataSet(muRequest, mrd.dspage, mrd.ob, out mrd.wb);
				}
		...
		protected void formatDataSet(MashupRequest muRequest, DataSet dsin, StringBuilder sb, out Workbook wb)
		{	
			wb = null;
			...
			if (dsin != null && dsin.Tables.Count > 0)
			{
				switch (muRequest.formatType)
				{
					...
					case "extjs":
						...
						Utilities.Transform.DataSetToExtjs(dsin, sb); 
						break;
