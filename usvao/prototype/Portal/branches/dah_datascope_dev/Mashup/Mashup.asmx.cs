    ...
    {	
        ...
        {	
            ...
            {	
				...
				// Add the requesting URL to the mashup request in case the adaptor wants it.
				string requestUri = Context.Request.Url.ToString();
				string requestBaseUrl = requestUri;
				// If this was a GET, then we need to trim the parameters off the URL.
				int index = requestUri.IndexOf("?");
				if (index > 0) {
					requestBaseUrl = requestUri.Substring(0, index);
				}
				muRequest.Add("requestBaseUrl", requestBaseUrl);

				//
				// If sleep parameter specified, sleep for that many milliseconds
				//
				if (muRequest.sleepAsMsecs > 0) {
					log.Info(tid + "Start sleep for " + muRequest.sleepAsMsecs + "ms");
					Thread.Sleep(muRequest.sleepAsMsecs);
					log.Info(tid + "End sleep");
				}
		...
		private void invokeAsyncAdaptor(IAsyncAdaptor a)
		{
			...
			{
				muResponse = new MashupResponse();
				//if ((muRequest.data == null) || muRequest.service.Equals("Mast.Summary"))
				if (!muRequest.service.Equals("Mashup.Table.Exporter"))
				{
