...
sub init
{
    ...
    my $q1            = "SQLVo::Queries::getLastDownTime";
    my $q2            = "SQLVo::Queries::getCurrentTime";
    my $q3            = "SQLVo::Queries::getCurrentHealthService";
    
    my $res            = &$q1($con);    
    my @a = @$res; 
    $self->{_lastdown} = $res;
    
    $self->{con}->{show}  = SQLVo::Queries::getServiceName($con);
...
sub getUpTime
{
	...
	my $tup            =  SQLVo::Queries::getLastTimeUp($self->{con});	
	...
	{   
	    ...
	    $tdown    = SQLVo::Queries::getEarliestTime($self->{con});
	    $uptime   = ['always up','',''];       
	}
	else
	{
