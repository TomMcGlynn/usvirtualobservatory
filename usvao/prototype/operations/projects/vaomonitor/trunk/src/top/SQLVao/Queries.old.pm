...
@EXPORT = qw (getTestUrls getServicenamesTestid getData getServiceandTest getcurrentHealth 
	      ...
	      getPenultimateStats getPenultimateStats1 get_email_addresses getErrorMessage 
...
sub getTestUrls
{
    ...
    my $time;
    my $sql   = qq( select  url, response_type,class,testname,notAvailable, params  from  Tests  where testid =  '$testid');
    my $sth  = $dbh->prepare($sql);
    $sth->execute();
    my $arrayref = $sth->fetchall_arrayref();
...
sub getRangeStatsPassing 
{
    ...
    Testhistory t where monitorstatus  = 'pass' and s.serviceId = t.serviceId and time >= '$start'   
		and  time <= '$end'   group by serviceId};
...
sub getHeasarcStats
{
    ...
    my $sql = qq {select name, count(g.monitorstatus)  from Services s,Tests t,Testhistory g  
		      ...
		      and s.serviceId = t.serviceId and g.time >= '$start' 
		      and g.time <= '$end' and g.testid = t.testid group by t.testId};
    my $sth = $con->{dbh}->prepare($sql); 
    $sth->execute();
    print "JJ: $sql\n";
    my $arrayref = $sth->fetchall_arrayref();
...
sub getRangeStatsFailing 
{
    ...
    my $sql = qq {select s.name,s.homeurl,t.serviceId, count(t.monitorstatus)   from  Services s,   
    Testhistory t where monitorstatus  in ('fail', 'abort') and s.serviceId = t.serviceId and time >= '$start' 
	and  time <= '$end'   group by serviceId};
    my $sth = $con->{dbh}->prepare($sql); 
    $sth->execute();
...
sub getStatsPassing
{
    ...
    my $sql = qq {  select s.name, t.serviceId, count(t.monitorstatus)  from Services s,  Testhistory  t  
			where  monitorstatus = 'pass' and s.serviceId = t.serviceId 
			and s.name = '$name' and time > now() - interval $con->{range}  week;
	       };
    my $sth = $con->{dbh}->prepare($sql); 
    $sth->execute();
...
sub getStatsFailing
{
    my ($con) = @_;
    my $name  = $con->{name}; 
    my $sql = qq{select s.name, s.homeurl, t.serviceId, count(t.monitorstatus)   from  Services s,   Testhistory t  
		     where monitorstatus in ('fail','abort') and s.serviceId = t.serviceId 
		     and s.name = '$name' and  time > now() - interval $con->{range}  week};
...
sub getStatsAllPassing
{

    my ($con) = @_;
    my $sql = qq {  select s.name, s.homeurl, t.serviceId, count(t.monitorstatus)  from Services s,  Testhistory  t  
			where  monitorstatus = 'pass' and s.serviceId = t.serviceId 
			and time > now() - interval $con->{range}  week group by  serviceId;
	       };
    my $sth = $con->{dbh}->prepare($sql); 
    $sth->execute();
...
sub getServiceName
{
    ...
    my $sql =  qq(select name from Services where serviceId = '$con->{sid}');
...
sub getEarliestTime
{
    ...
    my $sql =  qq(select unix_timestamp(time)  from Testhistory t where  serviceId= '$con->{sid}' order by time asc  limit 1 );
...
sub getcurrentHealthService
{
    ...
    my $sql = qq( select m.name as name,m.serviceId as serviceId,  m.testid as testid,m.testname, m.type as type,
		  Testhistory.monitorstatus as status, m.bigdate as time,
		  
		
		  Testhistory.runid as runid 
		  from 
		  (  select t.serviceId, max(t.time) as bigdate, t.testid,s.type, s.name,g.testname
		     from Testhistory t, Tests g, Services s where 
		     s.serviceId = t.serviceId and g.serviceId = t.serviceId and g.testid = t.testid group by 
		     t.testid 
		  ) 
		  as m 
		  join Testhistory on Testhistory.serviceId = m.serviceId 
		  and Testhistory.time = m.bigdate and name  = '$name');
    my $sth = $con->{'dbh'}->prepare($sql);
    $sth->execute();
    my $arrayref = $sth->fetchall_arrayref();
    return $arrayref;
...
sub getnewCurrentHealthService
{
    my ($con) = @_;
    my $pid = $$;
    my $hash = {};
...
sub getOldTables
{
    ...
    my $sql = qq ( 
		    ...
		    Tests a  where s.name = '$name' and a.deleted is null and s.serviceId = t.serviceId and t.testid = a.testid order by 
...
{    
    ...
    $sth2->execute() || die "cannot run statement";
...
sub getLastDownTime
{
    ...
    my $sql = qq(select name, monitorstatus,unix_timestamp(time),runid  from Testhistory t,Services s, Tests a  where  
		 a.deleted is null and s.serviceId = t.serviceId and a.testid = t.testid   and t.serviceId = $sid
...
sub getPenultimateStatsR
{
    my ($con,$service) = @_;
    my $sql = qq(select t.serviceId, t.runid , t.monitorstatus,t.time as bigdate,  t.testid,s.type, s.name,
		 g.testname,g.url   from Testhistory t, Tests g,  Services s where   s.serviceId = t.serviceId and 
		 g.serviceId =  t.serviceId and g.testid = t.testid and s.name = '$service'  and t.validtest = 'T'
		 and unix_timestamp(t.time)  <   (unix_timestamp(now()) -3600 )  and 
		 unix_timestamp(t.time) > (unix_timestamp(now()) -7800) group by testid);
...
sub getPenultimateStats1
{
    my ($con,$service) = @_;
    
    #my $s = qq(delete from J);
    #my $sh = $con->{dbh}->prepare($s);
    #$sh->execute();
    
    my $sql =  qq(select J.testid,max(J.bigdate), Testhistory.monitorstatus  from (select 
	 m.runid,m.testid,m.monitorstatus, m.bigdate from  (select t.serviceId, 
         t.runid , t.monitorstatus, unix_timestamp(t.time)   as bigdate,  
         t.testid,s.type, s.name,                   g.testname,g.url   from 
         Testhistory t, Tests g,  Services s where    s.serviceId = t.serviceId 
         and  g.serviceId =   t.serviceId and g.testid = 
	 t.testid and s.name = $service and  
         unix_timestamp(t.time) <  (unix_timestamp(now())  -1600) and 
         unix_timestamp(t.time) > ( unix_timestamp(now()) -7800)) as m) as J join 
         Testhistory  on unix_timestamp(Testhistory.time) = J.bigdate group by 
		J.testid);
...
sub getLastTimeUp
{
    ...
    my $sql1 =  qq(  insert into runids_$pid (select max(runid)   from Testhistory t,Tests  a 
		     where t.serviceId =   $con->{sid} and monitorstatus = 'pass' 
