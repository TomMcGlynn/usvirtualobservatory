...
We release the  1.0 version of Iris, the VAO application for
...
data from file.  Iris can display SEDs, allow the user to select
...
by STScI, provides a GUI for reading, editing, and displaying SEDs, as
...
extensive database of extragalactic SEDs.  We have named this combined
...
% tar -zxf iris-1.0-unix-<arch>.tar.gz
...
% tar -zxf iris-1.0-macosx-<arch>.tar.gz
...
The components of Iris (Specview and Sherpa) are now unified in the
1.0 release.  Both applications run simultaneously and
...
to the Iris 1.0 directory and double-click on the "Iris"
executable.
...
Running SED Importer 1.0
========================

The SED importer tool is now bundled with Iris.

This component of the VAO Iris SED Tool is called "SedImporter", and
allows users to convert their data, stored in non-standard formats, to
IVOA standard documents so that they can be loaded by the Iris Tool.
...
Typically, SedImporter will be started in GUI mode, to build a SED and
write out a compliant SED file.  It is also possible to save a "setup
...
Begin using SedImporter in GUI mode, to build or retrieve SEDs, by
typing the following in a new terminal window:
...
Open a folder manager window on Linux or the Finder on Mac.  Navigate
to the Iris 1.0 directory and double-click on the "SedImporter"
executable.
...
These files can be used to test SedImporter.  SedImporter can be used
to write out a compliant XML file that can then be read into Iris.

To run the SED importer in batch mode from the command line, provide
...
% <basedir>/SedImporter setup.ini outputfile.vot vot
...
The Iris 1.0 package includes sample files in the "examples"
...
% <basedir>/Iris \
    "http://vaobeta.ipac.caltech.edu/services/accessSED\?REQUEST=getData\&TARGETNAME=3c273"

or, retrieve a SED from NED with SedImporter, as noted above in the
section "Running SedImporter 1.0";
...
above, "Running Iris 1.0".

In this section we will illustrate Iris usage with the SED of 3C 273.
To read the SED into Iris, go to the Specview menu and click the
"File->Read" option in the File menu.  Select the "examples"
directory, and then select "3c273.xml".  Then click the "Open" button.

At this point, the SED of 3C 273 will automatically be plotted, in
...
Specview will always do the fit in a set of standard units, of
...
button to open the component dialog box.
