...
sub run_wesix_test
{
    ...
    #file to upload
    my $heasarc_file       =  '/www/htdocs/vo/vaomonitor/data/test_wesix.fits';

    #get start time
    my $start_time        = time();

    #get wesix home page at:  "http://nvogre.phyast.pitt.edu:8080/wesix/";
    my $wesix_home        = $url;
    my $wesixurl          = URI->new("$wesix_home");
    my $ua                = LWP::UserAgent->new;   
    my $response          = $ua->get($wesixurl);
    my @wesix_home_forms  = HTML::Form->parse($response);
    my $wesix_form        = shift @wesix_home_forms;
