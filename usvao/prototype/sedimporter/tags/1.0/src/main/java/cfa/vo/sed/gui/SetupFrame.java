...
public final class SetupFrame extends ConfirmJInternalFrame {
    ...
    private List<NewSedFrame> sedFrameList;

    public List<NewSedFrame> getSedFrameList() {
        return sedFrameList;
    }
    ...
    public void refresh() {
        ...
        for(NewSedFrame fr : sedFrameList) {
            sb.append(" ").append(fr.getSedID());
        }
        setSedID(sb.toString());
    }

    private boolean constructed = false;

    public SetupFrame(String name, ISegmentMetadata metadata, NewSedFrame sedFrame, String fileLocation, IFileFormat format, Integer segmentNumber) throws MalformedURLException {
        this(name, metadata, new ArrayList(Arrays.asList(new NewSedFrame[]{sedFrame})), fileLocation, format, segmentNumber);
    }

    /** Creates new form NewJInternalFrame*/
    public SetupFrame(String name, ISegmentMetadata metadata, List<NewSedFrame> sedFrameList, String fileLocation, IFileFormat format, Integer segmentNumber) throws MalformedURLException {
        super(name);
        initComponents();

        this.sedFrameList = sedFrameList;

        SetupBean c = new SetupBean();
        c.setFileLocation(fileLocation);
        c.setFormatName(format.getName());
        c.setPositionInFile(segmentNumber);
        ...
        setRa(sedFrameList.get(0).getRa());
        setDec(sedFrameList.get(0).getDec());
        setTargetName(sedFrameList.get(0).getTargetName());
    ...
    private void importSegment(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_importSegment
        ...
        try {
            ...
            for(NewSedFrame frame : sedFrameList) {
                SegmentWrapper sw = new SegmentWrapper(segment, this);
                frame.addSegment(sw);
