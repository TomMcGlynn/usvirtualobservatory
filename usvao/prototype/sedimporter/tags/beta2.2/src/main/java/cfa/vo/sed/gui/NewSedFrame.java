...
public final class NewSedFrame extends ConfirmJInternalFrame {
    ...
    private void addSegment(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_addSegment
        LoadSegmentDialog dia = new LoadSegmentDialog(MainView.getInstance(), true);
        ...
        if(dia.isChosen()) {
            ...
            if(dia.isSetURL()) {
                ...
                try {//TODO Multiple segments
                    ISegmentMetadata md = dia.getFormat().getFilter().getMetadata(dia.getURL()).get(0);
                    SetupFrame cf = new SetupFrame("New Segment", md, this, dia.getUrlS(), dia.getFormat());
                    desktopPane.add(cf);
                    cf.setVisible(true);
                } catch (Exception ex) {
            ...
            } else if(dia.hasSegments()) {
                List<Segment> segList = dia.getSegments();
                for(Segment segment : segList) {
                    if(!segment.createTarget().createPos().isSetValue()) {
                        ...
                        segment.getTarget().getPos().setValue(new DoubleParam[]{new DoubleParam(raD), new DoubleParam(decD)});
                    }
                }
                addSegments(segList);
            }
