...
public class SegmentImporter {
    ...
    public static List<Segment> getSegments(ISetup conf) throws SedInconsistentException, MalformedURLException, IOException, FilterException {
        // The number of segments depends on the type of error and/or on the value of the oneSegmentPerPoint variable
        ...
        IFileFormat format = NativeFileFormat.valueOf(conf.getFormatName());
        ...
        switch(errorType) {
            ...
            case SymmetricParameter:
                ...
                List<ISegmentMetadata> meta = format.getFilter().getMetadata(fileLoc);

                List<ISegmentParameter> params = meta.get(0).getParameters();
            ...
            case AsymmetricParameter:
                ...
                meta = format.getFilter().getMetadata(fileLoc);
                params = meta.get(0).getParameters();
            ...
            case SymmetricColumn:
                ...
                Number[] errNumbers = (Number[]) format.getFilter().getData(fileLoc, 1, conf.getSymmetricErrorColumnNumber());
            ...
            case AsymmetricColumn:
                ...
                Number[] lowErrNumbers = (Number[]) format.getFilter().getData(fileLoc, 1, conf.getLowerErrorColumnNumber());
                Number[] hiErrNumbers = (Number[]) format.getFilter().getData(fileLoc, 1, conf.getUpperErrorColumnNumber());
    ...
    public static List<ISegmentMetadata> getSegmentsMetadata(URL file, IFileFormat format) throws IOException, FilterException {
        ...
        IFilter filter = format.getFilter();

        return filter.getMetadata(file);

    }

    private static int fillSegment(Segment segment, ISetup conf) throws MalformedURLException, IOException, FilterException {
        URL fileLoc = new URL(conf.getFileLocation());
        IFileFormat format = NativeFileFormat.valueOf(conf.getFormatName().toUpperCase());
        ...
        Number[] xnumbers = (Number[]) format.getFilter().getData(fileLoc, 1, conf.getXAxisColumnNumber());
        Number[] ynumbers = (Number[]) format.getFilter().getData(fileLoc, 1, conf.getYAxisColumnNumber());
