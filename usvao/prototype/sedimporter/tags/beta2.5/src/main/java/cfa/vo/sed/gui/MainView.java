...
import cfa.vo.interop.SAMPStatusListener;
...
public class MainView extends JFrame {
    ...
    public static boolean SAMP_ENABLED = System.getProperty("samp", "true").toLowerCase().equals("false") ? false : true;
    ...
    public MainView() {
        ...
        if(SAMP_ENABLED) {
            sampController = new SAMPController("SedImporter", "An SED builder from the Virtual Astronomical Observatory", getClass().getResource("/iris_button_tiny.png").toString());
            sampController.addConnectionListener(new SampStatusListener());
            try {
                sampController.addMessageHandler(new TableHandler());
            } catch (SampException ex) {

            }
        }
        
        sampIcon.setVisible(SAMP_ENABLED);
...
//                OSXAdapter.setFileHandler(this, getClass().getDeclaredMethod("loadImageFile", new Class[] { String.class }));
    ...
    private void initComponents() {
        ...
        sampIcon.setBounds(50, 530, 210, 120);
        ...
        jLabel1.setBounds(600, 460, 360, -1);
        ...
        jMenuItem2.setText("Load a Setup file");
    ...
    private void showIrisHelp(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_showIrisHelp
        try {
            showLink(new URL("http://cxc.cfa.harvard.edu/csc/temp/sed"));
...
//        }
    ...
    private void showImporterHelp(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_showImporterHelp
        try {
            showLink(new URL("http://cxc.cfa.harvard.edu/csc/temp/sed/threads/importer"));
    ...
    */
    ...
    private class SampStatusListener implements SAMPStatusListener {
    ...
    private class TableHandler extends AbstractMessageHandler {
        ...
        public Map processCall(HubConnection hc, String string, Message msg) {
            try {
                ...
                if(name.isEmpty())
                    name = "SAMPTable";
                NewSedFrame frame = new NewSedFrame(name);
                desktopPane.add(frame);
                frame.setVisible(true);
...
//                        for(ValidationError err : validErrors) {
//                            System.out.println(err.getError());
//                        }

//                        ISegmentMetadata metadata = null;
//
//                        try {
//                            //Try to infer metatada from the file header
//                            metadata = NativeFileFormat.VOTABLE.getFilter().getMetadata(new URL("file://" + f.getAbsolutePath())).get(0);
//                        } catch (FilterException ex) {
//                            Logger.getLogger(MainView.class.getName()).log(Level.SEVERE, null, ex);
//                        }
//
//                        if(segment.createTarget().isSetName())
//                            targetName = segment.getTarget().getName().getValue();
//                        else
//                            targetName = (String) metadata.getParameterByUtype("Spectrum.Target.Name").getValue();
//
//                        if(segment.createCuration().isSetPublisher())
//                            publisher = segment.getCuration().getPublisher().getValue();
//                        else
//                            publisher = (String) metadata.getParameterByUtype("Spectrum.Curation.Publisher").getValue();
//
//                        if(segment.createTarget().isSetPos()) {
//                            DoubleParam[] coordsParam = segment.getTarget().getPos().getValue();
//                            coords = new Double[] {(Double)coordsParam[0].getCastValue(), (Double)coordsParam[1].getCastValue()};
//                        } else {
//                            ISegmentParameter param = metadata.getParameterByUtype("Spectrum.Target.Pos");
//                            if(param instanceof NullSegmentParameter) {
//                                param = metadata.getParameterByUtype("Spectrum.Char.SpatialAxis.Coverage.Location.Value");
//                            }
//
//                            if(param.getValue()!=null) {
//                                    if(param.getContentClass().isAssignableFrom(Number.class)) {
//                                        coords = (Number[]) param.getValue();
//                                    } else if (param.getContentClass().equals(String.class)) {
//                                        coords = new Double[2];
//                                        String[] coordsS = ((String) param.getValue()).split(" ");
//                                        coords[0] = Double.valueOf(coordsS[0]);
//                                        coords[1] = Double.valueOf(coordsS[1]);
//                                    }
//                            }
//                        }
//
...
//                    }
//
//                    TargetInfoDialog dialog = new TargetInfoDialog(MainView.this, true);
//                    dialog.setLocationRelativeTo(null);
//
//                    if(targetName!=null) {
//                            dialog.setTargetName(targetName);
//                            frame.setSedID(targetName);
//                    }
//
//                    dialog.setRaS(coords==null? "" : coords[0].toString());
//                    dialog.setDecS(coords==null? "" : coords[1].toString());
//
//                    dialog.setPublisher(publisher);
//
//                    dialog.setVisible(true);
//
//                    confirm = dialog.isConfirmed();
//
//                    if(confirm) {
//                        PositionParam pos = new PositionParam();
//                        pos.setValue(new DoubleParam[]{new DoubleParam(dialog.getRa()), new DoubleParam(dialog.getDec())});
//                        segment.createTarget().setPos(pos);
//                        segment.createTarget().setName(new TextParam(targetName));
//                        if(!segment.createChar().isSetSpatialAxis()) {
//                            segment.createChar().createSpatialAxis().createCoverage().createLocation().setValue(pos.getValue());
//                            segment.createChar().createSpatialAxis().createCoverage().createBounds().setExtent(new DoubleParam(Double.NaN));
//                        }
//                        if(!segment.createChar().isSetSpectralAxis()) {
//
//                        }
//                        TextParam txt = new TextParam();
//                        txt.setValue(dialog.getPublisher());
//                        segment.createCuration().setPublisher(txt);
//                    }
//
//                    if(confirm)
//                        frame.addSegment(sed.getSegment(i));
//                    else
//                        frame.dispose();
                }
                ...
                try {
                    ...
                    desktopPane.add(frame);
                    frame.setVisible(true);
                    frame.addSegment(new URL((String) msg.getParam("url")), NativeFileFormat.VOTABLE);
                } catch (Exception ex1) {
