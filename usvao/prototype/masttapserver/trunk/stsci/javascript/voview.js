...
//				this.title.innerHTML += this.exporter;
	...
	this.setWindowTitle = function() {
		...
		if (this.queryTitle) {
			window.document.title = "VOTable Results (" + this.queryTitle + ")";
		} else if (this.url) {
		window.document.title = "VOTable Results (" + this.url + ")";
		} else if (this.filename) {
		window.document.title = "VOTable Results (" + this.filename + ")";
		} else {
		window.document.title = "VOTable Results";
		}
	};
	...
	this.loadData = function() {
	    ...
	    if (this.stream != null && this.stream.value.length > 0) {
	        ...
	        var fullurl = "http://dower.stsci.edu/tapv1/TapService.aspx/sync?REQUEST=DOQUERY&LANG=ADQL&QUERY=" +
...
//		if (me.tagRowsImported && !me.tagRowsAttributeAdded) me.tagRowsAttribute();
   ...
   this.view2xslt = {
	...
	this.tagRowsXSLloader.makeRequest("./xsl/tagRows.xsl");
