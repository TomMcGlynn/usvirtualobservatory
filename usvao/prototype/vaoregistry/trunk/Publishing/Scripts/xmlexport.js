...
function setResourceType(isClone) {
    ...
    var val = selection;
    if (selection == 'cs:ConeSearch' || selection == 'sia:SimpleImageAcces' || selection == 'ssa:SimpleSpectralAccess')
    ...
    if (val == 'vs:CatalogService') {
        ...
        if (isClone) {
            formPages = [];
            setupForms(app, resourceDOM, newResource, isClone);
        }
    }
...
function setDOMAltPublisher(newVal, oldVal, resourceDOM) {
    setCurationTag(resourceDOM);
    if (curation != undefined && newVal != oldVal) {
            curation.getElementsByTagName('publisher')[0].setAttribute('ivo-id', null);
