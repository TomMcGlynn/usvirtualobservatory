...
namespace Publishing
{
    public partial class ValidationManagement : System.Web.UI.Page
    {
        ...
        string strViewResourceURL = "http://nvo.stsci.edu/vor10/getRecord.aspx?id=";
        ...
        private validationStatus GenerateDynamicFormData()
        {
            ...
            try
            {
                ...
                DropDownStatus.SelectedValue = list[0].Attributes["status"].Value;

                list = ((XmlDocument)Session["docResource"]).GetElementsByTagName("validationLevel");
                PanelMain.Controls.Add(FormBuilder.CreateValidationDropDown("DropDown_validationStatus", list[0].InnerText));
            ...
            for( int i = 0; i < list.Count; ++i)
            {
                ...
                foreach (XmlNode child in node.ChildNodes)
                {
                    if (child.Name.ToLower() == "validationlevel")
                    {
                        ...
                        PanelMain.Controls.Add(FormBuilder.CreateValidationDropDown("capability" + i, status));
                    }
                }
            }
            if (list.Count == 0)
        ...
        private validationStatus SaveDocToRegistry()
        {
            ...
            try
            {
                ...
                {
                    status += ResourceManagement.PublishXmlResource((XmlDocument)Session["docResource"], false, strQueryIdentifier);
                }
            }
            catch (Exception ex)
            {
        ...
        {
            DropDownList dlist = (DropDownList)PanelMain.FindControl("DropDown_validationStatus");
