<%@ Page language="c#" Codebehind="SearchRegistry.aspx.cs" EnableSessionState=true AutoEventWireup="false" Inherits="registry.QueryRegistry" %>
...
<!-- main table gets closed in SkyFooter.aspx -->
			...
			<table  border="1"  bordercolor="#6BA5D7" cellspacing="2" cellpadding="2">
				<tr >
					...
					<td >
						...
						href="QueryRegistry.aspx?startres=-1&<%=advanced?"&advanced=true":""%>&sql=<%=Server.UrlEncode(predicate)%> and ResourceType like '<%=dr[0]%>'">
						<%=dr[0]%><br>(<%=dr[1]%>)
						</a>
					</td>
					<%}%>
			...
			<tr><td valign="top"><table >
				...
				<%  String st = (String)dr["ResourceType"];
				...
				<td class="left"> <a href="UpdateRegistry.aspx?SearchIdentifier=<%=Server.UrlEncode((string)dr["Identifier"])%>&ro=t"><%=dr["Title"]%><b>(<%=dr["ShortName"]%>)</b></a>
						<br><font size="-2"><%=dr["description"]%></font></td>
				<td class="left"> <font size="-2"><%= (((string)dr["Subject"]).IndexOf(' ') <0)? ((string)dr["Subject"]).Replace(",",", ") : dr["Subject"] %><font size="-1"></td>
				<td class="left"> <font size="-2"><%=dr["ResourceType"]%></font></td>
