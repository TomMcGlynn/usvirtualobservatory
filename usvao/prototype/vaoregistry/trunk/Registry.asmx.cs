...
namespace registry
{
	...
	public class Registry : System.Web.Services.WebService
	{
        ...
        {
            string cmd = SQLHelper.createXMLResourceSelect(predicate, false);
        ...
        [WebMethod(Description = "Returns VOResources with status=1 and 3: Input WHERE predicate for SQL Query")]
        public System.Xml.XmlDocument[] QueryRIResourceXMLDocWithDeleted(string predicate)
        {
            string cmd = SQLHelper.createXMLResourceSelect(predicate, true);
            return SQLQueryRI10ResourceXMLDoc(cmd);
        }
        ...
        public System.Xml.XmlDocument[] QueryCapabilityResourceXMLDoc(string predicate, string capability)
        {
            ...
            string cmd = SQLHelper.createXMLCapabilityResourceSelect(predicate, capability);
            return SQLQueryRI10ResourceXMLDoc(cmd);
        }

        [WebMethod(Description = "Returns VOResources with status=1, filtered by capability Input capability substring, WHERE predicate for SQL Query")]
        public System.Xml.XmlDocument[] QueryAdvancedResourceXMLDoc(string predicate, string capability, string title, string shortname, string identifier, string publisher, string subject, string waveband)
        {
            if (predicate.Length == 0 && capability.Length == 0 && title.Length == 0 && shortname.Length == 0 &&
                identifier.Length == 0 && publisher.Length == 0 && subject.Length == 0 && waveband.Length == 0)
                return new System.Xml.XmlDocument[0];

            string cmd = SQLHelper.createXMLAdvancedResourceSelect(predicate, capability, title,  shortname, identifier, publisher, subject, waveband);
            return SQLQueryRI10ResourceXMLDoc(cmd);
        }

        [WebMethod(Description = "Returns VOResources with status=1, filtered by capability Input capability substring, WHERE predicate for SQL Query")]
        public ivoa.altVOTable.VOTABLE QueryAdvancedResourceCache(string predicate, string capability, string title, string shortname, string identifier, 
                                                                  string publisher, string subject, string waveband)
        {
            if (predicate.Length == 0 && capability.Length == 0 && title.Length == 0 && shortname.Length == 0 && identifier.Length == 0 && 
                publisher.Length == 0 && subject.Length == 0 && waveband.Length == 0)
                return new ivoa.altVOTable.VOTABLE();

            string cmd = SQLHelper.createAdvancedResourceCacheSelect(predicate, capability, title, shortname, identifier, publisher, subject, waveband);
            return BuildVOTableResults(cmd, 1);
        }
        ...
        {
            string cmd = SQLHelper.createXMLCapBandResourceSelect(predicate, capability, band);
            return SQLQueryRI10ResourceXMLDoc(cmd);
        }
        ...
        {
            string cmd = SQLHelper.createFullXMLResourceSelect(predicate);
            return SQLQueryRI10Resource(cmd);
        }
        ...
        public ivoa.net.vr1_0.Resource[] SQLQueryVOR10Resource(string cmd)
        {
            SqlConnection conn = null;
            ivoa.net.vr1_0.Resource[] sr = null;
            try
            {
                ...
                sr = new ivoa.net.vr1_0.Resource[ncount];
                for (int i = 0; i < ncount; i++)
                {
                    DataRow dr = ds.Tables[0].Rows[i];
                    sr[i] = (ivoa.net.vr1_0.Resource)ObjectMaker.CreateVOR10Resource(dr);
                }
            }
            finally
            {
        ...
        public ivoa.net.ri1_0.server.Resource[] SQLQueryRI10Resource(string cmd)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < ncount; i++)
                {
                    ...
                    try
                    {
                        res = (ivoa.net.ri1_0.server.Resource)ObjectMaker.CreateRI10Resource(dr);
                    }
                    catch (Exception e)
                    {
                        if (errLog != null)
        ...
        public ivoa.altVOTable.VOTABLE QueryCapabilityPredicateResourceCache(string predicate, string capability, int option)
        {
            string cmd = SQLHelper.createCapabilityPredicateSelectUsingCache(predicate, capability, option);
        ...
        public ivoa.altVOTable.VOTABLE SqlQueryRI10ResourceCache(string keywords, bool andKeys, int option)
        {
            ...
            if (option == 1)
                cmd = SQLHelper.getGetResourceCacheCmd();
            else
		...
		public oai_dc.oai_dcType[] SQLQueryOAIDC(string cmd)
		{
			...
			{	
				...
				{
					DataRow dr = ds.Tables[0].Rows[i];
					odc[i] = (oai_dc.oai_dcType)ObjectMaker.CreateOAIDC(dr);	
				}
			}
        ...
        [WebMethod (Description="Returns custom simplified Resources: Input WHERE predicate for SQL Query")]
...
//		[WebMethod (Description="Returns custom simplified Resources: Input Select Statement should return Resource Columns e.g. \n Select r.* from Resource r where maxRecords > 200")]
		public DBResource[] SQLQuery(string cmd)
		{
			SqlConnection conn = null;
			DBResource[] sr = null;
			try
			{	
				...
				sqlDA.Fill(ds);
				int ncount = ds.Tables[0].Rows.Count;
				sr = new DBResource[ncount];
				for (int i=0;i<ncount;i++)
				{
					DataRow dr = ds.Tables[0].Rows[i];
					sr[i] = (DBResource)ObjectMaker.CreateResource(dr);
					// fill in other parts from cone etc ..
				/*	if (sr[i].GetType() == typeof(ServiceCone)) 
					{
						ObjectMaker.GetData((ServiceCone)sr[i],dr);
					}
					if (sr[i].GetType() == typeof(ServiceSimpleImageAccess)) 
					{
						ObjectMaker.GetData((ServiceSimpleImageAccess)sr[i],dr);		
					}
					if (sr[i].GetType() == typeof(ServiceSkyNode)) 
					{
						ObjectMaker.GetData((ServiceSkyNode)sr[i],dr);
					}

					SqlCommand relcmd = SQLHelper.getResRelationsCmd(conn);
					relcmd.Parameters["@dbid"].Value=sr[i].pkey;
					SqlDataAdapter sqlDArel = new SqlDataAdapter(relcmd);
					DataSet dsRel= new DataSet();
					sqlDArel.Fill(dsRel);
					int relCount = dsRel.Tables[0].Rows.Count;
/*					sr[i].resourceRelations  = new ResourceRelation[relCount];

					if (relCount > 0)
					{

						for (int rr=0;rr<relCount;rr++)
						{
							DataRow drr = dsRel.Tables[0].Rows[rr];
							sr[i].resourceRelations[rr]=new ResourceRelation();
							ObjectMaker.GetData(sr[i].resourceRelations[rr],drr);
						}
					}
					SqlCommand intcmd = SQLHelper.getResInterfacesCmd(conn);
					intcmd.Parameters["@dbid"].Value=sr[i].dbid;
					SqlDataAdapter sqlDAIntf = new SqlDataAdapter(intcmd);
					DataSet dsInt= new DataSet();
					sqlDAIntf.Fill(dsInt);
					int interfaceCount = dsInt.Tables[0].Rows.Count;
					sr[i].resourceInterfaces = new ResourceInterface[interfaceCount];

					if (interfaceCount > 0)
					{
						for (int rr=0;rr<interfaceCount;rr++)
						{
							DataRow dri = dsInt.Tables[0].Rows[rr];
							sr[i].resourceInterfaces[rr]=new ResourceInterface();
							ObjectMaker.GetData(sr[i].resourceInterfaces[rr],dri);
							//Get Params if exist
							SqlCommand intParCmd = SQLHelper.getInterfaceParamsCmd(conn);
							intParCmd.Parameters["@interfaceNum"].Value=sr[i].resourceInterfaces[rr].interfaceNum;
							intParCmd.Parameters["@dbid"].Value=sr[i].dbid;
							SqlDataAdapter sqlDAIntp = new SqlDataAdapter(intParCmd);
							DataSet dsParams = new DataSet();
							sqlDAIntp.Fill(dsParams);
							int paramsCount = dsParams.Tables[0].Rows.Count;
							sr[i].resourceInterfaces[rr].interfaceParams = new InterfaceParam[paramsCount];
							if (paramsCount > 0)
							{
							}
						}
...
/*
		[WebMethod (Description="Returns SimpleResource (backward compatibility for DATASCOPE): Input WHERE predicate for SQL Query like e.g. \n maxSearchRadius > 1 and ResourceType like 'CONE'")]
		public SimpleResource[] QueryRegistry(string predicate)
		{
			SqlConnection conn = null;
			SimpleResource[] sr = null;
			predicate = predicate.ToUpper().Replace("SERVICETYPE","RESOURCETYPE");
			...
			{	
				...
				sqlDA.Fill(ds);
				int ncount = ds.Tables[0].Rows.Count;
				sr = new SimpleResource[ncount];
				for (int i=0;i<ncount;i++)
				{
					sr[i] = CreateSimpleResource(ds.Tables[0].Rows[i], conn);
				}
			}
		...
		protected SimpleResource CreateSimpleResource(DataRow dr, SqlConnection conn) 
		{
			long dbid = (long)dr["dbid"];
			SimpleResource res = new SimpleResource();
			res.Title		= 	(string)dr["Title"];
			res.Publisher	=	(string)dr["CurationPublisherName"];
			try
			{
			...
			try
			{
				res.Creator		=	(string)dr["CurationCreatorName"];
			}
			catch(Exception ) {}
			try
			{
			...
			try
			{
				res.Contributor	=	(string)dr["CurationContributor"];
			}
			catch(Exception ) {}
			try
			{
				res.Date		=	 DateTime.Parse((string)dr["CurationDate"]);
			}
			catch(Exception ) {}
			try
			{
				res.Version		=	(string)dr["CurationVersion"];
			}
			catch(Exception ) {}
			try
			{
			...
			try
			{
				res.ContactName		=	(string)dr["CurationContactName"];
			}
			catch(Exception ) {}
			try
			{
				res.ContactEmail		=	(string)dr["CurationContactEmail"];
			}
			catch(Exception ) {}
			try
			{
			...
			try
			{
				res.EntrySize	=	(double)dr["CoverageRegionOfRegard"];
			}
			catch(Exception ) {}
			...
			try
			{
				res.ServiceType	=	(string)dr["ResourceType"];
			}
			catch (Exception ) {}

			if (res.ServiceType.ToUpper().StartsWith("CONE") || res.ServiceType.ToUpper().StartsWith("SIA")) 
			{
				try 
				{
					res.MaxRecords	=(int)	(long)dr["MaxRecords"];
				} 
				catch (Exception ) {};
			}
			if (res.ServiceType.ToUpper().StartsWith("CONE"))
			{
				try 
				{
					res.MaxSR		=	(double)dr["MaxSearchRadius"];
				} 
		...
		/// <summary>
		/// (Description="returns cvs verions of classes in this service")
		/// </summary>
		[WebMethod (Description="returns cvs verions of classes in this service")]
		public string[] Revisions()
		{
			string[] ret = {"SimpleResource :"+SimpleResource.Revision,
    ...
    {
        private static string location = Properties.Settings.Default.log_location;
