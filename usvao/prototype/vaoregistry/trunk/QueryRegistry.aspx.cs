...
namespace registry
{
	...
	/// </summary>
	public class QueryRegistry : System.Web.UI.Page
	{
...
*/
        protected DataSet doGroups(RegistryAdmin reg, string predicate)
        {
            string mpredicate = predicate.ToUpper().Replace("RESOURCETYPE", " r.resourceType");
            mpredicate = mpredicate.Replace("CONTAINS (*,", "CONTAINS (r.*,");
            return reg.DSQuery("select r.tag, count(r.identifier), r.[content/description] from resource r where r.[@status]=1 and " + mpredicate + " group by r.tag, r.[content/description] order by r.tag", RegistryAdmin.PASS);
        }
