...
function createPullFromVoJob(path) {
    try
    {
        ...
        XML.Attrib("xmlns:xsi", "http://www.w3.org/2001/XMLSchema-instance");
        	XML.Node("vos:target","vos://"+space+"!vospace/"+path);
            ...
            XML.EndNode();
        	XML.BeginNode("vos:protocol");
        		XML.Attrib("uri", "ivo://ivoa.net/vospace/core#ftpget");
            XML.EndNode();
        XML.EndNode();
...
function createPullToVoJob(path, url, type) {
    try
    {
        var XML=new XMLWriter();
        XML.BeginNode("vos:transfer");
        ...
        XML.Attrib("xsi:schemaLocation", "http://www.ivoa.net/xml/UWS/v1.0 UWS.xsd");
        	...
        	XML.Node("vos:view","ivo://ivoa.net/vospace/core#defaultview");
        	XML.BeginNode("vos:protocol");
        		XML.Attrib("uri", "ivo://ivoa.net/vospace/core#"+type);
...
function createPushToVoJob(path) {
    try
    {
        var XML=new XMLWriter();
        XML.BeginNode("vos:transfer");
...
function createPushFromVoJob(path, url) {
    try
    {
        var XML=new XMLWriter();
        XML.BeginNode("vos:transfer");
...
function createNewNodeXml(type) {
    try
    {
        var XML=new XMLWriter();
        ...
        XML.Attrib("xmlns:vos", "http://www.ivoa.net/xml/VOSpace/v2.0");
        XML.Attrib("uri", "vos://"+space+"!vospace/");

        XML.Node("vos:properties", "");

        XML.BeginNode("vos:accepts");
        XML.BeginNode("vos:view");
        XML.Attrib("uri", "ivo://ivoa.net/vospace/core#anyview");
        XML.EndNode();
        XML.EndNode();
