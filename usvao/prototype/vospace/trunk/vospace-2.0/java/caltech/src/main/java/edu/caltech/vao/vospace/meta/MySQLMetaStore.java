...
public class MySQLMetaStore implements MetaStore{
	...
	try {
	    ...
	    // Connection URL
            ...
            connectionURL = "jdbc:mysql://" + DB_URL; 
	    //	    System.out.println(connectionURL);
	    // Get object pool
	    ObjectPool connectionPool = new GenericObjectPool(null);
	    // Get connection factory
	    ConnectionFactory connectionFactory = new DriverManagerConnectionFactory(connectionURL, DB_UID, DB_PWD);
...
//	query += whereQuery + " order by identifier ";
	...
	Statement statement = getConnection().createStatement();
	if (metadata instanceof String) {
	    String query = "update nodes set node = '" + (String) metadata + "' where identifier = '" + identifier + "'"; 
	    statement.executeUpdate(query);
	}
    }
    ...
    public void updateData(String identifier, String newIdentifier, Object metadata) throws SQLException {
        Statement statement = getConnection().createStatement();
	if (metadata instanceof String) {
	    String query = "update nodes set identifier = '" + newIdentifier + "', node = '" + (String) metadata + "' where identifier = '" + identifier + "'"; 
	    statement.executeUpdate(query);
	}
    }
    ...
    public void updateData(String identifier, String newIdentifier, String newLocation, Object metadata) throws SQLException {
        Statement statement = getConnection().createStatement();
	if (metadata instanceof String) {
	    String query = "update nodes set identifier = '" + newIdentifier + "', location = '" + newLocation + "', node = '" + (String) metadata + "' where identifier = '" + identifier + "'"; 
	    statement.executeUpdate(query);
	}
    }
	...
	String job = null;
	String query = "select job, t.completed from jobs j, transfers t where t.jobid = j.identifier and t.endpoint like '%" + endpoint + "'";
	ResultSet result = execute(query);
	return result;
    }
