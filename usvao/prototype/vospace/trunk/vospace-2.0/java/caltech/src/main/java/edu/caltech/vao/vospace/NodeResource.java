...
public class NodeResource extends VOSpaceResource {
    ...
    public Node getNode(@QueryParam("detail") String detail) throws VOSpaceException {
	    Node node = manager.getNode(ROOTNODE, detail);
	    return node;
    }
    ...
    @GET
    @Produces(MediaType.APPLICATION_XML)
    public Node getNode(@PathParam("nodeid") String nodeid, @QueryParam("detail") String detail) throws VOSpaceException {
	Node node = manager.getNode(getId(nodeid), detail);
	return node;
    }
    ...
    @Consumes(MediaType.APPLICATION_XML)
    @Produces(MediaType.APPLICATION_XML)
    public Node putNode(@PathParam("nodeid") String nodeid, Node node) throws VOSpaceException {	
	if (!node.getUri().equals(getId(nodeid))) throw new VOSpaceException(VOSpaceException.INTERNAL_SERVER_ERROR, "A specified URI is invalid");
